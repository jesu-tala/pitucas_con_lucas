(() => {
  ...
  phone.addEventListener("click", function(e) {
    ...
    if (groupSettleBtn) {
      ...
      if (mi) {
        ...
        if (monto > 0) {
          const promesa = miSaldo < 0 && suSaldo > 0 ? registerPaidBalance(gid, mi.id, otroId, monto) : miSaldo > 0 && suSaldo < 0 ? registerPaidBalance(gid, otroId, mi.id, monto) : Promise.resolve(false);
  ...
  function renderGroupDetail(groupId) {
    ...
    const balanceCard = '<div class="card stat-tile" style="padding:20px;text-align:center;margin-bottom:14px;background:' + (myBalance > 0 ? "var(--income-fill)" : myBalance < 0 ? "var(--expense-fill)" : "var(--surface)") + ';"><div class="stat-label">' + (myBalance === 0 ? "Est\xE1s al d\xEDa" : myBalance > 0 ? "Te deben en total" : "Debes en total") + '</div><div class="stat-value tabular" style="font-size:26px;color:' + (myBalance > 0 ? "var(--income-ink)" : myBalance < 0 ? "var(--expense-ink)" : "var(--text)") + ';">' + money(Math.abs(myBalance)) + "</div></div>";
    const breakdown = '<div class="sheet-block card" style="padding:16px;margin-bottom:14px;"><div class="sheet-block-title">Por persona</div>' + balances.map((s) => {
      return '<div class="split-row" style="align-items:center;">' + avatarHtml(s.nombre, s.color) + '<span style="flex:1;margin-left:10px;">' + s.nombre + '</span><span class="tabular" style="color:' + (s.balance > 0 ? "var(--income-ink)" : s.balance < 0 ? "var(--expense-ink)" : "var(--text-secondary)") + ';font-weight:600;">' + (s.balance === 0 ? "Al d\xEDa" : (s.balance > 0 ? "+" : "\u2212") + money(Math.abs(s.balance))) + "</span>" + (s.balance !== 0 ? '<button class="chip" style="margin-left:8px;" data-group-settle="' + groupId + "|" + s.participantId + '">Saldar</button>' : "") + "</div>";
