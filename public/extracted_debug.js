(() => {
  ...
  function catIconMarkup(name) {
    return ICONS[name] !== void 0 ? ICONS[name] : '<span class="emoji-icon">' + name + "</span>";
  }
  ...
  function renderGoalCard(meta) {
    ...
    return '<div class="card meta-goal-card"><div class="meta-goal-head"><span class="meta-goal-name">' + meta.nombre + "</span>" + termChip(meta.plazo) + (racha > 0 ? '<span class="meta-racha-badge">' + racha + " \u{1F525}</span>" : "") + '<button class="budget-edit-btn" data-edit-goal="' + meta.id + '" aria-label="Editar ' + meta.nombre + '">' + ICONS.edit + "</button></div>" + stockBlock + comisionRow + '<div class="meta-goal-spark-row"><div class="meta-goal-spark">' + buildSparkline(historialVals, 120, 32, "var(--accent)") + "</div>" + aporteBlock + '</div><div class="meta-check-row">' + checksRow + '</div><div class="meta-racha">' + (racha > 0 ? "Racha activa \u2014 cumpliste tu aporte " + racha + " " + (racha === 1 ? "mes" : "meses") + " seguidos hasta hoy" : "Sin racha activa \u2014 marca los meses en que cumpliste tu aporte") + "</div></div>";
  }
  ...
  function renderPlatformGroup(id) {
    ...
    // no pencil button.
    (sinValuacion ? "" : '<button class="budget-edit-btn" data-edit-platform="' + id + '" aria-label="Actualizar valor de ' + cat.nombre + '">' + ICONS.edit + "</button>") + '</div><div class="platform-goal-nest">' + metasBody + "</div></div>";
  ...
  function planMetaRowHtml(meta) {
    ...
    return '<div class="plan-row"><div class="plan-name">' + meta.nombre + "<small>Meta de aporte: " + money(meta.aporteMensualMeta) + '/mes</small></div><div class="plan-pctbox"><input type="text" inputmode="decimal" data-plan-goal-pct data-plan-goal-id="' + meta.id + '" value="' + pct + '"><span>%</span></div><div class="plan-amt tabular" data-plan-goal-amt="' + meta.id + '"></div></div>';
  }
  ...
  function menuScreenHead(title) {
    return '<div class="menu-screen-head"><button class="menu-back-btn" data-menu-back aria-label="Volver al men\xFA">' + ICONS.chevL + '</button><h2 class="menu-screen-title">' + title + "</h2></div>";
  }
  ...
  function renderMenuMedios() {
    ...
    const rows = Object.keys(PAYMENT_METHODS).map((id) => {
      ...
      return '<div class="card menu-item-card"><span class="menu-item-card-icon" style="--fill:var(--surface-sunken);--ink:var(--text-secondary);">' + ICONS[m.icon] + '</span><div class="menu-item-card-body"><div class="menu-item-card-name">' + m.nombre + '</div><div class="menu-item-card-sub">' + m.corto + '</div></div><div class="menu-item-card-actions"><button class="budget-edit-btn" data-edit-payment-method="' + id + '" aria-label="Editar ' + m.nombre + '">' + ICONS.edit + "</button></div></div>";
    }).join("");
  ...
  function renderDatosTransferenciaCard() {
    ...
    if (state.editingTransferInfo) {
      ...
      return '<div class="card" style="padding:16px;margin-top:14px;"><div class="budget-total-label">Datos de transferencia</div><p class="cat-picker-hint" style="margin:6px 0 10px;">Se usan solo para armar el texto que copias al pedir un cobro pendiente \u2014 no se comparten con nadie m\xE1s.</p><label class="draft-label">Nombre</label><input type="text" class="draft-input" data-transfer-info-input="nombre" value="' + dr.nombre + '" placeholder="Nombre completo"><label class="draft-label" style="margin-top:10px;">RUT</label><input type="text" class="draft-input" data-transfer-info-input="rut" value="' + dr.rut + '" placeholder="12.345.678-9"><label class="draft-label" style="margin-top:10px;">Banco</label><input type="text" class="draft-input" data-transfer-info-input="banco" value="' + dr.banco + '" placeholder="Ej: Banco Estado"><label class="draft-label" style="margin-top:10px;">Tipo de cuenta</label><input type="text" class="draft-input" data-transfer-info-input="tipoCuenta" value="' + dr.tipoCuenta + '" placeholder="Cuenta RUT, Vista, Corriente\u2026"><label class="draft-label" style="margin-top:10px;">N\xFAmero de cuenta</label><input type="text" class="draft-input" data-transfer-info-input="numeroCuenta" value="' + dr.numeroCuenta + '" placeholder="0000000000"><label class="draft-label" style="margin-top:10px;">Email (opcional)</label><input type="text" class="draft-input" data-transfer-info-input="email" value="' + dr.email + '" placeholder="tucorreo@ejemplo.cl"><div style="display:flex;gap:10px;margin-top:14px;"><button class="save-tx-btn" style="background:var(--surface-sunken);color:var(--text);flex:1;" data-cancel-transfer-info>Cancelar</button><button class="save-tx-btn" style="flex:1;" data-save-transfer-info>Guardar</button></div></div>';
    }
    const completos = transferInfoComplete();
    return '<div class="card" style="padding:16px;margin-top:14px;"><div class="budget-total-head"><span class="budget-total-label">Datos de transferencia</span><button class="budget-edit-btn" data-edit-transfer-info aria-label="Editar datos de transferencia">' + ICONS.edit + "</button></div>" + (completos ? '<div class="datos-transferencia-figs">' + (d.nombre ? "<div>" + d.nombre + "</div>" : "") + (d.rut ? "<div>RUT " + d.rut + "</div>" : "") + (d.banco || d.tipoCuenta ? "<div>" + [d.banco, d.tipoCuenta].filter(Boolean).join(" \xB7 ") + "</div>" : "") + (d.numeroCuenta ? "<div>Cuenta " + d.numeroCuenta + "</div>" : "") + (d.email ? "<div>" + d.email + "</div>" : "") + "</div>" : '<p class="cat-picker-hint" style="margin:6px 0 0;">Agr\xE9galos para poder copiar, listo para pegar en WhatsApp, un cobro pendiente junto con c\xF3mo te pueden transferir.</p>') + "</div>";
  }
  ...
  function syncSharedExpenses() {
    ...
    SHARED_EXPENSES.forEach((g) => {
      ...
      const tx = {
        ...
        porCobrar: [],
        reglaAuto: false,
        nota: 'Tu parte de "' + g.descripcion + '"' + (pagador ? " \u2014 pag\xF3 " + pagador.nombre : "") + (grupo ? " \xB7 grupo " + grupo.nombre : ""),
        groupId: g.grupo_id,
  ...
  function avatarHtml(nombre, color, size) {
    ...
    return '<span class="avatar-circle" style="width:' + s + "px;height:" + s + "px;font-size:" + Math.round(s * 0.42) + "px;--fill:var(--cat-" + (color || "lavender") + "-fill);--ink:var(--cat-" + (color || "lavender") + '-ink);">' + inicial + "</span>";
  }
  ...
  function renderGroupsList() {
    ...
    const cards = GROUPS.map((g) => {
      ...
      return '<li><button class="menu-list-item" data-group-open="' + g.id + '"><span class="menu-item-icon" style="font-size:20px;">' + (g.icono || "\u{1F465}") + '</span><span class="menu-item-label">' + g.nombre + '<span class="menu-item-sub">' + n + " participante" + (n === 1 ? "" : "s") + " \xB7 " + balanceTxt + '</span></span><span class="menu-item-chev">' + ICONS.chevL + "</span></button></li>";
    }).join("");
  ...
  function renderGroupExpenseDetailCard(gasto, groupId) {
    ...
    return '<div class="sheet-block card" style="padding:16px;margin-top:10px;"><div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">' + catAvatarHtml(ci) + '<span style="flex:1;"><span class="sheet-block-title" style="margin:0;display:block;">' + gasto.descripcion + '</span><span class="muted" style="font-size:12px;">' + dayLabel(gasto.fecha) + '</span></span><span class="tabular" style="font-weight:700;">' + money(gasto.monto) + '</span></div><div class="muted" style="font-size:12.5px;margin-bottom:8px;">Pag\xF3 ' + (pagador ? pagador.nombre : "?") + "</div>" + (gasto.reparto || []).map((r) => {
      const p = participantes.find((pp) => pp.id === r.participante_id);
      return '<div class="split-row" style="align-items:center;">' + avatarHtml(p ? p.nombre : "?", p ? p.color : "neutral", 24) + '<span style="flex:1;margin-left:8px;">' + (p ? p.nombre : "?") + '</span><span class="tabular muted">' + money(r.monto) + "</span></div>";
  ...
  function renderGroupGastosTab(groupId) {
    ...
    return '<div class="card stat-tile" style="padding:16px;text-align:center;margin-bottom:14px;"><div class="stat-label">Total gastado por el grupo</div><div class="stat-value tabular" style="font-size:24px;">' + money(total) + '</div></div><div class="sheet-block card" style="padding:16px;">' + (gastos.length ? '<div class="tx-list" style="box-shadow:none;border:none;">' + gastos.map((gc) => {
      ...
      return '<div class="tx-item" data-group-expense-open="' + gc.id + '">' + catAvatarHtml(ci) + '<span class="tx-info"><span class="tx-name">' + gc.descripcion + '</span><span class="tx-sub">' + dayLabel(gc.fecha) + " \xB7 pag\xF3 " + (pagador ? pagador.nombre : "?") + " \xB7 entre " + entre + '</span></span><span class="tx-right"><span class="tx-amount tabular">' + money(gc.monto) + "</span></span></div>";
  ...
  function renderGroupBalancesTab(groupId) {
    ...
    const transferSection = '<div class="sheet-block card" style="padding:16px;"><div class="sheet-block-title">Reembolsos sugeridos</div>' + (transfers.length ? transfers.map((t) => {
      ...
      return '<div class="split-row" style="align-items:center;' + (involvesMe ? "background:var(--accent-soft);border-radius:10px;padding:6px 8px;margin:2px -8px;" : "") + '">' + avatarHtml(from ? from.nombre : "?", from ? from.color : "neutral", 26) + '<span style="flex:1;margin-left:8px;">' + (from ? from.nombre : "?") + " \u2192 " + (to ? to.nombre : "?") + '</span><span class="tabular" style="font-weight:600;margin-right:8px;">' + money(t.monto) + '</span><button class="chip" data-mark-transfer-paid="' + groupId + "|" + t.from + "|" + t.to + "|" + Math.round(t.monto) + '">Marcar como pagado</button></div>';
  ...
  function segmentedHtml(name, options, value, disabled) {
    return '<div class="segmented" data-seg="' + name + '">' + options.map(
      (o) => '<button data-seg-val="' + o.id + '" class="' + (value === o.id ? "active" : "") + '" ' + (disabled ? "disabled" : "") + ">" + o.label + "</button>"
    ).join("") + "</div>";
  }
  ...
  function renderReceiptAssign() {
    ...
    const itemBlocks = b.items.map((item) => {
      ...
      return '<div class="card boleta-item-block"><div class="boleta-item-head"><span class="boleta-item-name">' + item.nombre + '</span><span class="boleta-item-amt tabular">' + money(item.monto) + '</span></div><div class="boleta-person-chips">' + chips + "</div>" + (asignados.length === 0 ? '<div class="file-format-hint" style="margin-top:8px;">Sin asignar todav\xEDa.</div>' : "") + "</div>";
    }).join("");
  ...
  function chipToggle(attrName, id, label, icon2, active) {
    return '<button class="cat-picker-chip" data-' + attrName + '="' + id + '" ' + (active ? 'style="background:var(--accent-soft);border-color:var(--accent);color:var(--accent-ink);"' : "") + ">" + (icon2 ? catIconMarkup(icon2) + " " : "") + label + "</button>";
  }
  ...
  __name(clearAppHeight, "clearAppHeight");
  ...
  __name(onViewportResize, "onViewportResize");
  clearAppHeight();
