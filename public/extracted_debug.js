(() => {
  ...
  function renderNewTxSheetContent(d) {
    ...
    (d.tipo === "gasto" ? renderQuickActionsBlock(d) : "") + '<div class="sheet-block card" style="padding:16px;"><div class="sheet-block-title">Medio de pago</div>' + (state.addingPaymentMethod ? "" : '<select class="draft-select" data-draft-field="medio">' + medioOpts + "</select>") + newPaymentMethodForm + '</div><button class="save-tx-btn" data-save-draft="1" ' + (canSave ? "" : "disabled") + ">Guardar transacci\xF3n</button>" + (canSave ? "" : '<div class="field-error">Ponle un nombre de comercio y un monto para poder guardar.</div>');
  }
