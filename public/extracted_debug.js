(() => {
  ...
  function monthlyReimbursementTotal(monthKey) {
  ...
  function catMonthExpense(catId, monthKey) {
    return txsOfMonth(monthKey).filter((t) => t.tipo === "gasto" && t.estado !== "no_es_gasto").reduce((sum, t) => sum + t.categorias.filter((c) => c.cat === catId).reduce((s, c) => s + catNetAmount(t, c), 0), 0);
  }
  ...
  function platformGeneralAmountHastaMes(platformId, monthKey) {
    const generalId = generalCatIdFor(platformId);
    let total = 0;
    TRANSACTIONS.forEach((t) => {
      if (t.tipo !== "inversion") return;
  ...
  function platformAportadoNetoHastaMes(id, monthKey) {
  ...
  function valorTotalEnMesONull(monthKey) {
    ...
    return activePlatformIds().reduce(
      (s, id) => s + (PLATFORM_DATA[id].sinValuacion ? platformAportadoNetoHastaMes(id, monthKey) : PLATFORM_DATA[id].valorHistorial[monthKey] || 0),
      0
    );
  }
  ...
  function metaHistorialAt(meta, monthKey) {
    const start = meta.startMonth || monthKey;
    if (monthKey < start) return null;
    const contribs = metaContribTxs(meta).filter((t) => t.fecha.slice(0, 7) <= monthKey).reduce((s, t) => s + metaContribAmount(t, meta.id), 0);
    return (meta.startingAmount || 0) + contribs;
  }
