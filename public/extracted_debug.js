(() => {
  ...
  function icon(name, cls) {
    return '<span class="icon-wrap' + (cls ? " " + cls : "") + '">' + ICONS[name] + "</span>";
  }
  __name(icon, "icon");
  function catIconMarkup(name) {
    return ICONS[name] !== void 0 ? ICONS[name] : '<span class="emoji-icon">' + esc(name) + "</span>";
  }
  __name(catIconMarkup, "catIconMarkup");

  // src/helpers.ts
  function txsOfMonth(m) {
  ...
  __name(termChip, "termChip");
  function paymentMethodInfo(id) {
    return PAYMENT_METHODS[id] || { nombre: "Medio desconocido", corto: "Sin medio", icon: "card" };
  }
  __name(paymentMethodInfo, "paymentMethodInfo");
  function paymentMethodTagIcon(medio) {
    if (medio.icon === "card") return "\u{1F4B3}";
    if (medio.icon === "cash") return "\u{1F4B0}";
    return "";
  }
  __name(paymentMethodTagIcon, "paymentMethodTagIcon");
  function catTotalAmount(t) {
    return t.categorias.reduce((s, c) => s + c.monto, 0);
  }
  ...
  __name(netExpenseTx, "netExpenseTx");
  function netExpenseFactor(t) {
    const gross = catTotalAmount(t);
    return gross > 0 ? netExpenseTx(t) / gross : 1;
  }
  __name(netExpenseFactor, "netExpenseFactor");
  function catNetAmount(t, c) {
    if (t.tipo !== "gasto") return c.monto;
    return c.monto * netExpenseFactor(t);
  }
  __name(catNetAmount, "catNetAmount");
  function reimbursementExcess(t) {
  ...
  function aggregatedTxAmount(t) {
    if (t.tipo === "gasto") return netExpenseTx(t);
    if (t.tipo === "ingreso") return netIncomeTx(t);
    return catTotalAmount(t);
  }
  __name(aggregatedTxAmount, "aggregatedTxAmount");
  function allPendingReceivables() {
    const out = [];
    TRANSACTIONS.forEach((t) => {
      (t.porCobrar || []).forEach((p, idx) => {
        if (p.pagado) return;
  ...
  function applyUnexpectedReimbursement(gastoTxId, incomeTxId) {
    ...
    gastoTx.porCobrar.push({
      persona: incomeTx.comercio || "Reembolso",
      monto: incomeTx.monto,
      pagado: true,
      tipo: "reembolso",
      montoRecibido: incomeTx.monto,
  ...
  function writeOffReceivable(expenseTxId, idx) {
    const expenseTx = getTx(expenseTxId);
    if (!expenseTx || !expenseTx.porCobrar[idx]) return false;
    const p = expenseTx.porCobrar[idx];
    ...
    const newTx = {
      ...
      fecha: todayISO(),
      hora: "12:00",
      comercio: (p.persona || "Cuenta por cobrar") + " \u2014 nunca pag\xF3",
      monto: amount,
      medio: expenseTx.medio,
      tipo: "gasto",
  ...
  function reimbursementTotalForMonths(monthKeys) {
    let total = 0, count = 0;
    TRANSACTIONS.forEach((t) => {
  ...
  __name(reimbursementTotalForMonths, "reimbursementTotalForMonths");
  function monthlyReimbursementTotal(monthKey2) {
    return reimbursementTotalForMonths([monthKey2]);
  }
  __name(monthlyReimbursementTotal, "monthlyReimbursementTotal");
  ...
  __name(applyLockRule, "applyLockRule");
  function applyCuotaMonto(t, newMonto) {
    t.monto = newMonto;
  ...
  __name(navPush, "navPush");
  function navPop() {
    return stack.length ? stack.pop() : null;
  }
  __name(navPop, "navPop");
  function navPeek() {
    return stack.length ? stack[stack.length - 1] : null;
  }
  __name(navPeek, "navPeek");
  function navDepth() {
    return stack.length;
  }
  __name(navDepth, "navDepth");
  function navClear() {
    stack.length = 0;
  }
  __name(navClear, "navClear");
  function navPopIfTop(type) {
  ...
  __name(navPopIfTop, "navPopIfTop");
  function navClearType(type) {
    stack = stack.filter((f) => f.type !== type);
  }
  __name(navClearType, "navClearType");
  function navStackSnapshot() {
    return stack.slice();
  }
  __name(navStackSnapshot, "navStackSnapshot");
  ...
  function renderTabbar() {
    const tabs = [
      ...
      { id: "menu", label: "Men\xFA", icon: "menu" }
    ];
    document.getElementById("tabbar").innerHTML = tabs.map(
      (t) => '<button class="tab ' + (state.tab === t.id ? "active" : "") + '" data-tab="' + t.id + '">' + ICONS[t.icon] + "<span>" + t.label + "</span></button>"
    ).join("");
  }
  __name(renderTabbar, "renderTabbar");
  ...
  // src/views/evolucion.ts
  function monthTotals(monthKey2) {
    const monthTx = txsOfMonth(monthKey2);
    ...
    monthTx.forEach((t) => {
      ...
      if (t.tipo === "ingreso") {
        ...
        else if (nature === "reembolso") ingresos += incomeNatureAmount(t);
  ...
  __name(monthTotals, "monthTotals");
  function yearTotals(year) {
    ...
    months.forEach((m) => {
      ...
      cobros += t.cobros;
      movimientoCapital += t.movimientoCapital;
      porClasificar += t.porClasificar;
    });
    return {
      year,
      months,
  ...
  function projectedContributions(mesesPromedio, aniosProyeccion) {
    ...
    const promedioMensual = mesesConDatos.length ? mesesConDatos.reduce((s, m) => s + monthTotals(m).inversiones, 0) / mesesConDatos.length : 0;
  ...
  function buildSparkline(values, w, h, color) {
    ...
    return '<svg viewBox="0 0 ' + w + " " + h + '" width="' + w + '" height="' + h + '" style="display:block;overflow:visible;"><path d="' + d + '" fill="none" stroke="' + color + '" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><circle cx="' + last[0].toFixed(1) + '" cy="' + last[1].toFixed(1) + '" r="3" fill="' + color + '"/></svg>';
  }
  __name(buildSparkline, "buildSparkline");
  function buildEvolutionBars(months, selMonth) {
    ...
    months.forEach((m, i) => {
      ...
      out += '<g data-evo-month="' + m + '" style="cursor:pointer;"><rect x="' + gx.toFixed(1) + '" y="0" width="' + groupW.toFixed(1) + '" height="' + (padTop + chartH + 4).toFixed(1) + '" rx="8" fill="' + (isSelected ? "var(--surface-sunken)" : "transparent") + '"/>' + bars + '<text x="' + (gx + groupW / 2).toFixed(1) + '" y="' + (H - 6) + '" text-anchor="middle" font-size="9.5" fill="' + (isSelected ? "var(--text)" : "var(--text-tertiary)") + '" font-weight="' + (isSelected ? "700" : "400") + '">' + short + "</text></g>";
    });
    return '<svg viewBox="0 0 ' + W + " " + H + '" width="100%" style="display:block;overflow:visible;">' + out + "</svg>";
  }
  __name(buildEvolutionBars, "buildEvolutionBars");
  function metaContribTxs(meta) {
    return TRANSACTIONS.filter((t) => t.tipo === "inversion" && t.categorias.some((c) => c.cat === meta.id));
  }
  __name(metaContribTxs, "metaContribTxs");
  function metaContribAmount(t, metaId) {
    return t.categorias.filter((c) => c.cat === metaId).reduce((s, c) => s + c.monto, 0);
  }
  __name(metaContribAmount, "metaContribAmount");
  function metaAportadoNeto(meta) {
    const contribs = metaContribTxs(meta).reduce((s, t) => s + metaContribAmount(t, meta.id), 0);
    return (meta.startingAmount || 0) + contribs;
  }
  __name(metaAportadoNeto, "metaAportadoNeto");
  function metaHistorialAt(meta, monthKey2) {
    const start = meta.startMonth || monthKey2;
    if (monthKey2 < start) return null;
    const contribs = metaContribTxs(meta).filter((t) => t.fecha.slice(0, 7) <= monthKey2).reduce((s, t) => s + metaContribAmount(t, meta.id), 0);
    return (meta.startingAmount || 0) + contribs;
  }
  __name(metaHistorialAt, "metaHistorialAt");
  ...
  __name(metaMonths, "metaMonths");
  function metaAcumuladoActual(meta) {
    return metaAportadoNeto(meta);
  }
  __name(metaAcumuladoActual, "metaAcumuladoActual");
  ...
  __name(metaGananciaEstimada, "metaGananciaEstimada");
  ...
  __name(metaRacha, "metaRacha");
  function goalsForPlatform(id) {
  ...
  function annualInvestmentGoalProgress(year) {
    ...
    const aporteAnio = fixedGoals.reduce(
      (s, m) => s + metaContribTxs(m).filter((t) => t.fecha.slice(0, 4) === year && t.estado !== "no_es_gasto").reduce((s2, t) => s2 + metaContribAmount(t, m.id), 0),
      0
    );
  ...
  __name(annualInvestmentGoalProgress, "annualInvestmentGoalProgress");
  ...
  __name(metaTotalRacha, "metaTotalRacha");
  function renderTotalChecksGrid() {
    const year = todayISO().slice(0, 4);
    const months = fullYearMonths(year);
    ...
    const cells = months.map((m) => {
      ...
      return '<button class="meta-total-check-cell' + (checked ? " done" : "") + '" data-toggle-goal-total-check="' + m + '" aria-pressed="' + (checked ? "true" : "false") + '" aria-label="' + label + " " + year + (checked ? ": objetivo total cumplido" : ": no marcado") + '"><span class="mcc-icon">' + (checked ? ICONS.check : ICONS.close) + '</span><span class="mcc-label">' + label + "</span></button>";
    }).join("");
    return '<div class="meta-total-checks-label muted" style="display:flex;align-items:center;gap:6px;justify-content:space-between;"><span>\xBFCumpliste tu objetivo total cada mes?</span>' + (racha > 0 ? '<span class="meta-racha-badge">' + racha + " \u{1F525}</span>" : "") + '</div><div class="meta-total-checks-grid">' + cells + '</div><div class="meta-racha">' + (racha > 0 ? "Racha activa \u2014 cumpliste tu objetivo total " + racha + " " + (racha === 1 ? "mes" : "meses") + " seguidos hasta hoy" : "Sin racha activa \u2014 marca los meses en que cumpliste tu objetivo total") + "</div>";
  }
  __name(renderTotalChecksGrid, "renderTotalChecksGrid");
  function renderGoalEditForm(meta, plataformaId) {
  ...
  __name(metaChecksMonths, "metaChecksMonths");
  function renderGoalCard(meta) {
    ...
    const comisionRow = comision != null ? '<div class="platform-comision-row"><span>Comisi\xF3n anual: <b class="tabular">' + comision + '%</b></span><span class="muted tabular">\u2248 ' + money(gananciaMeta * comision / 100) + "/a\xF1o sobre tu ganancia</span></div>" : "";
    ...
    const checksRow = metaChecksMonths(meta).map((m) => {
      ...
      return '<button class="meta-check-chip' + (done ? " done" : "") + '" data-toggle-goal-check="' + meta.id + '" data-toggle-goal-month="' + m + '"><span class="mcc-icon">' + (done ? ICONS.check : ICONS.close) + '</span><span class="mcc-label">' + short + "</span></button>";
    }).join("");
    const stockBlock = tieneObjetivo ? '<div class="meta-goal-figs"><span class="tabular gastado">' + money(acumulado) + '</span><span class="of-text"> de ' + money(objetivoEfectivo) + (objetivoEsEstimado ? " (estimado)" : "") + '</span><span class="budget-pct tabular">' + Math.round(pct) + '%</span></div><div class="budget-track"><div class="budget-fill" style="width:' + Math.max(0, Math.min(100, pct)) + '%;background:var(--accent);"></div></div>' : '<div class="meta-goal-figs"><span class="tabular gastado">' + money(acumulado) + '</span><span class="of-text"> aportado hasta ahora</span></div>';
    const aporteBlock = meta.aporteMensualMeta != null ? '<div class="meta-goal-aporte muted">Meta de aporte<br><span class="tabular" style="color:var(--text);font-weight:600;">' + money(meta.aporteMensualMeta) + "</span>/mes</div>" : "";
    return '<div class="card meta-goal-card"><div class="meta-goal-head"><span class="meta-goal-name">' + esc(meta.nombre) + "</span>" + termChip(meta.plazo) + (racha > 0 ? '<span class="meta-racha-badge">' + racha + " \u{1F525}</span>" : "") + '<button class="budget-edit-btn" data-edit-goal="' + meta.id + '" aria-label="Editar ' + esc(meta.nombre) + '">' + ICONS.edit + "</button></div>" + stockBlock + comisionRow + '<div class="meta-goal-spark-row"><div class="meta-goal-spark">' + buildSparkline(historialVals, 120, 32, "var(--accent)") + "</div>" + aporteBlock + '</div><div class="meta-check-row">' + checksRow + '</div><div class="meta-racha">' + (racha > 0 ? "Racha activa \u2014 cumpliste tu aporte " + racha + " " + (racha === 1 ? "mes" : "meses") + " seguidos hasta hoy" : "Sin racha activa \u2014 marca los meses en que cumpliste tu aporte") + "</div></div>";
  }
  __name(renderGoalCard, "renderGoalCard");
  ...
  __name(fullYearMonths, "fullYearMonths");
  function renderEvolutionView() {
    ...
    const legendRow = '<div class="evo-legend-row"><span class="evo-legend-item"><span class="evo-dot" style="background:var(--income-fill)"></span>Ingresos</span><span class="evo-legend-item"><span class="evo-dot" style="background:var(--expense-fill)"></span>Gastos</span><span class="evo-legend-item"><span class="evo-dot" style="background:var(--invest-fill)"></span>Inversiones</span></div>';
    const detailRow = '<div class="evo-detail-row"><div class="evo-detail-item"><span class="evo-detail-label">Ingresos</span><span class="evo-detail-value tabular">' + money(sel.ingresos) + '</span></div><div class="evo-detail-item"><span class="evo-detail-label">Gastos</span><span class="evo-detail-value tabular">' + money(sel.gastos) + '</span></div><div class="evo-detail-item"><span class="evo-detail-label">Inversiones</span><span class="evo-detail-value tabular">' + money(sel.inversiones) + '</span></div><div class="evo-detail-item"><span class="evo-detail-label">Tasa de ahorro</span><span class="evo-detail-value tabular">' + Math.round(sel.tasaAhorro) + '%</span></div><div class="evo-detail-item"><span class="evo-detail-label">Tasa de gastos</span><span class="evo-detail-value tabular">' + Math.round(sel.tasaGastos) + "%</span></div></div>";
    const yr = yearTotals(selMonth.slice(0, 4));
    const yearRow = '<div class="evo-detail-row"><div class="evo-detail-item"><span class="evo-detail-label">Ingresos</span><span class="evo-detail-value tabular">' + money(yr.ingresos) + '</span></div><div class="evo-detail-item"><span class="evo-detail-label">Gastos</span><span class="evo-detail-value tabular">' + money(yr.gastos) + '</span></div><div class="evo-detail-item"><span class="evo-detail-label">Inversiones</span><span class="evo-detail-value tabular">' + money(yr.inversiones) + '</span></div><div class="evo-detail-item"><span class="evo-detail-label">Tasa de ahorro</span><span class="evo-detail-value tabular">' + Math.round(yr.tasaAhorro) + '%</span></div><div class="evo-detail-item"><span class="evo-detail-label">Tasa de gastos</span><span class="evo-detail-value tabular">' + Math.round(yr.tasaGastos) + "%</span></div></div>";
    const html = '<div class="section-title" style="margin-top:4px;">Ingresos, gastos e inversiones por mes</div><div class="card evo-card">' + legendRow + buildEvolutionBars(months, selMonth) + '<div class="evo-caption muted">Toca un mes para ver el detalle</div><div class="evo-detail-month">' + MONTH_LABEL[selMonth] + "</div>" + detailRow + '</div><div class="section-title">Total del a\xF1o ' + yr.year + '</div><div class="card evo-card"><div class="evo-detail-month">Enero \u2013 Diciembre ' + yr.year + "</div>" + yearRow + // Composition (donut by category, Fijo/Variable/Inversión) lives in Balance, not here --
    ...
    '<button class="budget-ver-mas" data-goto-balance-anual>Ver desglose del a\xF1o \u2192</button></div><div class="evo-caption muted" style="padding:0 4px;">\xBFBuscas tus metas de inversi\xF3n y tus plataformas? Ahora viven juntas en <b>Inversiones</b>.</div><div style="height:12px;"></div>';
  ...
  __name(referenceMonthlyIncome, "referenceMonthlyIncome");
  function investmentGoalPct() {
    const ref = referenceMonthlyIncome();
    return ref > 0 ? monthlyInvestmentGoalCLP() / ref * 100 : 0;
  }
  __name(investmentGoalPct, "investmentGoalPct");
  function sumSpendingGoalPct() {
    return SPENDING_GOAL_PCT.fijo + SPENDING_GOAL_PCT.variable + investmentGoalPct();
  }
  __name(sumSpendingGoalPct, "sumSpendingGoalPct");
  function goalZones(metaPct, masEsMejor) {
  ...
  __name(goalZones, "goalZones");
  function goalZoneRow(nombre, pct, monto, zones, sinIngresos, metaPct) {
    ...
    const goalMarker = sinIngresos || metaPct == null ? "" : '<div class="meta-goal-marker" style="left:' + Math.max(0, Math.min(100, metaPct)) + '%">' + Math.round(metaPct) + "%</div>";
    let statusHtml;
    if (sinIngresos) {
    ...
    return '<div class="meta-row"><div class="meta-row-head"><span class="meta-row-name">' + nombre + '</span><span class="meta-row-figs"><span class="meta-row-pct tabular">' + pctText + '</span><span class="meta-row-amt tabular">' + money(monto) + '</span></span></div><div class="meta-track-wrap">' + goalMarker + '<div class="meta-track" style="background:' + gradient + '">' + marker + "</div></div>" + statusHtml + "</div>";
  }
  __name(goalZoneRow, "goalZoneRow");
  function renderGoalSummaryCard(periodTx, ingresos, metaInvPct) {
    ...
    const avisoSuma = sumaMetas > 100 ? '<div class="meta-caption warn">Ojo: tus 3 metas suman ' + Math.round(sumaMetas) + "% de tus ingresos \u2014 eso es m\xE1s del 100%, no calzan entre ellas. Ajusta Fijo/Variable en Presupuesto.</div>" : "";
    return '<div class="card meta-card"><div class="donut-card-title">Fijo \xB7 Variable \xB7 Inversi\xF3n</div><div class="donut-card-sub">Como porcentaje de tus ingresos del mes, contra tus propias metas</div>' + goalZoneRow("Gasto fijo", pctFijo, fijo, goalZones(SPENDING_GOAL_PCT.fijo, false), sinIngresos, SPENDING_GOAL_PCT.fijo) + goalZoneRow("Gasto variable", pctVar, variable, goalZones(SPENDING_GOAL_PCT.variable, false), sinIngresos, SPENDING_GOAL_PCT.variable) + goalZoneRow("Inversi\xF3n", pctInv, inversion, goalZones(metaInvPct, true), sinIngresos, metaInvPct) + '<div class="meta-caption">"Fijo" = tus gastos con recurrencia mensual o anual \xB7 "Variable" = el resto \xB7 tu meta de Inversi\xF3n (' + Math.round(metaInvPct) + "%) sale sola de lo que ya definiste en Inversiones. Edita Fijo/Variable en Presupuesto.</div>" + avisoSuma + "</div>";
  }
  __name(renderGoalSummaryCard, "renderGoalSummaryCard");
  ...
  function yearSwitcherHtml(year) {
    return '<div class="month-switcher"><span class="m-label">A\xF1o ' + year + "</span></div>";
  }
  ...
  __name(renderOtrasEntradasCards, "renderOtrasEntradasCards");
  function catMonthExpense(catId, monthKey2) {
    return txsOfMonth(monthKey2).filter((t) => t.tipo === "gasto" && t.estado !== "no_es_gasto").reduce((sum, t) => sum + t.categorias.filter((c) => c.cat === catId).reduce((s, c) => s + catNetAmount(t, c), 0), 0);
  }
  __name(catMonthExpense, "catMonthExpense");
  function priorMonths(monthKey2, n) {
    const idx = MONTHS.indexOf(monthKey2);
    const result = [];
  ...
  __name(priorMonths, "priorMonths");
  function catPromedio3Meses(catId, monthKey2) {
    const prior = priorMonths(monthKey2, 3);
    if (prior.length === 0) return null;
    const total = prior.reduce((s, m) => s + catMonthExpense(catId, m), 0);
    return total / prior.length;
  }
  __name(catPromedio3Meses, "catPromedio3Meses");
  function catGastoMesAnterior(catId, monthKey2) {
    const prior = priorMonths(monthKey2, 1);
    if (prior.length === 0) return null;
    return catMonthExpense(catId, prior[0]);
  }
  __name(catGastoMesAnterior, "catGastoMesAnterior");
  function budgetZoneColor(pct) {
    return pct >= 100 ? "var(--expense-fill)" : pct >= 80 ? "var(--cat-butter-fill)" : "var(--income-fill)";
  }
  __name(budgetZoneColor, "budgetZoneColor");
  ...
  __name(budgetAlertBadge, "budgetAlertBadge");
  ...
  __name(sumaPresupuestosCategorias, "sumaPresupuestosCategorias");
  function renderBudgetCatsCalce(meta) {
    const sumaCats = sumaPresupuestosCategorias();
    if (sumaCats === 0) return "";
    const diff = meta - sumaCats;
    if (Math.abs(diff) < 1) {
      return '<div class="budget-cats-calce ok">' + ICONS.checkCircle + " Tus categor\xEDas calzan justo con el presupuesto total (" + money(sumaCats) + ").</div>";
    }
    if (diff > 0) {
      return '<div class="budget-cats-calce">Categor\xEDas: ' + money(sumaCats) + " asignados \xB7 quedan " + money(diff) + " del total sin repartir.</div>";
    }
    return '<div class="budget-cats-calce warn">Categor\xEDas: ' + money(sumaCats) + " asignados \xB7 " + money(Math.abs(diff)) + " m\xE1s que tu presupuesto total.</div>";
  }
  __name(renderBudgetCatsCalce, "renderBudgetCatsCalce");
  function renderBudgetTotalCard(month) {
    ...
    if (state.editingBudgetTotal) {
      return '<div class="card budget-total-card editing"><div class="budget-total-label">Presupuesto total del mes</div><input type="text" inputmode="decimal" class="draft-input tabular" data-budget-total-input value="' + state.budgetTotalDraft + '" placeholder="0" style="margin-top:8px;"><div style="display:flex;gap:10px;margin-top:12px;"><button class="save-tx-btn" style="background:var(--surface-sunken);color:var(--text);flex:1;" data-cancel-budget-total>Cancelar</button><button class="save-tx-btn" style="flex:1;" data-save-budget-total>Guardar</button></div></div>';
    }
    return '<div class="card budget-total-card"><div class="budget-total-head"><span class="budget-total-label">Presupuesto total del mes</span><button class="budget-edit-btn" data-edit-budget-total aria-label="Editar presupuesto total">' + ICONS.edit + '</button></div><div class="budget-total-note muted">Son tus gastos del mes \u2014 no incluye lo que aportas a inversi\xF3n (eso se ve aparte en Balance).</div><div class="budget-total-figs"><span class="tabular gastado">' + money(gastoTotal) + '</span><span class="of-text"> de ' + money(meta) + "</span></div>" + renderBudgetBar(pct) + '<div class="budget-total-remaining muted">' + (restante >= 0 ? "Te quedan " + money(restante) + " \xB7 llevas el " + Math.round(pct) + "% de tu presupuesto" : "Te pasaste por " + money(Math.abs(restante)) + " \xB7 ya usaste el " + Math.round(pct) + "% de tu presupuesto") + "</div>" + renderBudgetCatsCalce(meta) + "</div>";
  }
  __name(renderBudgetTotalCard, "renderBudgetTotalCard");
  ...
  function renderBudgetView() {
    ...
    const conHtml = conPresupuesto.length ? conPresupuesto.map(renderBudgetCatCard).join("") : '<div class="empty-state" style="padding:20px 4px;">' + ICONS.inbox + "<div>Todav\xEDa no tienes categor\xEDas con presupuesto.</div></div>";
    const sinHtml = sinPresupuesto.map(renderBudgetCatCard).join("");
    document.getElementById("resumen-content").innerHTML = monthSwitcherHtml() + renderBudgetTotalCard(month) + renderMetasGastoCard() + '<div class="section-title">Categor\xEDas con presupuesto</div>' + conHtml + (sinPresupuesto.length ? '<div class="section-title">Sin presupuesto</div>' + sinHtml : "") + '<div style="height:12px;"></div>';
  }
  __name(renderBudgetView, "renderBudgetView");
  function balancePeriodoSelectorHtml() {
    return segmentedHtml("balance-periodo", [{ id: "mes", label: "Mes" }, { id: "a\xF1o", label: "A\xF1o" }], state.balancePeriodo);
  }
  __name(balancePeriodoSelectorHtml, "balancePeriodoSelectorHtml");
  function renderBalanceView() {
    if (state.balancePeriodo === "a\xF1o") {
      renderBalanceViewAnio();
      return;
    }
    const month = MONTHS[state.monthIndex];
    const monthTx = txsOfMonth(month);
    const mt = monthTotals(month);
    const html = balancePeriodoSelectorHtml() + monthSwitcherHtml() + '<div class="stat-grid"><div class="card stat-tile stat-ingresos"><div class="stat-label">Ingreso</div><div class="stat-value tabular">' + money(mt.ingresos) + '</div></div><div class="card stat-tile stat-gastos"><div class="stat-label">Gastos</div><div class="stat-value tabular">' + money(mt.gastos) + '</div></div><div class="card stat-tile stat-inversiones"><div class="stat-label">Inversiones</div><div class="stat-value tabular">' + money(mt.inversiones) + '</div></div><div class="card stat-tile stat-balance"><div class="stat-label">Balance</div><div class="stat-value tabular" style="color:' + (mt.balance >= 0 ? "var(--income-ink)" : "var(--expense-ink)") + '">' + money(mt.balance) + "</div></div></div>" + renderReembolsoCard(monthlyReimbursementTotal(month), "Reembolsado este mes") + renderOtrasEntradasCards(mt) + renderGoalSummaryCard(monthTx, mt.ingresos, investmentGoalPct()) + renderDonutBlock("Ingresos por categor\xEDa", "De d\xF3nde lleg\xF3 la plata este mes", "ingreso", monthTx, month) + renderDonutBlock("Gastos por categor\xEDa", "A d\xF3nde se te fue la plata este mes", "gasto", monthTx, month) + renderDonutBlock("Inversiones por categor\xEDa", "Tus aportes por plataforma este mes", "inversion", monthTx, month);
    document.getElementById("resumen-content").innerHTML = html;
  }
  ...
  function renderBalanceViewAnio() {
    ...
    const html = balancePeriodoSelectorHtml() + yearSwitcherHtml(year) + '<div class="stat-grid"><div class="card stat-tile stat-ingresos"><div class="stat-label">Ingreso</div><div class="stat-value tabular">' + money(yr.ingresos) + '</div></div><div class="card stat-tile stat-gastos"><div class="stat-label">Gastos</div><div class="stat-value tabular">' + money(yr.gastos) + '</div></div><div class="card stat-tile stat-inversiones"><div class="stat-label">Inversiones</div><div class="stat-value tabular">' + money(yr.inversiones) + '</div></div><div class="card stat-tile stat-balance"><div class="stat-label">Balance</div><div class="stat-value tabular" style="color:' + (yr.balance >= 0 ? "var(--income-ink)" : "var(--expense-ink)") + '">' + money(yr.balance) + "</div></div></div>" + renderReembolsoCard(reimbursementTotalForMonths(fullYearMonths(year)), "Reembolsado este a\xF1o") + renderOtrasEntradasCards(yr) + renderGoalSummaryCard(yearTx, yr.ingresos, metaInvPctAnio) + renderDonutBlock("Ingresos por categor\xEDa", "De d\xF3nde lleg\xF3 la plata este a\xF1o", "ingreso", yearTx, year) + renderDonutBlock("Gastos por categor\xEDa", "A d\xF3nde se te fue la plata este a\xF1o", "gasto", yearTx, year) + renderDonutBlock("Inversiones por categor\xEDa", "Tus aportes por plataforma este a\xF1o", "inversion", yearTx, year);
    document.getElementById("resumen-content").innerHTML = html;
  }
  __name(renderBalanceViewAnio, "renderBalanceViewAnio");
  function renderReembolsoCard(r, label) {
    if (r.count === 0) return "";
    return '<div class="card reembolso-card"><span class="reembolso-icon">' + ICONS.checkCircle + '</span><div><div class="reembolso-label">' + label + '</div><div class="reembolso-value tabular">' + money(r.total) + "</div></div></div>";
  }
  __name(renderReembolsoCard, "renderReembolsoCard");
  function renderComingSoon(sub) {
    document.getElementById("resumen-content").innerHTML = '<div class="card placeholder-card">' + ICONS.sparkle + "<h3>Pr\xF3ximamente</h3><p>Esta secci\xF3n todav\xEDa no est\xE1 lista.</p></div>";
  }
  __name(renderComingSoon, "renderComingSoon");

  // src/views/inversiones.ts
  function platformIds() {
    return Object.keys(CATEGORIES).filter((k) => CATEGORIES[k].tipo === "inversion");
  }
  __name(platformIds, "platformIds");
  function isPlatformArchived(id) {
    return !!(PLATFORM_DATA[id] && PLATFORM_DATA[id].archivada);
  }
  __name(isPlatformArchived, "isPlatformArchived");
  function activePlatformIds() {
    return platformIds().filter((id) => !isPlatformArchived(id));
  }
  __name(activePlatformIds, "activePlatformIds");
  function archivedPlatformIds() {
    return platformIds().filter((id) => isPlatformArchived(id));
  }
  __name(archivedPlatformIds, "archivedPlatformIds");
  function goalCapablePlatformIds() {
    return activePlatformIds().filter((id) => !PLATFORM_DATA[id].sinValuacion);
  }
  __name(goalCapablePlatformIds, "goalCapablePlatformIds");
  function generalCatIdFor(platformId) {
    return platformId + "__general";
  }
  __name(generalCatIdFor, "generalCatIdFor");
  function platformGeneralAmount(platformId) {
    const generalId = generalCatIdFor(platformId);
    let total = 0;
    TRANSACTIONS.forEach((t) => {
      if (t.tipo !== "inversion") return;
      t.categorias.forEach((c) => {
        if (c.cat === generalId) total += c.monto;
      });
    });
    return total;
  }
  __name(platformGeneralAmount, "platformGeneralAmount");
  function platformAportadoNeto(id) {
    return goalsForPlatform(id).reduce((s, m) => s + metaAportadoNeto(m), 0) + platformGeneralAmount(id);
  }
  __name(platformAportadoNeto, "platformAportadoNeto");
  function investmentCatOptions(selectedId) {
    const out = [];
    platformIds().forEach((platId) => {
      ...
      goals.forEach((g) => out.push({ value: g.id, label: plat.nombre + " \xB7 " + g.nombre, plataformaId: platId, icon: plat.icon }));
      const generalLabel = goals.length === 0 && PLATFORM_DATA[platId].sinValuacion ? plat.nombre : plat.nombre + " \xB7 General";
      out.push({ value: generalId, label: generalLabel, plataformaId: platId, icon: plat.icon });
    });
    return out;
  }
  __name(investmentCatOptions, "investmentCatOptions");
  function platformValorMonths(id) {
    return MONTHS.filter((m) => PLATFORM_DATA[id].valorHistorial[m] != null);
  }
  __name(platformValorMonths, "platformValorMonths");
  function platformCurrentValue(id) {
  ...
  function platformDiasDesdeActualizacion(id) {
    ...
    return Math.max(0, Math.round((hoy.getTime() - fecha.getTime()) / 864e5));
  }
  __name(platformDiasDesdeActualizacion, "platformDiasDesdeActualizacion");
  ...
  __name(inversionesMonthsCalendarYear, "inversionesMonthsCalendarYear");
  function mesTieneValorParaTodas(monthKey2) {
    const ids = activePlatformIds();
    return ids.length > 0 && ids.every((id) => PLATFORM_DATA[id].sinValuacion || PLATFORM_DATA[id].valorHistorial[monthKey2] != null);
  }
  __name(mesTieneValorParaTodas, "mesTieneValorParaTodas");
  function aportadoAcumuladoHastaMesONull(monthKey2) {
    if (!mesTieneValorParaTodas(monthKey2)) return null;
    let total = 0;
    TRANSACTIONS.forEach((t) => {
      if (t.tipo !== "inversion" || t.estado === "no_es_gasto") return;
      if (t.fecha.slice(0, 7) <= monthKey2) total += aggregatedTxAmount(t);
    });
    return total;
  }
  __name(aportadoAcumuladoHastaMesONull, "aportadoAcumuladoHastaMesONull");
  function platformGeneralAmountHastaMes(platformId, monthKey2) {
    const generalId = generalCatIdFor(platformId);
    let total = 0;
    TRANSACTIONS.forEach((t) => {
      if (t.tipo !== "inversion") return;
      if (t.fecha.slice(0, 7) > monthKey2) return;
      t.categorias.forEach((c) => {
        if (c.cat === generalId) total += c.monto;
      });
    });
  ...
  __name(platformGeneralAmountHastaMes, "platformGeneralAmountHastaMes");
  function platformAportadoNetoHastaMes(id, monthKey2) {
    return goalsForPlatform(id).reduce((s, m) => s + (metaHistorialAt(m, monthKey2) || 0), 0) + platformGeneralAmountHastaMes(id, monthKey2);
  }
  __name(platformAportadoNetoHastaMes, "platformAportadoNetoHastaMes");
  function valorTotalEnMesONull(monthKey2) {
    if (!mesTieneValorParaTodas(monthKey2)) return null;
    return activePlatformIds().reduce(
      (s, id) => s + (PLATFORM_DATA[id].sinValuacion ? platformAportadoNetoHastaMes(id, monthKey2) : PLATFORM_DATA[id].valorHistorial[monthKey2] || 0),
      0
    );
  }
  __name(valorTotalEnMesONull, "valorTotalEnMesONull");
  function buildDualLineChart(months, seriesA, seriesB, colorA, colorB) {
    ...
    __name(xAt, "xAt");
    function yAt(v) {
      return padTop + (1 - (v - minV) / range) * plotH;
    }
    __name(yAt, "yAt");
    ...
    __name(segmentos, "segmentos");
    function pathsFor(segs, extraAttrs) {
      return segs.map(
        (seg) => '<path d="' + seg.map((p, i) => (i === 0 ? "M" : "L") + p[0].toFixed(1) + " " + p[1].toFixed(1)).join(" ") + '" ' + extraAttrs + "/>"
      ).join("");
    }
    __name(pathsFor, "pathsFor");
    ...
    const labelsX = months.map(
      (m, i) => '<text x="' + xAt(i).toFixed(1) + '" y="' + (H - 6) + '" text-anchor="middle" font-size="9" fill="var(--text-tertiary)">' + monthAbbr(parseInt(m.slice(5, 7), 10)) + "</text>"
    ).join("");
    const yTicks = [maxV, (maxV + minV) / 2, minV];
    const gridY = yTicks.map((v) => {
      const y = yAt(v);
      return '<line x1="' + padL + '" y1="' + y.toFixed(1) + '" x2="' + (W - padR) + '" y2="' + y.toFixed(1) + '" stroke="var(--border)" stroke-width="1" stroke-dasharray="2 3"/><text x="' + (padL - 6) + '" y="' + (y + 3).toFixed(1) + '" text-anchor="end" font-size="9" fill="var(--text-tertiary)">' + moneyShort(v) + "</text>";
    }).join("");
    function ultimoPunto(vals) {
      for (let i = vals.length - 1; i >= 0; i--) {
        if (vals[i] != null) return [xAt(i), yAt(vals[i]), vals[i]];
      }
      return null;
    }
    __name(ultimoPunto, "ultimoPunto");
    ...
    if (lastA && lastB) {
      ...
      labelsFin = '<text x="' + lastA[0].toFixed(1) + '" y="' + Math.max(10, yA).toFixed(1) + '" text-anchor="end" font-size="10" font-weight="700" fill="' + colorA + '">' + moneyPlainMasked(lastA[2]) + '</text><text x="' + lastB[0].toFixed(1) + '" y="' + Math.max(10, yB).toFixed(1) + '" text-anchor="end" font-size="10" font-weight="700" fill="' + colorB + '">' + moneyPlainMasked(lastB[2]) + "</text>";
    } else if (lastA) {
  ...
  __name(renderPlatformEditForm, "renderPlatformEditForm");
  function platformDeleteBlock(id) {
    ...
    if (tieneMetas) {
      return '<div class="file-format-hint">No se puede cerrar ni eliminar: tiene metas asociadas. Elim\xEDnalas primero.</div>';
    }
    if (!enUso) {
      if (state.confirmDeletePlatformId === id) {
        return '<div class="file-format-hint" style="margin-bottom:8px;">\xBFEliminar esta plataforma? No se puede deshacer, y tambi\xE9n desaparece de Men\xFA &gt; Categor\xEDas.</div><div style="display:flex;gap:10px;"><button class="save-tx-btn" style="background:var(--surface-sunken);color:var(--text);flex:1;" data-cancel-delete-platform>Cancelar</button><button class="save-tx-btn" style="flex:1;background:var(--expense-ink);" data-confirm-delete-platform="' + id + '">S\xED, eliminar</button></div>';
      }
  ...
  function renderPlatformGroup(id) {
    ...
    const header = '<button class="platform-head-toggle" data-toggle-platform="' + id + '" aria-expanded="' + (open ? "true" : "false") + '"><span class="platform-icon" style="' + categoryColorVars(cat) + '">' + catIconMarkup(cat.icon) + '</span><span class="platform-head-body"><span class="platform-name">' + esc(cat.nombre) + "</span>" + (sinValuacion ? "" : '<span class="platform-update-tag' + (stale ? " stale" : "") + '">Actualizado hace ' + dias + " " + (dias === 1 ? "d\xEDa" : "d\xEDas") + "</span>") + '</span><span class="platform-head-value tabular">' + money(valorActual) + '</span><span class="platform-chev' + (open ? " open" : "") + '">' + ICONS.chevR + "</span></button>";
    ...
    const combinedSummary = metas.length > 0 && totalObjetivo > 0 ? '<div class="platform-meta-summary"><div class="platform-meta-summary-head"><span>Tus metas en ' + esc(cat.nombre) + "</span>" + (rachaCombinada > 0 ? '<span class="meta-racha-badge">' + rachaCombinada + " \u{1F525}</span>" : "") + '</div><div class="platform-meta-summary-figs tabular">' + money(totalAcumulado) + '<span class="of-text"> de ' + money(totalObjetivo) + '</span><span class="budget-pct tabular">' + Math.round(combinedPct) + '%</span></div><div class="budget-track"><div class="budget-fill" style="width:' + Math.max(0, Math.min(100, combinedPct)) + '%;background:var(--accent);"></div></div></div>' : "";
    const metasBody = sinValuacion ? "" : combinedSummary + metas.map(renderGoalCard).join("") + (addingHere ? renderGoalEditForm(null, id) : '<button class="budget-add-link platform-add-meta-link" data-add-goal="' + id + '">+ Agregar meta a ' + esc(cat.nombre) + "</button>");
    const body = '<div class="platform-body"><div class="platform-figs"><div class="platform-fig"><span class="platform-fig-label">' + (sinValuacion ? "Aportado" : "Total en esta plataforma") + '</span><span class="platform-fig-value tabular">' + money(valorActual) + '</span></div><div class="platform-fig"><span class="platform-fig-label">Aportado neto</span><span class="platform-fig-value tabular muted">' + money(aportado) + "</span></div></div>" + // The plazo chip is only shown if it doesn't have its own goals yet — as soon as
    // you add a goal, its plazo takes over and this one becomes redundant.
    (!tieneMetas && !sinValuacion ? '<div class="platform-diff-row">' + termChip(PLATFORM_DATA[id].plazo) + "</div>" : "") + comisionRow + '<div class="platform-actions-row"><button class="budget-ver-mas" data-platform-see-more="' + id + '">Ver transacciones \u2192</button>' + // Nothing to update on a sinValuacion platform (no valuation, no comision of its own) --
    // no pencil button.
    (sinValuacion ? "" : '<button class="budget-edit-btn" data-edit-platform="' + id + '" aria-label="Actualizar valor de ' + esc(cat.nombre) + '">' + ICONS.edit + "</button>") + '</div><div class="platform-goal-nest">' + metasBody + "</div></div>";
    return '<div class="card platform-group open">' + header + body + "</div>";
  }
  __name(renderPlatformGroup, "renderPlatformGroup");
  function renderArchivedPlatformsBlock() {
    const ids = archivedPlatformIds();
    if (!ids.length) return "";
    return '<div class="section-title" style="margin-top:18px;">Plataformas cerradas</div>' + ids.map((id) => {
      const cat = catInfo(id);
      return '<div class="card platform-card archived-card"><div class="platform-head"><span class="platform-icon" style="--fill:var(--surface-sunken);--ink:var(--text-tertiary)">' + catIconMarkup(cat.icon) + '</span><span class="platform-name muted">' + esc(cat.nombre) + '</span><button class="budget-edit-btn" data-reopen-platform="' + id + '" aria-label="Reabrir ' + esc(cat.nombre) + '">' + ICONS.repeat + '</button></div><button class="budget-ver-mas" data-platform-see-more="' + id + '">Ver transacciones \u2192</button></div>';
    }).join("");
  }
  __name(renderArchivedPlatformsBlock, "renderArchivedPlatformsBlock");
  function round1(n) {
  ...
  function planGroupBlock(plazoKey) {
    ...
    const rows = metas.length ? metas.map(planMetaRowHtml).join("") : '<div class="plan-empty-hint muted">Sin metas de plazo ' + info.label.toLowerCase() + " todav\xEDa \u2014 agr\xE9galas en Inversiones.</div>";
    return '<div class="card plan-block"><div class="plan-block-head" style="color:var(--cat-' + info.color + '-ink);"><span class="tag" style="background:var(--cat-' + info.color + '-fill);"></span>' + info.label + " plazo</div>" + rows + (metas.length ? '<div class="plan-subtotal" style="color:var(--cat-' + info.color + '-ink);"><span>Subtotal ' + info.label.toLowerCase() + ' \xB7 <span data-plan-group-pct="' + plazoKey + '"></span>%</span><span class="plan-subtotal-amt tabular" data-plan-group-amt="' + plazoKey + '"></span></div>' : "") + "</div>";
  }
  __name(planGroupBlock, "planGroupBlock");
  function renderPlanificadorSection() {
    ...
    return '<div class="section-title">Planificador de sueldo</div><div class="card plan-base-card"><div class="plan-base-input-row"><div class="plan-base-field"><label class="draft-label">Total mensual a repartir</label><div class="plan-base-input"><span>$</span><input type="text" inputmode="numeric" data-plan-base-input value="' + moneyPlain(P.base) + '"></div></div><div style="flex-shrink:0;"><span class="plan-total-pill" data-plan-total-pill><span class="dot"></span><span data-plan-total-txt></span></span><div class="plan-unassigned" data-plan-unassigned></div></div></div><div class="plan-base-hint muted">Sugerido: ingresos \u2212 gastos de ' + mesActualLabel + " = " + money(defaultBase) + '. Ed\xEDtalo si tu excedente real es otro.</div></div><div class="plan-bar" data-plan-bar></div><div class="plan-legend" data-plan-legend></div><div class="plan-cols">' + planGroupBlock("corto") + planGroupBlock("medio") + planGroupBlock("largo") + "</div>";
  }
  __name(renderPlanificadorSection, "renderPlanificadorSection");
  ...
  __name(updateProyeccionCompute, "updateProyeccionCompute");
  function updatePlanCompute() {
    ...
    ["corto", "medio", "largo"].forEach((plazoKey) => {
      ...
      if (pctEl) pctEl.textContent = String(round1(groupPct[plazoKey]));
      const amtEl = document.querySelector('[data-plan-group-amt="' + plazoKey + '"]');
      if (amtEl) amtEl.textContent = money(base * groupPct[plazoKey] / 100);
    });
    ...
    if (pillEl && txtEl && unEl) {
      if (Math.abs(diff) < 0.05) {
        ...
        unEl.textContent = "Asignaste " + money(base * -diff / 100) + " (" + round1(-diff) + "%) m\xE1s de tu excedente";
      }
    }
  ...
  __name(updatePlanCompute, "updatePlanCompute");
  function renderInvestmentsView() {
    ...
    '<div class="platform-total-sub" style="margin-top:2px;color:var(--text-tertiary);font-size:11.5px;">Aporte mensual objetivo: <b class="tabular">' + money(monthlyInvestmentGoalCLP()) + "</b> \xB7 " + Math.round(investmentGoalPct()) + "% de tus ingresos</div>" + renderTotalChecksGrid() + "</div>" : (
      ...
      // this calculation).
      '<div class="platform-total-goal-block"><div class="platform-total-label" style="color:var(--accent-ink);">Objetivo de inversi\xF3n ' + anio + '</div><p class="muted" style="font-size:12.5px;margin:4px 0 0;">Ninguna de tus metas tiene un aporte mensual fijo todav\xEDa, as\xED que no hay un objetivo anual que medir.' + (otrosAporteAnio > 0 ? " Aun as\xED, llevas " + money(otrosAporteAnio) + " invertidos este a\xF1o." : "") + "</p>" + renderTotalChecksGrid() + "</div>"
    );
    const proy = projectedContributions(3, 20);
  ...
  __name(renderInvestmentsView, "renderInvestmentsView");
  function renderSummarySubContent() {
  ...
  __name(renderSummarySubContent, "renderSummarySubContent");
  var SUBS_META = {
    balance: { label: "Balance" },
    presupuesto: { label: "Presupuesto" },
    evolucion: { label: "Evoluci\xF3n" },
    inversiones: { label: "Inversiones" }
  };
  function renderSummarySubtabsInner() {
    return state.summarySubOrder.map(
      (id) => '<button class="subtab ' + (state.summarySub === id ? "active" : "") + (state.subtabDragId === id ? " dragging" : "") + '" data-summary-sub="' + id + '">' + SUBS_META[id].label + "</button>"
    ).join("");
  }
  __name(renderSummarySubtabsInner, "renderSummarySubtabsInner");
  ...
  __name(renderSummaryView, "renderSummaryView");
  ...
  function isCategoryInUse(catId) {
    return TRANSACTIONS.some((t) => t.categorias.some((c) => c.cat === catId));
  }
  __name(isCategoryInUse, "isCategoryInUse");
  function isPaymentMethodInUse(medioId) {
  ...
  function groupedRules() {
    ...
    return Object.keys(map).map((comercio) => {
      ...
      return {
        ...
        count: txs.length,
        tipo: recent.tipo,
        recurrencia: recent.recurrencia,
        cat: recent.categorias[0] ? recent.categorias[0].cat : null
      };
    }).sort((a, b) => a.comercio.localeCompare(b.comercio));
  }
  __name(groupedRules, "groupedRules");
  function menuScreenHead(title) {
    return '<div class="menu-screen-head"><button class="menu-back-btn" data-menu-back aria-label="Volver al men\xFA">' + ICONS.chevL + '</button><h2 class="menu-screen-title">' + esc(title) + "</h2></div>";
  }
  __name(menuScreenHead, "menuScreenHead");
  ...
  __name(normalizeFecha, "normalizeFecha");
  ...
  __name(parseCsvMonto, "parseCsvMonto");
  function parseStatementCSV(text) {
    ...
    lines.forEach((line, idx) => {
      ...
      if (parts.length < 3) {
        errors.push("L\xEDnea " + (idx + 2) + ": formato inv\xE1lido");
        return;
      }
      ...
      if (!fecha || !descripcion || isNaN(monto)) {
        errors.push("L\xEDnea " + (idx + 2) + ": no se pudo leer la fecha, descripci\xF3n o monto");
        return;
      }
  ...
  __name(parseStatementCSV, "parseStatementCSV");
  function importStatementRows(rows) {
    if (state.demoMode) {
      toast("No disponible en modo demo");
      return { creadas: 0, conRegla: 0, pendientes: 0 };
    }
    const reglaByComercio = {};
    groupedRules().forEach((r) => {
      reglaByComercio[r.comercio] = r;
    });
    ...
    rows.forEach((row) => {
      ...
      if (regla && regla.cat) conRegla++;
      else pendientes++;
      TRANSACTIONS.unshift({
        id: "timp" + nextImportId(),
        fecha: row.fecha,
        hora: "00:00",
        comercio: row.descripcion,
        monto,
        medio: ensureCheckingAccountMethod(),
        tipo,
  ...
  __name(importStatementRows, "importStatementRows");
  var DIAS_PARA_RECORDAR_RESPALDO = 30;
  function respaldoInfo() {
    const hayDatos = TRANSACTIONS.length > 0;
    if (!ultimoRespaldo) return { nunca: true, dias: null, vencido: hayDatos, hayDatos };
    const hoy = (/* @__PURE__ */ new Date(todayISO() + "T12:00:00")).getTime();
    const ult = (/* @__PURE__ */ new Date(ultimoRespaldo + "T12:00:00")).getTime();
    const dias = Math.max(0, Math.round((hoy - ult) / 864e5));
    return { nunca: false, dias, vencido: hayDatos && dias >= DIAS_PARA_RECORDAR_RESPALDO, hayDatos };
  }
  __name(respaldoInfo, "respaldoInfo");
  function respaldoSubtitulo() {
    ...
    if (r.dias === 0) return "\xDAltimo respaldo: hoy";
    if (r.dias === 1) return "\xDAltimo respaldo: ayer";
    return "\xDAltimo respaldo: hace " + r.dias + " d\xEDas" + (r.vencido ? " \u2014 conviene bajar uno nuevo" : "");
  }
  __name(respaldoSubtitulo, "respaldoSubtitulo");
  function renderMenuMain() {
    const nReglas = groupedRules().length;
    const items = [
      { section: "cuenta", icon: "lockSmall", label: "Mi cuenta", sub: currentUser ? currentUser.email : "Sesi\xF3n" },
      { section: "categorias", icon: "tags", label: "Categor\xEDas", sub: Object.keys(CATEGORIES).length + " categor\xEDas" },
      { section: "medios", icon: "card", label: "Medios de pago", sub: Object.keys(PAYMENT_METHODS).length + " medios de pago" },
      { section: "reglas", icon: "lockSmall", label: "Reglas de clasificaci\xF3n", sub: nReglas + " regla" + (nReglas === 1 ? "" : "s") + " autom\xE1tica" + (nReglas === 1 ? "" : "s") },
      ...
      { section: "importarcorreo", icon: "inbox", label: "Importar desde tu correo", sub: "Autom\xE1tico, v\xEDa Gmail" },
      { section: "notificaciones", icon: "bell", label: "Notificaciones", sub: state.notifSubscribed ? "Activadas en este dispositivo" : "Av\xEDsame de transacciones y presupuesto" },
      ...
      { section: "asesoria", icon: "sparkle", label: "Asesor\xEDa financiera con Claude", sub: "Pr\xF3ximamente" }
    ];
    document.getElementById("view-root").innerHTML = '<ul class="menu-list">' + items.map(
      (i) => '<li><button class="menu-list-item" data-menu-open="' + i.section + '"><span class="menu-item-icon">' + ICONS[i.icon] + '</span><span class="menu-item-label">' + esc(i.label) + '<span class="menu-item-sub' + (i.alerta ? " menu-item-sub-alerta" : "") + '">' + esc(i.sub) + '</span></span><span class="menu-item-chev">' + ICONS.chevL + "</span></button></li>"
    ).join("") + "</ul>";
  }
  __name(renderMenuMain, "renderMenuMain");
  function renderMenuCatEditForm() {
    const d = state.catDraft;
    const isNew = state.editingCategoryId === "nueva";
    return '<div class="card" style="padding:16px;"><label class="draft-label">Nombre</label><input type="text" class="draft-input" data-cat-draft-field="nombre" value="' + esc(d.nombre) + '" placeholder="Ej: Mascotas"><label class="draft-label" style="margin-top:12px;">Tipo</label>' + segmentedHtml("cat-draft-tipo", [{ id: "gasto", label: "Gasto" }, { id: "ingreso", label: "Ingreso" }], d.tipo, !isNew) + (!isNew ? '<div class="platform-hint muted">El tipo no se puede cambiar una vez creada la categor\xEDa.</div>' : "") + '<label class="draft-label" style="margin-top:12px;">\xCDcono</label><div class="icon-picker emoji-icon-picker">' + CAT_EMOJI_CHOICES.map((em) => '<button type="button" data-cat-draft-icon="' + em + '" class="' + (d.icon === em ? "active" : "") + '">' + em + "</button>").join("") + '</div><input type="text" class="draft-input" data-cat-draft-field="icon" value="' + esc(d.icon) + '" maxlength="8" placeholder="O escribe/pega cualquier otro emoji \u{1F60A}" style="margin-top:8px;text-align:center;"><label class="draft-label" style="margin-top:12px;">Color</label><div class="hue-picker-row"><span class="hue-swatch" style="background:' + categoryFillCss(d.colorHue) + ";color:" + categoryFillCss(d.colorHue) + ";border-color:" + categoryFillCss(d.colorHue) + '"></span><input type="range" min="0" max="359" value="' + d.colorHue + '" data-cat-draft-hue class="hue-slider"></div>' + (function() {
  ...
  function renderMenuMedios() {
    ...
    const rows = Object.keys(PAYMENT_METHODS).map((id) => {
      ...
      return '<div class="card menu-item-card"><span class="menu-item-card-icon" style="--fill:var(--surface-sunken);--ink:var(--text-secondary);">' + ICONS[m.icon] + '</span><div class="menu-item-card-body"><div class="menu-item-card-name">' + esc(m.nombre) + '</div><div class="menu-item-card-sub">' + esc(m.corto) + '</div></div><div class="menu-item-card-actions"><button class="budget-edit-btn" data-edit-payment-method="' + id + '" aria-label="Editar ' + esc(m.nombre) + '">' + ICONS.edit + "</button></div></div>";
    }).join("");
    document.getElementById("view-root").innerHTML = menuScreenHead("Medios de pago") + rows + '<button class="budget-add-link" data-add-payment-method style="margin:2px 0 4px;">+ Agregar medio de pago</button>';
  }
  __name(renderMenuMedios, "renderMenuMedios");
  function renderMenuReglas() {
    ...
    document.getElementById("view-root").innerHTML = menuScreenHead("Reglas de clasificaci\xF3n") + '<p class="muted" style="font-size:12.5px;margin:0 0 14px;line-height:1.5;">Cuando activas el candado dentro del detalle de una transacci\xF3n, esa categor\xEDa, tipo y recurrencia se aplican a futuras compras del mismo comercio. Ac\xE1 puedes revisarlas y eliminarlas.</p>' + (reglas.length === 0 ? '<div class="card placeholder-card">' + ICONS.lockSmall + "<h3>Todav\xEDa no tienes reglas</h3><p>Act\xEDvalas desde el detalle de cualquier transacci\xF3n, con el \xEDcono de candado.</p></div>" : reglas.map((r) => {
      ...
      return '<div class="card rule-card"><div class="rule-card-head"><span class="rule-card-comercio">' + esc(r.comercio) + '</span><span class="rule-card-count">' + r.count + " transac.</span>" + (confirmando ? "" : '<button class="budget-edit-btn" data-ask-delete-rule="' + encodeURIComponent(r.comercio) + '" aria-label="Eliminar regla de ' + esc(r.comercio) + '">' + ICONS.trash + "</button>") + '</div><div class="rule-card-detail">' + (cat ? '<span class="rule-card-catchip" style="' + categoryColorVars(cat) + '">' + catIconMarkup(cat.icon) + " " + esc(cat.nombre) + "</span>" : "") + "<span>" + (r.tipo === "gasto" ? "Gasto" : r.tipo === "ingreso" ? "Ingreso" : "Inversi\xF3n") + "</span><span>\xB7</span><span>" + (r.recurrencia === "mensual" ? "Fijo mensual" : "Variable") + "</span></div>" + (confirmando ? '<div class="file-format-hint" style="margin:10px 0 8px;">\xBFSeguro que quieres eliminar la regla de "' + esc(r.comercio) + '"? Las transacciones ya clasificadas no cambian, pero las nuevas de este comercio dejar\xE1n de clasificarse solas.</div><div style="display:flex;gap:10px;"><button class="save-tx-btn" style="background:var(--surface-sunken);color:var(--text);flex:1;" data-cancel-delete-rule>Cancelar</button><button class="save-tx-btn" style="flex:1;background:var(--cat-pink-fill);color:var(--expense-ink);" data-confirm-delete-rule="' + encodeURIComponent(r.comercio) + '">S\xED, eliminar</button></div>' : "") + "</div>";
  ...
  function renderGroupCategoryRulesSection() {
    ...
    return '<h3 style="font-size:14px;margin:22px 0 10px;">Reglas de grupo por categor\xEDa</h3>' + catIds.map((catId) => {
      ...
      return '<div class="card rule-card"><div class="rule-card-head"><span class="rule-card-comercio">' + esc(cat.nombre) + "</span>" + (confirmando ? "" : '<button class="budget-edit-btn" data-ask-delete-group-rule="' + catId + '" aria-label="Eliminar regla de grupo para ' + esc(cat.nombre) + '">' + ICONS.trash + "</button>") + '</div><div class="rule-card-detail"><span class="rule-card-catchip" style="' + categoryColorVars(cat) + '">' + catIconMarkup(cat.icon) + " " + esc(cat.nombre) + "</span><span>\u2192 " + esc(grupo ? grupo.icono + " " + grupo.nombre : "grupo eliminado") + "</span><span>\xB7</span><span>" + (regla.divisionTipo === "iguales" ? "Por partes" : regla.divisionTipo === "pct" ? "Por %" : "Monto fijo") + "</span></div>" + (confirmando ? '<div class="file-format-hint" style="margin:10px 0 8px;">\xBFEliminar la regla de "' + esc(cat.nombre) + '" \u2192 ' + (grupo ? grupo.nombre : "ese grupo") + '? Las transacciones ya compartidas no cambian -- solo deja de sugerirse la pr\xF3xima vez.</div><div style="display:flex;gap:10px;"><button class="save-tx-btn" style="background:var(--surface-sunken);color:var(--text);flex:1;" data-cancel-delete-group-rule>Cancelar</button><button class="save-tx-btn" style="flex:1;background:var(--cat-pink-fill);color:var(--expense-ink);" data-confirm-delete-group-rule="' + catId + '">S\xED, eliminar</button></div>' : "") + "</div>";
    }).join("");
  }
  ...
  __name(renderMenuExportar, "renderMenuExportar");
  function renderMenuRespaldo() {
    const r = respaldoInfo();
    const aviso = !r.vencido ? "" : '<div class="card" style="padding:14px 16px;margin-bottom:12px;border-color:var(--expense-ink);"><div style="font-weight:700;font-size:13.5px;color:var(--expense-ink);margin-bottom:4px;">' + (r.nunca ? "Todav\xEDa no tienes ning\xFAn respaldo" : "Tu \xFAltimo respaldo es de hace " + r.dias + " d\xEDas") + '</div><div class="muted" style="font-size:12.5px;">Este archivo es tu \xFAnica copia fuera de la app. Si algo se corrompe o se borra por error, es lo \xFAnico que permite recuperar tus categor\xEDas, medios de pago y transacciones tal como los ten\xEDas.</div></div>';
    document.getElementById("view-root").innerHTML = menuScreenHead("Respaldo en JSON") + aviso + '<div class="card" style="padding:16px;"><div class="menu-item-card" style="padding:0;margin-bottom:16px;"><span class="menu-item-card-icon" style="--fill:var(--cat-sky-fill);--ink:var(--cat-sky-ink)">' + ICONS.inbox + '</span><div class="menu-item-card-body"><div class="menu-item-card-name">Copia completa de tus datos</div><div class="menu-item-card-sub">Transacciones, categor\xEDas, medios, presupuestos, metas y plataformas</div></div></div><button class="save-tx-btn" data-export-json style="width:100%;">Descargar JSON</button><div class="file-format-hint">' + (r.nunca ? "Todav\xEDa no has descargado ninguno." : "\xDAltimo descargado: " + dayLabel(ultimoRespaldo) + ".") + " Pensado para guardar una copia de respaldo o migrarla m\xE1s adelante \u2014 no se puede volver a importar desde esta maqueta.</div></div>";
  }
  __name(renderMenuRespaldo, "renderMenuRespaldo");
  function renderMenuImportar() {
    const s = state.importSummary;
    document.getElementById("view-root").innerHTML = menuScreenHead("Importar CSV de cartola") + (s ? '<div class="card" style="padding:16px;"><div class="menu-item-card" style="padding:0;margin-bottom:14px;"><span class="menu-item-card-icon" style="--fill:var(--cat-mint-fill);--ink:var(--cat-mint-ink)">' + ICONS.checkCircle + '</span><div class="menu-item-card-body"><div class="menu-item-card-name">' + s.archivo + '</div><div class="menu-item-card-sub">' + s.creadas + " fila" + (s.creadas === 1 ? "" : "s") + " le\xEDda" + (s.creadas === 1 ? "" : "s") + '</div></div></div><div class="rule-card-detail" style="margin-bottom:8px;">' + ICONS.check + "<span>" + s.creadas + ' transacciones creadas</span></div><div class="rule-card-detail" style="margin-bottom:8px;">' + ICONS.lockSmall + "<span>" + s.conRegla + ' categorizadas autom\xE1ticamente por una regla existente</span></div><div class="rule-card-detail" style="margin-bottom:' + (s.errores.length ? "8px" : "0") + ';">' + ICONS.question + "<span>" + s.pendientes + " quedaron pendientes de categorizar</span></div>" + (s.errores.length ? '<div class="rule-card-detail" style="color:var(--expense-ink);">' + s.errores.length + " fila" + (s.errores.length === 1 ? "" : "s") + " no se pudo leer</div>" : "") + '<button class="budget-add-link" data-import-again style="margin-top:14px;">Importar otro archivo</button>' + (s.pendientes > 0 ? '<button class="save-tx-btn" style="width:100%;margin-top:10px;" data-goto-pending>Ir a categorizarlas</button>' : "") + "</div>" : '<div class="card file-drop-card">' + ICONS.inbox + '<p>Sube un archivo CSV con tus movimientos. Cada fila necesita fecha, descripci\xF3n del comercio y monto (negativo para gastos, positivo para ingresos).</p><label class="save-tx-btn" style="display:inline-block;cursor:pointer;">Elegir archivo<input type="file" accept=".csv,text/csv" data-csv-file-input style="display:none;"></label><div class="file-format-hint">Formato esperado: <code>fecha,descripcion,monto</code><br>Ej: <code>2026-08-20,Jumbo \xD1u\xF1oa,-45000</code></div></div>');
  }
  __name(renderMenuImportar, "renderMenuImportar");
  var RECON_PDFJS_VERSION = "3.11.174";
  var CC_COLS = { fecha: [15, 50], detalle: [50, 236], sucursal: [236, 343], cargo: [343, 419], abono: [419, 533], saldo: [533, 650] };
  var TC_COLS = { lugar: [20, 97], fecha: [97, 140], codigo: [140, 194], detalle: [194, 379], monto_op: [379, 441], monto_pagar: [441, 503], ncuota: [503, 534], valor_cuota: [534, 650] };
  ...
  __name(ensurePdfJs, "ensurePdfJs");
  ...
  __name(parseMontoCLP, "parseMontoCLP");
  async function extractPdfPagesWords(arrayBuffer, password) {
    ...
    for (let p = 1; p <= pdf.numPages; p++) {
      ...
      content.items.forEach(function(it) {
        ...
        if (!text) return;
        words.push({ text, x0: it.transform[4], top: viewport.height - it.transform[5] });
      });
      pages.push(words);
    }
    return pages;
  }
  __name(extractPdfPagesWords, "extractPdfPagesWords");
  function groupRows(words) {
    ...
    return rows.map(function(r) {
      return r.items.slice().sort(function(a, b) {
        return a.x0 - b.x0;
      });
    });
  }
  ...
  __name(bucketColumn, "bucketColumn");
  function bucketizeRow(row, cols) {
    ...
    Object.keys(out).forEach(function(k) {
      flat[k] = out[k].join(" ").trim();
    });
    return flat;
  }
  __name(bucketizeRow, "bucketizeRow");
  function detectarTipoCartola(pagesWords) {
    const t = (pagesWords[0] || []).map(function(w) {
      return w.text;
    }).join(" ").toUpperCase();
  ...
  __name(detectarTipoCartola, "detectarTipoCartola");
  function contextoAnioCuentaCorriente(pagesWords) {
    const t = (pagesWords[0] || []).map(function(w) {
      return w.text;
    }).join(" ");
    const fechas = t.match(/\d{2}\/\d{2}\/\d{4}/g) || [];
    if (!fechas.length) return { year: (/* @__PURE__ */ new Date()).getFullYear(), month: (/* @__PURE__ */ new Date()).getMonth() + 1 };
    const ultima = fechas[fechas.length - 1];
    const [dd, mm, yyyy] = ultima.split("/").map(Number);
    return { year: yyyy, month: mm };
  }
  __name(contextoAnioCuentaCorriente, "contextoAnioCuentaCorriente");
  ...
  __name(fechaConContexto, "fechaConContexto");
  function parseCuentaCorrienteMovs(pagesWords) {
    ...
    pagesWords.forEach(function(words) {
      const rows = groupRows(words).map(function(r) {
        return bucketizeRow(r, CC_COLS);
      });
      let i = 0;
      while (i < rows.length) {
        const r = rows[i];
        if (/^\d{2}\/\d{2}$/.test(r.fecha)) {
          if (r.detalle && (r.cargo || r.abono)) {
            ...
            merged.push({ fecha: r.fecha, detalle, cargo: r.cargo || nxt.cargo, abono: r.abono || nxt.abono });
            i += 2;
          }
        } else {
          i += 1;
  ...
  __name(parseCuentaCorrienteMovs, "parseCuentaCorrienteMovs");
  function parseTarjetaNacionalMovs(pagesWords) {
    const merged = [];
    pagesWords.forEach(function(words) {
      const rows = groupRows(words).map(function(r) {
        return bucketizeRow(r, TC_COLS);
      });
    ...
    merged.forEach(function(r) {
      ...
      let esEspecial = null;
      if (/MONTO CANCELADO/.test(filaCompleta)) esEspecial = "pago_recibido";
      ...
      movimientos.push({
        ...
        monto: montoFinal < 0 ? montoFinal : -Math.abs(montoFinal),
        // on a card statement, every purchase is an expense
        tipoMov: "gasto",
        esEspecial,
        ...
        cuotaNumero: esCuota ? parseInt(cuotaMatch[1], 10) : void 0,
        cuotaTotal: esCuota ? parseInt(cuotaMatch[2], 10) : void 0,
        montoOperacion: esCuota ? Math.abs(monto) : void 0
      });
    });
    return movimientos;
  }
  __name(parseTarjetaNacionalMovs, "parseTarjetaNacionalMovs");
  async function parseStatementPDF(arrayBuffer, password) {
    if (!ensurePdfJs()) throw new Error("No se pudo cargar el lector de PDF (revisa tu conexi\xF3n a internet).");
    let pagesWords;
    try {
  ...
  __name(loadAvailableStatements, "loadAvailableStatements");
  async function useImportedStatement(id, password) {
    if (state.demoMode) {
      toast("No disponible en modo demo");
      return;
    }
    const item = state.reconciliar.disponibles.find(function(d) {
      return d.id === id;
    });
    if (!item) return;
    ...
    try {
      ...
      if (!res.tipo) {
        state.reconciliar.error = "No reconoc\xED el formato de este PDF \u2014 pru\xE9balo subi\xE9ndolo a mano para revisar.";
        renderMenuView();
        return;
      }
      ...
      if (err && err.message === "PDF_PASSWORD_REQUERIDA") {
        state.reconciliar.errorPassword = password ? "Esa clave no abri\xF3 el archivo \u2014 pru\xE9bala de nuevo." : "Este PDF pide una clave.";
      } else {
  ...
  __name(useImportedStatement, "useImportedStatement");
  async function tryOpenStatementFile(buffer, nombre, password) {
    if (state.demoMode) {
      toast("No disponible en modo demo");
      return;
    }
    ...
    try {
      ...
      if (!res.tipo) {
        state.reconciliar.error = "No reconoc\xED el formato de este PDF \u2014 por ahora solo lee cartolas de cuenta corriente y estados de cuenta de tarjeta de cr\xE9dito de Banco Edwards / Banco de Chile.";
        renderMenuView();
        return;
      }
  ...
  __name(tryOpenStatementFile, "tryOpenStatementFile");
  function findSimilarTx(mov) {
    const montoAbs = Math.abs(mov.monto);
    return TRANSACTIONS.find(function(t) {
      if (t.tipo !== mov.tipoMov) return false;
      if (Math.abs(t.monto - montoAbs) > 1) return false;
      const d1 = /* @__PURE__ */ new Date(t.fecha + "T00:00:00"), d2 = /* @__PURE__ */ new Date(mov.fecha + "T00:00:00");
      const diffDias = Math.abs(d1.getTime() - d2.getTime()) / 864e5;
      return diffDias <= 2;
    }) || null;
  }
  __name(findSimilarTx, "findSimilarTx");
  function renderCartolasDisponiblesBlock() {
    ...
    const filas = R.disponibles.map(function(d) {
      ...
      if (abriendo) {
        return '<div class="card" style="padding:14px;margin-bottom:8px;"><div style="font-weight:700;font-size:13.5px;margin-bottom:2px;">' + label + '</div><div class="muted" style="font-size:12px;margin-bottom:10px;">Lleg\xF3 por correo el ' + fechaTxt + '</div><input type="password" inputmode="numeric" class="draft-input" data-statement-password-input placeholder="\xDAltimos 4 d\xEDgitos de tu RUT antes del d\xEDgito verificador" value="' + (R.passwordDraft || "") + '">' + (R.errorPassword ? '<div class="field-error">' + R.errorPassword + "</div>" : "") + '<div style="display:flex;gap:8px;margin-top:10px;"><button class="save-tx-btn" style="background:var(--surface-sunken);color:var(--text);flex:1;" data-statement-cancel>Cancelar</button><button class="save-tx-btn" style="flex:1;" data-statement-open="' + d.id + '">Abrir</button></div></div>';
      }
  ...
  function renderMenuReconciliar() {
    ...
    if (R.archivoNombrePendiente) {
      document.getElementById("view-root").innerHTML = head + '<div class="card" style="padding:14px;"><div style="font-weight:700;font-size:13.5px;margin-bottom:2px;">' + R.archivoNombrePendiente + '</div><div class="muted" style="font-size:12px;margin-bottom:10px;">Este PDF est\xE1 protegido con clave.</div><input type="password" inputmode="numeric" class="draft-input" data-statement-password-input placeholder="\xDAltimos 4 d\xEDgitos de tu RUT antes del d\xEDgito verificador" value="' + (R.passwordDraft || "") + '">' + (R.errorPassword ? '<div class="field-error">' + R.errorPassword + "</div>" : "") + '<div style="display:flex;gap:8px;margin-top:10px;"><button class="save-tx-btn" style="background:var(--surface-sunken);color:var(--text);flex:1;" data-reconcile-file-cancel>Cancelar</button><button class="save-tx-btn" style="flex:1;" data-reconcile-file-open>Abrir</button></div></div>';
      return;
    }
    if (!R.movimientos.length && !R.error) {
      document.getElementById("view-root").innerHTML = head + renderCartolasDisponiblesBlock() + '<div class="card file-drop-card">' + ICONS.inbox + '<p>Sube el PDF de tu cuenta corriente o de tu estado de cuenta de tarjeta de cr\xE9dito. La app compara cada movimiento contra lo que ya tienes registrado \u2014 nunca sube el archivo a ning\xFAn servidor, se lee ac\xE1 mismo en tu navegador.</p><label class="save-tx-btn" style="display:inline-block;cursor:pointer;">Elegir archivo PDF<input type="file" accept="application/pdf" data-reconcile-file-input style="display:none;"></label></div>';
      return;
    }
    if (R.error) {
      document.getElementById("view-root").innerHTML = head + '<div class="card placeholder-card">' + ICONS.ban + "<h3>No se pudo leer</h3><p>" + R.error + '</p><button class="save-tx-btn" style="margin-top:12px;" data-reconcile-reset>Probar con otro archivo</button></div>';
      return;
    }
    const normales = R.movimientos.filter(function(m) {
      return m.esEspecial !== "pago_tarjeta" && m.esEspecial !== "pago_recibido";
    });
    const pagosTarjeta = R.movimientos.filter(function(m) {
      return m.esEspecial === "pago_tarjeta";
    });
    const conMatch = normales.filter(function(m) {
      return !!m.__match;
    });
    const sinMatch = normales.filter(function(m) {
      return !m.__match;
    });
    let resumenTarjeta = "";
    if (pagosTarjeta.length) {
      const totalPagos = pagosTarjeta.reduce(function(s, m) {
        return s + Math.abs(m.monto);
      }, 0);
      const ym = R.movimientos[0] ? R.movimientos[0].fecha.slice(0, 7) : todayISO().slice(0, 7);
      const comprasRegistradas = txsOfMonth(ym).filter(function(t) {
        return t.tipo === "gasto" && PAYMENT_METHODS[t.medio] && PAYMENT_METHODS[t.medio].icon === "card";
      }).reduce(function(s, t) {
        return s + t.monto;
      }, 0);
      resumenTarjeta = '<div class="card" style="padding:14px 16px;margin-bottom:14px;"><div class="sheet-block-title" style="margin-bottom:6px;">Pago de tarjeta este mes</div><p class="muted" style="margin-bottom:4px;">Se pag\xF3 ' + money(totalPagos) + " en " + pagosTarjeta.length + " cargo" + (pagosTarjeta.length === 1 ? "" : "s") + ' de tarjeta de cr\xE9dito.</p><p class="muted">Tienes ' + money(comprasRegistradas) + " en compras con tarjeta registradas este mes \u2014 esto es solo referencial, la cartola de tarjeta detalla cada compra por separado.</p></div>";
    }
    function filaHtml(m, idx, yaRegistrada) {
      const acciones = yaRegistrada ? '<span class="tx-state state-cobrado-inline">Ya registrada</span>' : '<div style="display:flex;gap:6px;"><button class="chip" data-reconcile-add="' + idx + '">+ Agregar</button><button class="chip" style="background:var(--surface-sunken);color:var(--text-secondary);" data-reconcile-not-expense="' + idx + '">' + (m.tipoMov === "ingreso" ? "No es ingreso" : "No es gasto") + "</button></div>";
      return '<div class="card" style="padding:12px 14px;margin-bottom:8px;display:flex;align-items:center;justify-content:space-between;gap:10px;"><div style="min-width:0;"><div style="font-weight:700;font-size:13.5px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + esc(m.comercioSugerido || m.detalle) + '</div><div class="muted" style="font-size:12px;">' + dayLabel(m.fecha) + (m.esEspecial === "sueldo" ? " \xB7 Sueldo" : "") + '</div></div><div style="display:flex;flex-direction:column;align-items:flex-end;gap:6px;flex-shrink:0;"><span class="tabular" style="font-weight:600;">' + (m.tipoMov === "ingreso" ? "+" : "") + money(Math.abs(m.monto)) + "</span>" + acciones + "</div></div>";
    }
    __name(filaHtml, "filaHtml");
  ...
  function renderReconcileDiffSection(R) {
    ...
    if (diff.agregar.length) {
      html += '<div class="card" style="padding:14px;margin-bottom:10px;"><div class="sheet-block-title" style="margin-bottom:6px;">Faltan en la app (' + diff.agregar.length + ')</div><p class="muted" style="margin-bottom:10px;">Est\xE1n en la cartola pero ninguna transacci\xF3n registrada calza con ellos.</p>' + (totalAltas ? '<button class="budget-add-link" data-reconcile-diff-add-altas style="margin-bottom:8px;">Agregar las ' + totalAltas + " de confianza alta</button>" : "") + diff.agregar.map(function(item) {
        ...
        return '<div style="display:flex;justify-content:space-between;gap:10px;padding:8px 0;border-top:1px solid var(--border);"><div style="min-width:0;"><div style="font-weight:600;font-size:13px;">' + esc(m.comercioSugerido || m.detalle) + '</div><div class="muted" style="font-size:11.5px;">' + dayLabel(m.fecha) + " \xB7 confianza " + item.confianza + '</div></div><span class="tabular" style="font-weight:600;flex-shrink:0;">' + (m.tipoMov === "ingreso" ? "+" : "") + money(Math.abs(m.monto)) + "</span></div>";
      }).join("") + "</div>";
    }
    if (diff.eliminarPropuesto.length) {
      html += '<div class="card" style="padding:14px;margin-bottom:10px;"><div class="sheet-block-title" style="margin-bottom:6px;">Posibles a eliminar (' + diff.eliminarPropuesto.length + ')</div><p class="muted" style="margin-bottom:10px;">Registradas autom\xE1ticamente, pero esta cartola no las respalda (o las muestra anuladas). Marca las que quieras eliminar \u2014 nunca se borran solas.</p>' + diff.eliminarPropuesto.map(function(item) {
        ...
        return '<label style="display:flex;align-items:flex-start;gap:10px;padding:8px 0;border-top:1px solid var(--border);cursor:pointer;"><input type="checkbox" data-reconcile-diff-elim-check="' + t.id + '" ' + (checked ? "checked" : "") + ' style="margin-top:3px;"><div style="min-width:0;flex:1;"><div style="font-weight:600;font-size:13px;">' + esc(t.comercio) + " \u2014 " + money(t.monto) + '</div><div class="muted" style="font-size:11.5px;">' + dayLabel(t.fecha) + '</div><div class="muted" style="font-size:11.5px;margin-top:2px;">' + item.motivo + "</div></div></label>";
    ...
    if (diff.revisar.length) {
      html += '<div class="card" style="padding:14px;margin-bottom:10px;"><div class="sheet-block-title" style="margin-bottom:6px;">Para revisar a mano (' + diff.revisar.length + ')</div><p class="muted" style="margin-bottom:10px;">Coincidencia dudosa, o hay m\xE1s de una candidata \u2014 no se toca nada autom\xE1ticamente, decide t\xFA.</p>' + diff.revisar.map(function(item) {
        ...
        const candidatosTxt = item.candidatos.map(function(t) {
          return esc(t.comercio) + " (" + dayLabel(t.fecha) + ", " + money(t.monto) + ")";
        }).join(" \xB7 ");
        return '<div style="padding:8px 0;border-top:1px solid var(--border);"><div style="font-weight:600;font-size:13px;">' + esc(m.comercioSugerido || m.detalle) + " \u2014 " + money(Math.abs(m.monto)) + " (" + dayLabel(m.fecha) + ')</div><div class="muted" style="font-size:11.5px;">confianza ' + item.confianza + " \xB7 posibles: " + candidatosTxt + "</div></div>";
      }).join("") + "</div>";
    }
    if (diff.manualesIgnoradas.length) {
      html += '<details class="card" style="padding:14px;margin-bottom:10px;"><summary style="font-weight:700;font-size:13.5px;cursor:pointer;">Manuales que no se tocan (' + diff.manualesIgnoradas.length + ')</summary><p class="muted" style="margin:8px 0;">Son transacciones manuales \u2014 la reconciliaci\xF3n autom\xE1tica nunca las modifica ni elimina, aunque la cartola no las respalde.</p>' + diff.manualesIgnoradas.map(function(item) {
        const t = item.tx;
        return '<div style="padding:6px 0;border-top:1px solid var(--border);"><div style="font-weight:600;font-size:12.5px;">' + esc(t.comercio) + " \u2014 " + money(t.monto) + " (" + dayLabel(t.fecha) + ')</div><div class="muted" style="font-size:11px;">' + item.motivo + "</div></div>";
      }).join("") + "</details>";
    }
    return html;
  }
  ...
  function createTxFromMovement(m, opts) {
    ...
    TRANSACTIONS.unshift({
      id: "trec" + nextImportId(),
      fecha: m.fecha,
      hora: "00:00",
      comercio: m.comercioSugerido || m.detalle,
  ...
  __name(createTxFromMovement, "createTxFromMovement");
  function renderMenuDemo() {
    document.getElementById("view-root").innerHTML = menuScreenHead("Modo demo") + '<div class="card" style="padding:16px;"><div class="menu-item-card" style="padding:0;"><span class="menu-item-card-icon" style="--fill:var(--cat-butter-fill);--ink:var(--cat-butter-ink)">' + ICONS.lock + '</span><div class="menu-item-card-body"><div class="menu-item-card-name">Mostrar datos de ejemplo</div><div class="menu-item-card-sub">\xDAtil para mostrar la app en p\xFAblico sin revelar tus n\xFAmeros reales</div></div><button class="switch ' + (state.demoMode ? "on" : "") + '" data-toggle-demo aria-label="Activar modo demo" aria-pressed="' + (state.demoMode ? "true" : "false") + '"></button></div><div class="platform-hint muted" style="margin-top:14px;">Cuando est\xE1 activado, TODA tu data real (transacciones, categor\xEDas, grupos, inversiones) se reemplaza por un set de ejemplo con n\xFAmeros inventados -- se ve completo, no enmascarado. Tu data real queda intacta y no se guarda nada de lo que hagas mientras el demo est\xE1 activo; al desactivarlo vuelves exactamente donde estabas. No se puede importar por correo, leer boletas, reconciliar cartolas ni enviar notificaciones push mientras el demo est\xE1 activo.</div></div>';
  }
  __name(renderMenuDemo, "renderMenuDemo");
  function renderMenuAsesoria() {
    document.getElementById("view-root").innerHTML = menuScreenHead("Asesor\xEDa financiera con Claude") + '<div class="card placeholder-card">' + ICONS.sparkle + '<h3>Pr\xF3ximamente</h3><p>La idea es conectar un asistente con acceso a tus transacciones, presupuestos y metas de inversi\xF3n, para que puedas preguntarle directamente sobre tu plata \u2014 por ejemplo "\xBFen qu\xE9 categor\xEDa me estoy pasando este mes?" o "\xBFc\xF3mo voy con la meta del pie del depto?".</p><p class="muted" style="margin-top:10px;">Todav\xEDa no est\xE1 disponible en esta maqueta.</p></div>';
  }
  __name(renderMenuAsesoria, "renderMenuAsesoria");
  function transferInfoComplete() {
    const d = TRANSFER_INFO;
    return !!(d.nombre || d.rut || d.banco || d.tipoCuenta || d.numeroCuenta || d.email);
  }
  __name(transferInfoComplete, "transferInfoComplete");
  function renderDatosTransferenciaCard() {
    const d = TRANSFER_INFO;
    if (state.editingTransferInfo) {
      const dr = state.transferInfoDraft;
      return '<div class="card" style="padding:16px;margin-top:14px;"><div class="budget-total-label">Datos de transferencia</div><p class="cat-picker-hint" style="margin:6px 0 10px;">Se usan solo para armar el texto que copias al pedir un cobro pendiente \u2014 no se comparten con nadie m\xE1s.</p><label class="draft-label">Nombre</label><input type="text" class="draft-input" data-transfer-info-input="nombre" value="' + esc(dr.nombre) + '" placeholder="Nombre completo"><label class="draft-label" style="margin-top:10px;">RUT</label><input type="text" class="draft-input" data-transfer-info-input="rut" value="' + esc(dr.rut) + '" placeholder="12.345.678-9"><label class="draft-label" style="margin-top:10px;">Banco</label><input type="text" class="draft-input" data-transfer-info-input="banco" value="' + esc(dr.banco) + '" placeholder="Ej: Banco Estado"><label class="draft-label" style="margin-top:10px;">Tipo de cuenta</label><input type="text" class="draft-input" data-transfer-info-input="tipoCuenta" value="' + esc(dr.tipoCuenta) + '" placeholder="Cuenta RUT, Vista, Corriente\u2026"><label class="draft-label" style="margin-top:10px;">N\xFAmero de cuenta</label><input type="text" class="draft-input" data-transfer-info-input="numeroCuenta" value="' + esc(dr.numeroCuenta) + '" placeholder="0000000000"><label class="draft-label" style="margin-top:10px;">Email (opcional)</label><input type="text" class="draft-input" data-transfer-info-input="email" value="' + esc(dr.email) + '" placeholder="tucorreo@ejemplo.cl"><div style="display:flex;gap:10px;margin-top:14px;"><button class="save-tx-btn" style="background:var(--surface-sunken);color:var(--text);flex:1;" data-cancel-transfer-info>Cancelar</button><button class="save-tx-btn" style="flex:1;" data-save-transfer-info>Guardar</button></div></div>';
    }
    const completos = transferInfoComplete();
    return '<div class="card" style="padding:16px;margin-top:14px;"><div class="budget-total-head"><span class="budget-total-label">Datos de transferencia</span><button class="budget-edit-btn" data-edit-transfer-info aria-label="Editar datos de transferencia">' + ICONS.edit + "</button></div>" + (completos ? '<div class="datos-transferencia-figs">' + (d.nombre ? "<div>" + esc(d.nombre) + "</div>" : "") + (d.rut ? "<div>RUT " + esc(d.rut) + "</div>" : "") + (d.banco || d.tipoCuenta ? "<div>" + [d.banco, d.tipoCuenta].filter(Boolean).join(" \xB7 ") + "</div>" : "") + (d.numeroCuenta ? "<div>Cuenta " + esc(d.numeroCuenta) + "</div>" : "") + (d.email ? "<div>" + esc(d.email) + "</div>" : "") + "</div>" : '<p class="cat-picker-hint" style="margin:6px 0 0;">Agr\xE9galos para poder copiar, listo para pegar en WhatsApp, un cobro pendiente junto con c\xF3mo te pueden transferir.</p>') + "</div>";
  }
  __name(renderDatosTransferenciaCard, "renderDatosTransferenciaCard");
  function renderMenuCuenta() {
    const email = currentUser ? currentUser.email : "\u2014";
    document.getElementById("view-root").innerHTML = menuScreenHead("Mi cuenta") + '<div class="card menu-item-card" style="padding:16px;"><span class="menu-item-card-icon" style="--fill:var(--accent-soft);--ink:var(--accent-ink)">' + ICONS.lockSmall + '</span><div class="menu-item-card-body"><div class="menu-item-card-name">' + email + '</div><div class="menu-item-card-sub">Tus datos est\xE1n protegidos: solo t\xFA (o quien invites m\xE1s adelante a tu hogar) puede verlos.</div></div></div>' + renderDatosTransferenciaCard() + '<button class="budget-delete-link" style="margin-top:14px;" data-auth-logout>Cerrar sesi\xF3n</button>';
  }
  ...
  function buildChargeWhatsAppText(t) {
    ...
    let n = 1;
    if (tuParte > 0) {
      lines.push(n + ". T\xFA " + fmt.format(tuParte));
      n++;
    }
    pendientes.forEach((p) => {
      lines.push(n + ". " + (p.persona || "Sin nombre") + " " + fmt.format(Math.round(p.monto || 0)));
      n++;
    });
    lines.push("");
    ...
    if (d.banco || d.tipoCuenta) datosLines.push([d.banco, d.tipoCuenta].filter(Boolean).join(" \xB7 "));
  ...
  __name(buildChargeWhatsAppText, "buildChargeWhatsAppText");
  async function loadEmailImportScreen() {
    if (!sb || !currentHouseholdId) {
      state.emailImportLoading = false;
      state.emailImportError = "No hay conexi\xF3n con el servidor todav\xEDa.";
      renderMenuView();
      return;
    }
    try {
      ...
      console.error("Pitucas sin lucas \u2014 error cargando importaci\xF3n por correo:", err);
      state.emailImportError = translateAuthError(err);
    }
    state.emailImportLoading = false;
    renderMenuView();
  }
  __name(loadEmailImportScreen, "loadEmailImportScreen");
  function guessPaymentMethodIdFromSuggestion(sug) {
    if (!sug) return null;
    const m = String(sug).match(/(\d{4})\D*$/);
    if (!m) return null;
    const last4 = m[1];
    const found = Object.keys(PAYMENT_METHODS).find(function(id) {
      return (PAYMENT_METHODS[id].corto || "").indexOf(last4) >= 0;
    });
    return found || null;
  }
  __name(guessPaymentMethodIdFromSuggestion, "guessPaymentMethodIdFromSuggestion");
  function ensurePaymentMethodForSuggestion(sug) {
  ...
  __name(ensurePaymentMethodForSuggestion, "ensurePaymentMethodForSuggestion");
  function ensureCheckingAccountMethod() {
    const id = "cuenta_vista";
    if (!PAYMENT_METHODS[id]) {
      PAYMENT_METHODS[id] = { nombre: "Cuenta Vista", corto: "Cta. Vista", icon: "bank" };
    }
    return id;
  }
  __name(ensureCheckingAccountMethod, "ensureCheckingAccountMethod");
  function ensureUnknownPaymentMethod() {
    const id = "medio_desconocido";
    if (!PAYMENT_METHODS[id]) {
      PAYMENT_METHODS[id] = { nombre: "Medio sin identificar", corto: "Sin identificar", icon: "card" };
    }
    return id;
  }
  __name(ensureUnknownPaymentMethod, "ensureUnknownPaymentMethod");
  function guessCatIdFromImportRow(row) {
    if (row.tipo !== "inversion") return null;
    const f = (row.fuente || "").toLowerCase();
    const candidatos = ["racional", "fintual", "banco_chile", "buda"];
    const found = candidatos.find(function(id) {
      return CATEGORIES[id] && f.indexOf(id.replace("_", "")) >= 0;
    });
  ...
  async function absorbImportedRows() {
    if (!sb || !currentHouseholdId) return;
    try {
      const { data: rows, error } = await sb.from("transacciones_importadas").select("*").eq("household_id", currentHouseholdId).eq("procesado", false).order("fecha", { ascending: true });
      if (error) throw error;
      ...
      if (!guardado) {
        toast("No se pudo guardar la transacci\xF3n importada \u2014 revisa tu conexi\xF3n, se reintentar\xE1 m\xE1s tarde");
        return;
      }
      toast(rows.length === 1 ? "Se agreg\xF3 1 transacci\xF3n desde tu correo" : "Se agregaron " + rows.length + " transacciones desde tu correo");
  ...
  __name(absorbImportedRows, "absorbImportedRows");
  var SHARED_EXPENSE_PAYMENT_METHOD_ID = "grupo_compartido";
  function ensureSharedExpensePaymentMethod() {
    const id = SHARED_EXPENSE_PAYMENT_METHOD_ID;
    if (!PAYMENT_METHODS[id]) {
      PAYMENT_METHODS[id] = { nombre: "Gasto de grupo", corto: "Grupo", icon: "users" };
    }
    return id;
  }
  __name(ensureSharedExpensePaymentMethod, "ensureSharedExpensePaymentMethod");
  function syncSharedExpenses() {
    const previas = TRANSACTIONS.filter((t) => t.sharedByOthers).map((t) => Object.assign({}, t));
    for (let i = TRANSACTIONS.length - 1; i >= 0; i--) {
      if (TRANSACTIONS[i].sharedByOthers) TRANSACTIONS.splice(i, 1);
    }
    if (!currentUser) return;
    SHARED_EXPENSES.forEach((g) => {
      const miParticipanteId = participantIdForUser(g.grupo_id, currentUser.id);
      if (!miParticipanteId) return;
      if (g.pagado_por === miParticipanteId) return;
      const miReparto = (g.reparto || []).find((r) => r.participante_id === miParticipanteId);
      if (!miReparto || miReparto.monto <= 0) return;
      ...
      const mapeo = registradorParticipanteId ? CATEGORY_MAPPINGS.find(
        (m) => m.user_id === currentUser.id && m.de_participante === registradorParticipanteId && m.categoria_ajena === g.categoria_origen
      ) : null;
      ...
      const tx = {
        ...
        porCobrar: [],
        reglaAuto: false,
        nota: 'Tu parte de "' + esc(g.descripcion) + '"' + (pagador ? " \u2014 pag\xF3 " + pagador.nombre : "") + (grupo ? " \xB7 grupo " + grupo.nombre : ""),
        groupId: g.grupo_id,
  ...
  function materializarPartesDeGruposQueYaNoEstan(previas) {
    previas.forEach((prev) => {
      ...
      const tx = Object.assign({}, prev, {
        ...
        origen: "manual",
        nota: (prev.nota ? prev.nota + " \u2014 " : "") + "El grupo ya no est\xE1: se conserv\xF3 tu parte como movimiento propio."
      });
      TRANSACTIONS.push(tx);
      ensureMonthExists(tx.fecha.slice(0, 7));
    });
  }
  __name(materializarPartesDeGruposQueYaNoEstan, "materializarPartesDeGruposQueYaNoEstan");
  async function classifySharedExpenseFromOthers(txId, catId) {
    const tx = getTx(txId);
    ...
    if (g && currentUser) {
      ...
      if (registradorParticipanteId && g.categoria_origen) {
        ...
        CATEGORY_MAPPINGS.push(nuevo);
        if (sb) {
          try {
            await sb.from("mapeo_categorias").upsert(nuevo, { onConflict: "user_id,de_participante,categoria_ajena" });
          } catch (err) {
            console.error("Pitucas sin lucas \u2014 error guardando el mapeo de categor\xEDas:", err);
          }
        }
      }
    }
  ...
  __name(classifySharedExpenseFromOthers, "classifySharedExpenseFromOthers");
  async function loadSharedExpenses() {
    ...
    try {
      ...
      if (eM) throw eM;
      setGroups(grupos || []);
      setPaidBalances(saldos || []);
      setCategoryMappings(mapeo || []);
  ...
  __name(loadSharedExpenses, "loadSharedExpenses");
  var groupsRealtimeChannel = null;
  function setGroupsRealtimeChannel(v) {
    groupsRealtimeChannel = v;
  }
  __name(setGroupsRealtimeChannel, "setGroupsRealtimeChannel");
  function subscribeToGroupsLive() {
    if (!sb || groupsRealtimeChannel) return;
    groupsRealtimeChannel = sb.channel("gastos-compartidos").on("postgres_changes", { event: "*", schema: "public", table: "grupos" }, loadSharedExpenses).on("postgres_changes", { event: "*", schema: "public", table: "grupo_participantes" }, loadSharedExpenses).on("postgres_changes", { event: "*", schema: "public", table: "gastos_compartidos" }, loadSharedExpenses).on("postgres_changes", { event: "*", schema: "public", table: "gasto_reparto" }, loadSharedExpenses).on("postgres_changes", { event: "*", schema: "public", table: "saldos_pagados" }, loadSharedExpenses).subscribe();
  }
  ...
  async function deleteGroup(groupId) {
    ...
    if (checkErr) {
      console.error("Pitucas sin lucas \u2014 no se pudo confirmar que el grupo se borr\xF3:", checkErr);
      return { ok: false, error: checkErr };
    }
    if (sigueExistiendo) {
      const fakeError = { message: "No tienes permiso para eliminar este grupo (solo quien lo cre\xF3 puede hacerlo)." };
  ...
  async function joinGroup(inviteCode, nombre) {
    ...
    if (error) {
      console.error("Pitucas sin lucas \u2014 error uni\xE9ndose al grupo:", error);
      return { ok: false, error };
    }
    await loadSharedExpenses();
    return { ok: true, error: null };
  ...
  __name(addParticipantWithoutAccount, "addParticipantWithoutAccount");
  async function editGroupParticipant(participantId, nombre) {
    if (!sb) return { ok: false, error: null };
    const { error } = await sb.from("grupo_participantes").update({ nombre }).eq("id", participantId);
    if (error) {
      console.error("Pitucas sin lucas \u2014 error editando participante:", error);
      return { ok: false, error };
    }
    await loadSharedExpenses();
    return { ok: true, error: null };
  }
  __name(editGroupParticipant, "editGroupParticipant");
  async function deleteGroupParticipant(participantId) {
    if (!sb) return { ok: false, error: null };
    ...
    if (error) {
      console.error("Pitucas sin lucas \u2014 error eliminando participante:", error);
      return { ok: false, error };
    }
    const { data: sigueExistiendo, error: checkErr } = await sb.from("grupo_participantes").select("id").eq("id", participantId).maybeSingle();
    if (checkErr) {
      console.error("Pitucas sin lucas \u2014 no se pudo confirmar que el participante se borr\xF3:", checkErr);
      return { ok: false, error: checkErr };
    }
    if (sigueExistiendo) {
  ...
  __name(deleteGroupParticipant, "deleteGroupParticipant");
  async function crearGastoCompartido(opts) {
    ...
    const otrosSplits = Object.keys(opts.reparto).filter((pid) => pid !== miParticipanteId).map((pid) => ({ persona: (GROUP_PARTICIPANTS.find((p) => p.id === pid) || {}).nombre || "", monto: opts.reparto[pid], pagado: false, tipo: "persona", montoRecibido: null, linkedTxId: null, groupId: opts.groupId, participanteId: pid }));
    const txOrigen = soyYoQuienPago ? {
      id: "gasto-" + Date.now(),
      fecha: opts.fecha,
      hora: todayISO() === opts.fecha ? (/* @__PURE__ */ new Date()).toTimeString().slice(0, 5) : "12:00",
      comercio: opts.descripcion,
      monto: Math.round(opts.monto),
      medio: opts.medio || ensureSharedExpensePaymentMethod(),
      tipo: "gasto",
      recurrencia: "variable",
      estado: otrosSplits.length ? "por_cobrar" : "confirmado",
    ...
    const { data: gasto, error } = await sb.from("gastos_compartidos").insert({
      grupo_id: opts.groupId,
      descripcion: opts.descripcion,
      categoria_origen: opts.categoriaOrigen || null,
      monto: Math.round(opts.monto),
      fecha: opts.fecha,
      pagado_por: opts.pagadoPorId,
      registrado_por: currentUser.id,
      division_tipo: opts.divisionTipo || "iguales",
      tx_origen_id: txOrigen.id
    }).select().single();
    if (error) {
    ...
    txOrigen.porCobrar.forEach((p) => {
      p.sharedExpenseId = gasto.id;
    });
  ...
  __name(crearGastoCompartido, "crearGastoCompartido");
  async function shareExistingTransaction(txId, groupId, pagadoPorId, divisionTipo, reparto) {
    if (!sb || !currentUser) return null;
    const tx = getTx(txId);
    if (!tx) return null;
    ...
    const { data: gasto, error } = await sb.from("gastos_compartidos").insert({
      ...
      categoria_origen: tx.categorias[0] ? catInfo(tx.categorias[0].cat).nombre : null,
      monto: Math.round(tx.monto),
      fecha: tx.fecha,
  ...
  __name(notifApiSupported, "notifApiSupported");
  ...
  __name(urlBase64ToUint8Array_, "urlBase64ToUint8Array_");
  async function loadNotifStatus() {
    ...
    try {
      if (!notifApiSupported()) {
        ...
        const reg = await navigator.serviceWorker.getRegistration("./sw.js").catch(function() {
          return null;
        });
        const sub = reg ? await reg.pushManager.getSubscription().catch(function() {
          return null;
        }) : null;
  ...
  __name(loadNotifStatus, "loadNotifStatus");
  async function enableNotifications() {
    ...
    try {
      ...
      if (!pushWorkerConfigured()) throw new Error("Todav\xEDa falta desplegar el Worker de notificaciones (ver DOCUMENTACION.md).");
      const permission = await Notification.requestPermission();
      if (permission !== "granted") throw new Error("No diste el permiso de notificaciones (rev\xEDsalo en los ajustes del navegador/celular).");
      ...
      if (!sb || !currentHouseholdId) throw new Error("No hay conexi\xF3n con el servidor todav\xEDa.");
  ...
  __name(disableNotifications, "disableNotifications");
  function enviarPushHogar(title, message, url) {
    ...
    fetch(PUSH_WORKER_URL + "/notify", {
      ...
      body: JSON.stringify({ household_id: currentHouseholdId, token: state.importToken, title, message: message || "", url: url || "./index.html" })
    }).catch(function(err) {
      console.error("Pitucas sin lucas \u2014 error mandando notificaci\xF3n push:", err);
    });
    return true;
  }
  __name(enviarPushHogar, "enviarPushHogar");
  async function sendTestPush() {
    ...
    renderMenuView();
    try {
      if (!pushWorkerConfigured()) throw new Error("Todav\xEDa falta desplegar el Worker de notificaciones.");
      if (!sb || !currentHouseholdId || !state.importToken) throw new Error("No hay conexi\xF3n con el servidor todav\xEDa -- espera un momento y prueba de nuevo.");
      const res = await fetch(PUSH_WORKER_URL + "/notify", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ household_id: currentHouseholdId, token: state.importToken, title: "Aviso de prueba", message: "Si ves esto, las notificaciones est\xE1n funcionando.", url: "./index.html" })
      });
      let data = null;
      try {
        data = await res.json();
      ...
      if (!res.ok) {
        state.notifTestResult = "El Worker respondi\xF3 con un error (" + res.status + "): " + (data && data.error ? data.error : "sin m\xE1s detalle.");
      } else if (data && data.delivered > 0) {
        state.notifTestResult = "El Worker mand\xF3 el aviso a " + data.delivered + " dispositivo(s). Si aun as\xED no te lleg\xF3, revisa que las notificaciones est\xE9n permitidas para este sitio en los ajustes de tu celular/navegador.";
      } else {
        state.notifTestResult = data && data.note ? data.note : "El Worker respondi\xF3, pero no hay ning\xFAn dispositivo suscrito en tu hogar para mandarle el aviso -- prueba desactivar y activar notificaciones de nuevo en este dispositivo.";
      }
    } catch (err) {
  ...
  __name(fotoBoletaABase64_, "fotoBoletaABase64_");
  async function leerBoletaConOCR(file) {
    if (!boletaWorkerConfigured()) return { error: "Todav\xEDa falta terminar de configurar el lector de boletas." };
    if (!sb || !currentHouseholdId || !state.importToken) return { error: "No hay conexi\xF3n con el servidor todav\xEDa -- espera un momento y prueba de nuevo." };
    try {
      ...
      if (!data.items || !data.items.length) {
        return { error: "No se encontraron items en la foto -- prueba con otra foto m\xE1s n\xEDtida, o agr\xE9galos a mano." };
      }
      return { items: data.items, comercio: data.comercio || null };
    } catch (err) {
      console.error("Pitucas sin lucas \u2014 error leyendo boleta:", err);
      return { error: "No se pudo leer la boleta (revisa tu conexi\xF3n)." };
    }
  }
  __name(leerBoletaConOCR, "leerBoletaConOCR");
  function budgetAlertText(catNombre, umbral, gastado, meta) {
    return {
      titulo: catNombre + ": has alcanzado el " + umbral + "% de tu presupuesto mensual!",
      mensaje: money(gastado) + " de " + money(meta)
    };
  }
  __name(budgetAlertText, "budgetAlertText");
  function checkBudgetPushAlerts() {
  ...
  __name(renderMenuImportarCorreo, "renderMenuImportarCorreo");
  function renderMenuNotificaciones() {
    const head = menuScreenHead("Notificaciones");
    if (!notifApiSupported()) {
      document.getElementById("view-root").innerHTML = head + '<div class="card placeholder-card">' + ICONS.ban + "<h3>No disponible en este navegador</h3><p>Este navegador no soporta notificaciones push. Prueba desde Chrome/Safari en tu celular, idealmente con la app instalada en tu pantalla de inicio.</p></div>";
      return;
    }
    ...
    const statusBlock = '<div class="card placeholder-card">' + (state.notifSubscribed ? ICONS.checkCircle : ICONS.bell) + "<h3>" + (state.notifSubscribed ? "Activadas en este dispositivo" : "Notificaciones desactivadas") + '</h3><p>Te avisamos apenas llega una transacci\xF3n nueva desde tu correo, y cuando un presupuesto de categor\xEDa cruza el 80%, 90% o 100% de su meta. Esto se activa por separado en cada celular/computador donde uses la app.</p><button class="save-tx-btn" style="margin-top:12px;" data-notif-toggle' + (state.notifBusy ? " disabled" : "") + ">" + (state.notifBusy ? "Un momento\u2026" : state.notifSubscribed ? "Desactivar en este dispositivo" : "Activar en este dispositivo") + "</button></div>";
    const testBlock = !state.notifSubscribed ? "" : '<div class="card placeholder-card" style="margin-top:12px;"><button class="save-tx-btn" style="background:var(--surface-sunken);color:var(--text);" data-notif-test' + (state.notifTestBusy ? " disabled" : "") + ">" + (state.notifTestBusy ? "Enviando\u2026" : "Enviar aviso de prueba") + "</button>" + (state.notifTestResult ? '<p class="platform-hint" style="margin-top:10px;">' + state.notifTestResult + "</p>" : "") + "</div>";
    document.getElementById("view-root").innerHTML = head + errorBlock + statusBlock + testBlock;
  }
  __name(renderMenuNotificaciones, "renderMenuNotificaciones");
  function renderMenuView() {
    document.getElementById("header-title").textContent = "Men\xFA";
  ...
  __name(renderMenuView, "renderMenuView");
  ...
  __name(dateInMonth, "dateInMonth");
  ...
  function monthLabelOf(monthsBack) {
  ...
  __name(monthLabelOf, "monthLabelOf");
  function demoCategories() {
    const out = {};
    Object.keys(CATEGORY_SEED_DEFAULTS).forEach((k) => {
      out[k] = Object.assign({}, CATEGORY_SEED_DEFAULTS[k]);
    });
  ...
  __name(demoCategories, "demoCategories");
  function demoTransactions() {
    const tx = [];
    let n = 0;
    const id = /* @__PURE__ */ __name(() => "demo_t" + ++n, "id");
    const sueldos = [125e4, 125e4, 122e4, 12e5, 12e5];
    for (let m = 4; m >= 0; m--) {
      ...
      tx.push({ id: id(), fecha: dateInMonth(m, 5), hora: "10:00", comercio: "Arriendo depto", monto: 38e4, medio: "cuenta_vista", tipo: "gasto", recurrencia: "mensual", estado: "confirmado", categorias: [{ cat: "hogar", monto: 38e4 }], porCobrar: [], reglaAuto: true, nota: "" });
      tx.push({ id: id(), fecha: dateInMonth(m, 20), hora: "19:10", comercio: "Jumbo \xD1u\xF1oa", monto: 48e3 + m * 1e3, medio: "debito_bci", tipo: "gasto", recurrencia: "variable", estado: "confirmado", categorias: [{ cat: "supermercado", monto: 48e3 + m * 1e3 }], porCobrar: [], reglaAuto: false, nota: "" });
      ...
      tx.push({ id: id(), fecha: dateInMonth(m, 8), hora: "08:10", comercio: "Copec Providencia", monto: 19e3, medio: "visa_bch", tipo: "gasto", recurrencia: "variable", estado: "confirmado", categorias: [{ cat: "transporte", monto: 19e3 }], porCobrar: [], reglaAuto: true, nota: "" });
      tx.push({ id: id(), fecha: dateInMonth(m, 10), hora: "10:00", comercio: "Aporte Fintual", monto: 1e5, medio: "cuenta_vista", tipo: "inversion", recurrencia: "mensual", estado: "confirmado", categorias: [{ cat: "fintual", monto: 1e5 }], porCobrar: [], reglaAuto: false, nota: "" });
    }
    tx.push({
      id: id(),
      fecha: dateInMonth(0, 12),
      hora: "12:30",
      comercio: "Farmacias Ahumada",
      monto: 15200,
      medio: "debito_bci",
      tipo: "gasto",
      recurrencia: "variable",
      estado: "por_cobrar",
      categorias: [{ cat: "salud", monto: 15200 }],
      porCobrar: [{ persona: "Isapre", monto: null, pagado: false, tipo: "reembolso", montoRecibido: null, linkedTxId: null }],
      reglaAuto: false,
      nota: "Reembolso de la isapre en camino"
    });
    tx.push({
      id: id(),
      fecha: dateInMonth(0, 6),
      hora: "21:00",
      comercio: "Restobar Lastarria",
      monto: 64e3,
      medio: "visa_bch",
      tipo: "gasto",
      recurrencia: "variable",
      estado: "por_cobrar",
      categorias: [{ cat: "restoranes", monto: 64e3 }],
      porCobrar: [
      ...
      pagadorDivisionValor: 1,
      reglaAuto: false,
      nota: "Cumplea\xF1os"
    });
    tx.push({
      id: id(),
      fecha: dateInMonth(0, 15),
      hora: "16:20",
      comercio: "Falabella \xB7 Notebook",
      monto: 3e4,
      medio: "visa_bch",
      tipo: "gasto",
      recurrencia: "variable",
      estado: "confirmado",
      categorias: [{ cat: "hogar", monto: 3e4 }],
      porCobrar: [],
      reglaAuto: false,
      nota: "",
      cuotas: { total: 3, montoTotal: 9e4 }
    });
    tx.push({
      id: id(),
      fecha: dateInMonth(0, 3),
      hora: "14:10",
      comercio: "Transferencia entre mis cuentas",
      monto: 2e5,
      medio: "cuenta_vista",
      tipo: "gasto",
      recurrencia: "variable",
      ...
      porCobrar: [],
      reglaAuto: false,
      nota: "Traspaso, no es un gasto real"
    });
    tx.push({
      id: id(),
      fecha: dateInMonth(0, 16),
      hora: "09:00",
      comercio: "Freelance dise\xF1o web",
      monto: 18e4,
      medio: "cuenta_vista",
      tipo: "ingreso",
      recurrencia: "variable",
      estado: "confirmado",
      categorias: [{ cat: "pololos_extra", monto: 18e4 }],
      porCobrar: [],
      reglaAuto: false,
      nota: ""
    });
    tx.push({
      id: id(),
      fecha: dateInMonth(0, 2),
      hora: "12:00",
      comercio: "Venta bicicleta vieja",
      monto: 4e4,
      medio: "efectivo",
      tipo: "ingreso",
      recurrencia: "variable",
  ...
  function demoBudgets() {
    return {
      supermercado: { meta: 18e4, alertas: { 80: true, 90: true, 100: true } },
      transporte: { meta: 9e4, alertas: { 80: true, 90: false, 100: true } },
      restoranes: { meta: 6e4, alertas: { 80: true, 90: true, 100: true } },
      hogar: { meta: 42e4, alertas: { 80: false, 90: false, 100: true } },
      salud: { meta: 3e4, alertas: { 80: true, 90: true, 100: true } },
      suscripciones: { meta: 15e3, alertas: { 80: true, 90: true, 100: true } }
    };
  }
  __name(demoBudgets, "demoBudgets");
  function demoPlatformData() {
    const hist = /* @__PURE__ */ __name((base, step) => {
      const out = {};
      for (let m = 4; m >= 0; m--) out[monthKey(m)] = base + (4 - m) * step;
      return out;
    }, "hist");
    return {
      fintual: { valorHistorial: hist(81600, 105e3), fechaActualizacion: dateInMonth(0, 10), tasaAnual: null, comision: null, plazo: "largo" },
      racional: { valorHistorial: hist(40800, 33e3), fechaActualizacion: dateInMonth(1, 10), tasaAnual: null, comision: null, plazo: "largo" },
      banco_chile: { valorHistorial: hist(37e5, 45e4), fechaActualizacion: dateInMonth(0, 5), tasaAnual: null, comision: null, plazo: null },
      buda: { valorHistorial: hist(2e5, 15e3), fechaActualizacion: dateInMonth(1, 1), tasaAnual: null, comision: null, plazo: "largo" }
    };
  }
  __name(demoPlatformData, "demoPlatformData");
  function demoInvestmentGoals() {
    const startMonth = monthKey(4);
    return [
      { id: "demo_m1", nombre: "Fondo de emergencia", montoObjetivo: 3e6, aporteMensualMeta: 15e4, plataformaId: "banco_chile", plazo: "corto", comision: null, startMonth, startingAmount: 208e4, checks: { [monthKey(4)]: true, [monthKey(3)]: true, [monthKey(2)]: true, [monthKey(1)]: true, [monthKey(0)]: true } },
      { id: "demo_m2", nombre: "Pie departamento", montoObjetivo: 8e6, aporteMensualMeta: 3e5, plataformaId: "banco_chile", plazo: "medio", comision: null, startMonth, startingAmount: 312e4, checks: { [monthKey(4)]: true, [monthKey(3)]: true, [monthKey(2)]: true, [monthKey(1)]: true, [monthKey(0)]: true } },
      { id: "demo_m3", nombre: "Jubilaci\xF3n (APV)", aporteMensualMeta: 1e5, plataformaId: "fintual", plazo: "largo", comision: null, startMonth, startingAmount: 0, checks: { [monthKey(4)]: true, [monthKey(3)]: true, [monthKey(2)]: true, [monthKey(1)]: true, [monthKey(0)]: true } },
      { id: "demo_m4", nombre: "Cripto largo plazo", aporteMensualMeta: 4e4, plataformaId: "buda", plazo: "largo", comision: null, startMonth, startingAmount: 0, checks: { [monthKey(4)]: true, [monthKey(3)]: true, [monthKey(2)]: false, [monthKey(1)]: true, [monthKey(0)]: true } }
    ];
  }
  __name(demoInvestmentGoals, "demoInvestmentGoals");
  function demoGroupsBundle() {
    const myId = currentUser ? currentUser.id : "demo-sin-sesion";
    const grupoId = "demo_g1";
    const pYo = "demo_p1", pCaro = "demo_p2";
    const grupos = [{ id: grupoId, nombre: "Depto con Caro", icono: "\u{1F3E0}", creado_por: myId, invite_code: "DEMO01", created_at: dateInMonth(1, 1) + "T00:00:00Z" }];
    const participantes = [
      { id: pYo, grupo_id: grupoId, user_id: myId, nombre: "T\xFA", color: "lavender" },
    ...
    const gastosCompartidos = [
      {
        id: "demo_se1",
        grupo_id: grupoId,
        descripcion: "Supermercado del depto",
        categoria_origen: "supermercado",
        monto: 64e3,
        fecha: dateInMonth(0, 9),
        pagado_por: pYo,
        registrado_por: myId,
        division_tipo: "iguales",
        tx_origen_id: null,
        reparto: [
          { id: "demo_sp1", gasto_compartido_id: "demo_se1", participante_id: pYo, monto: 32e3 },
          { id: "demo_sp2", gasto_compartido_id: "demo_se1", participante_id: pCaro, monto: 32e3 }
        ]
      },
      {
        id: "demo_se2",
        grupo_id: grupoId,
        descripcion: "Cuenta de la luz",
        categoria_origen: "hogar",
        monto: 45e3,
        fecha: dateInMonth(0, 4),
        pagado_por: pCaro,
        registrado_por: myId,
        division_tipo: "iguales",
        tx_origen_id: null,
  ...
  __name(demoGroupsBundle, "demoGroupsBundle");
  function buildDemoBlob() {
    ...
    return {
      ...
      mediosPago: {
        ...
        debito_bci: { nombre: "D\xE9bito BCI", corto: "\u2022\u2022\u2022\u2022 9034", icon: "card" },
      ...
      planificador: { base: 9e5, metaPcts: {} },
      metasTotalChecks: {},
  ...
  __name(buildDemoBlob, "buildDemoBlob");
  function isDemoActive() {
    return realSnapshot !== null;
  }
  __name(isDemoActive, "isDemoActive");
  function enterDemoMode() {
    ...
    realSnapshot = {
      ...
      participantes,
      gastosCompartidos,
      saldosPagados: clone(PAID_BALANCES),
      mapeoCategorias: clone(CATEGORY_MAPPINGS),
      sb
    };
    if (groupsRealtimeChannel && sb) {
      sb.removeChannel(groupsRealtimeChannel);
      setGroupsRealtimeChannel(null);
    }
    setSb(null);
  ...
  __name(enterDemoMode, "enterDemoMode");
  ...
  __name(splitTypeLabel, "splitTypeLabel");
  function buildIntegrantesRows(groupId) {
  ...
  __name(buildIntegrantesRows, "buildIntegrantesRows");
  function buildTransaccionesRows(groupId) {
  ...
  __name(buildTransaccionesRows, "buildTransaccionesRows");
  function buildDetalleDivisionRows(groupId) {
    const participantes = participantsOfGroup(groupId);
    const rows = [];
  ...
  __name(buildDetalleDivisionRows, "buildDetalleDivisionRows");
  function buildTransferenciasRows(groupId) {
    const participantes = participantsOfGroup(groupId);
    const nombreDe = /* @__PURE__ */ __name((id) => {
      const p = participantes.find((x) => x.id === id);
      return p ? p.nombre : "?";
    }, "nombreDe");
    const saldadas = PAID_BALANCES.filter((s) => s.grupo_id === groupId).map((s) => ({
      fecha: s.fecha,
      de: nombreDe(s.de_participante),
      a: nombreDe(s.a_participante),
  ...
  __name(buildTransferenciasRows, "buildTransferenciasRows");
  function groupExportSheets(groupId) {
    return {
      "Integrantes": [["Nombre", "Saldo neto", "Total pagado", "Total que le corresponde"]].concat(
        buildIntegrantesRows(groupId).map((r) => [r.nombre, r.saldoNeto, r.totalPagado, r.totalLeCorresponde])
      ),
      "Transacciones": [["Fecha", "Descripci\xF3n", "Categor\xEDa", "Monto total", "Qui\xE9n pag\xF3", "Modo de divisi\xF3n"]].concat(
        buildTransaccionesRows(groupId).map((r) => [r.fecha, r.descripcion, r.categoria, r.montoTotal, r.quienPago, r.modoDivision])
      ),
      "Detalle de divisi\xF3n": [["Transacci\xF3n", "Participante", "Parte que le toca"]].concat(
        buildDetalleDivisionRows(groupId).map((r) => [r.transaccion, r.participante, r.parte])
      ),
      "Transferencias": [["Fecha", "De", "A", "Monto", "Estado"]].concat(
        buildTransferenciasRows(groupId).map((r) => [r.fecha, r.de, r.a, r.monto, r.estado])
      )
    };
  }
  __name(groupExportSheets, "groupExportSheets");
  function buildGroupExportWorkbookArrayBuffer(groupId) {
  ...
  __name(buildGroupExportWorkbookArrayBuffer, "buildGroupExportWorkbookArrayBuffer");
  ...
  function applyCommonFilters(list) {
    ...
    if (state.categoryFilterMonth) {
      list = state.categoryFilterMonth.length === 4 ? list.filter((t) => t.fecha.slice(0, 4) === state.categoryFilterMonth) : list.filter((t) => t.fecha.slice(0, 7) === state.categoryFilterMonth);
    }
  ...
  __name(applyCommonFilters, "applyCommonFilters");
  function filteredTx() {
  ...
  function renderFilterSummary() {
    ...
    if (state.filter === "entradas") {
      ...
      return '<div class="stat-grid" style="grid-template-columns:1fr 1fr;margin-bottom:14px;"><div class="card stat-tile stat-ingresos"><div class="stat-label">Entradas</div><div class="stat-value tabular">' + money(ingresos) + '</div></div><div class="card stat-tile" style="background:var(--surface);border:1px solid var(--border);"><div class="stat-label">Reembolsos</div><div class="stat-value tabular">' + money(reembolsos) + "</div></div></div>";
    }
    if (state.filter === "porcobrar") {
      ...
      return '<div class="stat-grid" style="grid-template-columns:1fr 1fr 1fr;margin-bottom:14px;"><div class="card stat-tile" style="padding:11px 10px;background:var(--surface);border:1px solid var(--border);"><div class="stat-label" style="font-size:10.5px;">Salidas</div><div class="stat-value tabular" style="font-size:15px;">' + money(bruto) + '</div></div><div class="card stat-tile" style="padding:11px 10px;background:color-mix(in srgb, var(--invest-fill) 16%, var(--surface));"><div class="stat-label" style="font-size:10.5px;">Por cobrar</div><div class="stat-value tabular" style="font-size:15px;color:var(--invest-ink);">' + money(pendiente) + '</div></div><div class="card stat-tile" style="padding:11px 10px;background:color-mix(in srgb, var(--income-fill) 16%, var(--surface));"><div class="stat-label" style="font-size:10.5px;">Saldadas</div><div class="stat-value tabular" style="font-size:15px;color:var(--income-ink);">' + money(saldado) + "</div></div></div>";
    }
    if (state.filter === "reembolso") {
      ...
      return '<div class="stat-grid" style="grid-template-columns:1fr 1fr 1fr;margin-bottom:14px;"><div class="card stat-tile" style="padding:11px 10px;background:var(--surface);border:1px solid var(--border);"><div class="stat-label" style="font-size:10.5px;">Gasto original</div><div class="stat-value tabular" style="font-size:15px;">' + money(bruto) + '</div></div><div class="card stat-tile" style="padding:11px 10px;background:color-mix(in srgb, var(--cat-mint-fill) 30%, var(--surface));"><div class="stat-label" style="font-size:10.5px;">Ya lleg\xF3</div><div class="stat-value tabular" style="font-size:15px;color:var(--cat-mint-ink);">' + money(yaLlego) + '</div></div><div class="card stat-tile" style="padding:11px 10px;background:color-mix(in srgb, var(--invest-fill) 16%, var(--surface));"><div class="stat-label" style="font-size:10.5px;">Pendiente</div><div class="stat-value tabular" style="font-size:15px;color:var(--invest-ink);">' + money(pendiente) + "</div></div></div>";
    }
    if (state.filter === "pendientes") {
      const relevantes = base.filter((t) => t.estado === "pendiente");
      const total = relevantes.reduce((s, t) => s + t.monto, 0);
      return '<div class="card" style="padding:12px 14px;margin-bottom:14px;display:flex;align-items:center;justify-content:space-between;"><span class="muted" style="font-size:12.5px;">' + relevantes.length + " transacci\xF3n" + (relevantes.length === 1 ? "" : "es") + ' sin clasificar</span><span class="tabular" style="font-weight:500;font-size:14px;">' + money(total) + "</span></div>";
    }
    return "";
  }
  __name(renderFilterSummary, "renderFilterSummary");
  function renderTxItem(t) {
    ...
    else if (isMulti) leftLabel = cats.length + " categor\xEDas";
  ...
  function renderTxResultsInner() {
    ...
    let groupsHtml = "";
    if (list.length === 0) {
      groupsHtml = '<div class="empty-state">' + ICONS.inbox + "<div>No hay transacciones que calcen con esta b\xFAsqueda o filtro.</div></div>";
    } else {
      const groups = [];
      ...
      groupsHtml = groups.map(
        (g) => '<div class="day-group"><div class="day-label">' + capitalizeFirst(dayLabel(g.fecha)) + '</div><div class="tx-list">' + g.items.map(renderTxItem).join("") + "</div></div>"
      ).join("");
    }
    return renderFilterSummary() + groupsHtml;
  }
  __name(renderTxResultsInner, "renderTxResultsInner");
  function renderTxResultsOnly() {
    const el = document.getElementById("tx-results");
    if (el) el.innerHTML = renderTxResultsInner();
  }
  __name(renderTxResultsOnly, "renderTxResultsOnly");
  function renderTransactionsView() {
    ...
    if (state.categoryFilter) {
      ...
      const pillLabel = catInfo(state.categoryFilter).nombre + (periodoLabel ? " \xB7 " + periodoLabel : "");
    ...
    const searchRow = '<div class="search-row"><div class="search-field"><span class="search-icon">' + ICONS.search + '</span><input type="text" class="search-input" id="tx-search-input" placeholder="Buscar por comercio, ej: uber" value="' + (state.searchQuery || "").replace(/"/g, "&quot;") + '"><button class="search-clear" id="tx-search-clear" data-clear-search aria-label="Borrar b\xFAsqueda" ' + (state.searchQuery ? "" : "hidden") + ">" + ICONS.close + '</button></div><button class="filter-open-btn' + (advCount ? " active" : "") + '" data-open-filters aria-label="Filtros">' + ICONS.filterFunnel + (advCount ? '<span class="filter-badge">' + advCount + "</span>" : "") + "</button></div>";
    let sueldoBanner = "";
  ...
  function renderCuotasProximasBanner() {
    ...
    const filas = proximas.slice(0, maxFilas).map(
      (t) => '<div class="cuota-proxima-row"><span>' + esc(t.comercio) + " \xB7 cuota " + t.cuotaNumero + "/" + t.cuotaTotal + '<span class="cuota-proxima-fecha"> \xB7 ' + capitalizeFirst(dayLabel(t.fecha)) + '</span></span><span class="tabular">' + money(t.monto) + "</span></div>"
    ).join("");
  ...
  __name(openSalarySuggestionSheet, "openSalarySuggestionSheet");
  ...
  __name(resetTabToDefault, "resetTabToDefault");
  function capturePeekHTML(frame) {
  ...
  __name(capturePeekHTML, "capturePeekHTML");
  phone.addEventListener("click", function(e) {
    ...
    if (reloadEmailImportBtn) {
      state.emailImportLoaded = false;
      state.emailImportError = null;
    ...
    if (suppressNextSubtabClick) {
      setSuppressNextSubtabClick(false);
      return;
    }
    ...
    if (dismissSueldo) {
      state.salaryBannerDismissedMonth = todayISO().slice(0, 7);
      renderTransactionsView();
      return;
    }
    const confirmSueldo = e.target.closest("[data-confirm-salary-suggestion]");
    if (confirmSueldo) {
      openSalarySuggestionSheet(confirmSueldo.getAttribute("data-confirm-salary-suggestion"));
      return;
    }
    ...
    if (subBtn) {
      state.summarySub = subBtn.getAttribute("data-summary-sub");
      renderSummaryView();
      return;
    }
      ...
      if (group === "division-tipo" && state.shareDraft) {
        ...
        if (t2) {
          if (val === "iguales") {
            ...
            const base = computeShareAmounts(t2.monto, { ...d, divisionTipo: tipoAnterior });
            d.participantesIncluidos.forEach((id) => {
              d.customValues[id] = val === "pct" ? String(t2.monto ? Math.round((base[id] || 0) / t2.monto * 1e3) / 10 : 0) : String(base[id] || 0);
            });
          }
        }
        renderSheet();
    ...
    if (paidBtn) {
      ...
      if (t && t.porCobrar[idx]) {
        ...
        if (allCollected(t)) toast("\xA1Ya te pagaron todo!");
        renderSheet();
        renderIfListVisible();
      }
      return;
    ...
    if (saveDraftBtn && !saveDraftBtn.disabled) {
      ...
      if (tx && grupoIdOrigen) {
        ...
        renderSheet();
        toast("Transacci\xF3n agregada \u2014 completa el reparto abajo");
      }
      return;
    }
    ...
    if (saveBudget) {
      ...
      if (meta !== null && meta > 0) {
        ...
        toast("Pon una meta mensual v\xE1lida");
      }
      return;
    }
    ...
    if (saveBudgetTotal) {
      ...
      if (v !== null && v > 0) {
        ...
        toast("Pon un presupuesto total v\xE1lido");
      }
      return;
    }
    ...
    if (saveMetasGasto) {
      ...
      if (fijo !== null && fijo >= 0 && variable !== null && variable >= 0) {
        ...
        toast("Pon valores v\xE1lidos para Fijo y Variable");
      }
      return;
    }
    ...
    if (saveDatosTransferencia) {
      ...
      Object.keys(TRANSFER_INFO).forEach((k) => {
        TRANSFER_INFO[k] = (TRANSFER_INFO[k] || "").trim();
      });
      state.editingTransferInfo = false;
    ...
    if (copyChargeBtn) {
      ...
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(txt).then(function() {
          toast(transferInfoComplete() ? "Copiado \u2014 listo para pegar en WhatsApp" : "Copiado \u2014 agrega tus datos de transferencia en Men\xFA > Mi cuenta para incluirlos");
        }).catch(function() {
          toast("No se pudo copiar");
        });
      } else {
    ...
    if (saveGoalBtn) {
      ...
      if (nombre && objetivoValido && aporteValido) {
        const montoObjetivo = objetivoRaw !== "" && objetivoVal !== null ? Math.round(objetivoVal) : void 0;
        const aporteMensualMeta = aporteRaw !== "" && aporteVal !== null ? Math.round(aporteVal) : void 0;
        if (id === "nueva") {
          ...
          if (meta) {
            ...
            meta.aporteMensualMeta = aporteMensualMeta;
            meta.plazo = state.goalDraft.plazo || null;
            meta.comision = comisionFinal;
            meta.startMonth = startMonth;
            meta.startingAmount = startingAmount;
          }
        ...
        toast("Completa el nombre y, si escribiste objetivo o aporte, que sean montos v\xE1lidos");
      }
      return;
    }
    ...
    if (gotoCreateGoalBtn) {
      ...
      if (platId) {
        ...
        state.goalDraft = { nombre: "", montoObjetivo: "", aporteMensualMeta: "", aportadoInicial: "", mesInicio: todayISO().slice(0, 7), plazo: "", comision: "" };
        toast("Crea tu meta y vuelve a clasificar esta transacci\xF3n");
      } else {
        toast("Primero agrega una plataforma de inversi\xF3n");
      }
      render();
      return;
    }
    ...
    if (deletePlatformBtn) {
      ...
      if (isCategoryInUse(id) || isCategoryInUse(generalCatIdFor(id))) {
        toast("No puedes eliminar una plataforma con transacciones");
        return;
      }
      if (goalsForPlatform(id).length > 0) {
    ...
    if (saveNewPlatformBtn) {
      ...
      if (valor === null || valor < 0) {
        toast("Pon un valor v\xE1lido (o d\xE9jalo en 0)");
        return;
      }
      ...
      if (t) {
        if (act === "confirmar") {
          if (t.categorias.length === 0) {
            toast("Primero elige una categor\xEDa");
          } else {
            t.estado = "confirmado";
          ...
          if (hasReceivableType(t, "reembolso")) {
            ...
            toast("Se quit\xF3 el reembolso pendiente");
          } else {
            t.estado = "por_cobrar";
            state.splitCollectMode[t.id] = true;
          ...
          if (t.estado === "no_es_gasto") {
            t.estado = t.categorias.length > 0 ? "confirmado" : "pendiente";
            toast('Ya no est\xE1 marcado como "' + etiqueta + '"');
          } else {
            t.categorias = [];
            t.estado = "no_es_gasto";
            toast("Marcado como " + etiqueta);
          }
    ...
    if (pickGastoReembolsoBtn && state.linkFlow && state.linkFlow.mode === "fromIngreso") {
      ...
      if (applyUnexpectedReimbursement(gastoTxId, incomeTxId)) {
        state.linkFlow = null;
        toast("Reembolso aplicado \u2014 el gasto se descont\xF3 en su categor\xEDa");
    ...
    if (openReceiptBtn) {
      openReceiptFlow(openReceiptBtn.getAttribute("data-open-receipt"));
      return;
    }
    ...
    if (notifToggleBtn) {
      if (state.notifSubscribed) disableNotifications();
    ...
    if (groupCreateOpenBtn) {
      ...
      state.groupDraft = { nombre: "", icono: "\u{1F465}" };
      renderGroupsView();
      return;
    }
    ...
    if (groupAddParticipantConfirmBtn) {
      ...
      if (nombre) {
        addParticipantWithoutAccount(gid, nombre, "peach").then(function(p) {
          state.addingParticipant = false;
          toast(p ? p.nombre + " se agreg\xF3 al grupo" : "No se pudo agregar \u2014 revisa tu conexi\xF3n");
          renderGroupsView();
        });
      }
      return;
    ...
    if (openEditParticipantBtn) {
      ...
      const p = GROUP_PARTICIPANTS.find((p2) => p2.id === pid);
    ...
    if (saveEditParticipantBtn) {
      ...
      if (nombre) {
        editGroupParticipant(pid, nombre).then(function(res) {
          state.editingParticipantId = null;
          toast(res.ok ? "Nombre actualizado" : "No se pudo editar \u2014 " + (res.error ? res.error.message : "revisa tu conexi\xF3n"));
          renderGroupsView();
        });
      }
      return;
    ...
    if (confirmDeleteParticipantBtn) {
      ...
      deleteGroupParticipant(pid).then(function(res) {
        state.confirmDeleteParticipantId = null;
        toast(res.ok ? "Participante eliminado" : "No se pudo eliminar \u2014 " + (res.error ? res.error.message : "revisa tu conexi\xF3n"));
        renderGroupsView();
      });
      return;
    }
    ...
    if (markTransferBtn) {
      ...
      registerPaidBalance(gid, fromId, toId, parseInt(montoStr, 10)).then(function(ok) {
        toast(ok ? "Transferencia registrada" : "No se pudo registrar la transferencia \u2014 revisa tu conexi\xF3n");
        renderGroupsView();
      });
      return;
    }
    ...
    if (askDeleteGroupBtn) {
      state.confirmDeleteGroupId = askDeleteGroupBtn.getAttribute("data-ask-delete-group");
      renderGroupsView();
      return;
    }
    ...
    if (saveCatBtn) {
      ...
      const d = state.catDraft;
      if (!d.nombre.trim()) {
        toast("Ponle un nombre a la categor\xEDa");
        return;
      }
      if (idAttr === "nueva") {
    ...
    if (paymentMethodDraftIconBtn) {
      state.medioDraft.icon = paymentMethodDraftIconBtn.getAttribute("data-payment-method-draft-icon");
      renderMenuView();
      return;
    }
    ...
    if (reconciliarReset) {
      state.reconciliar = {
        ...
        movimientos: [],
        pagosTarjeta: null,
        disponibles: state.reconciliar.disponibles,
        usandoId: null,
        passwordDraft: "",
        errorPassword: null,
        archivoBuffer: null,
    ...
    if (reconciliarAgregar) {
      ...
      const normales = state.reconciliar.movimientos.filter(function(m2) {
        return m2.esEspecial !== "pago_tarjeta" && m2.esEspecial !== "pago_recibido";
      });
    ...
    if (reconciliarAgregarTodo) {
      const normales = state.reconciliar.movimientos.filter(function(m) {
        return m.esEspecial !== "pago_tarjeta" && m.esEspecial !== "pago_recibido";
      });
      let n = 0;
  ...
  phone.addEventListener("change", function(e) {
    ...
    if (sel) {
      ...
      if (t && t.sharedByOthers && sel.value) {
        classifySharedExpenseFromOthers(t.id, sel.value).catch(function(err) {
          console.error("Pitucas sin lucas \u2014 error clasificando el gasto ajeno:", err);
        });
        state.categoryEditMode[t.id] = false;
    ...
    if (draftMedio && state.draftTx) {
      if (draftMedio.value === "__nuevo_medio__") {
        ...
        renderSheet();
        setTimeout(() => {
          const el = document.querySelector('[data-new-payment-method-field="nombre"]');
          if (el) el.focus();
        }, 50);
        return;
      }
    ...
    if (txPaymentMethodSelect) {
      ...
      if (t) {
        t.medio = txPaymentMethodSelect.value;
        renderIfListVisible();
      }
      return;
    }
    ...
    if (receiptFileInput && state.boleta) {
      ...
      if (file) {
        ...
        leerBoletaConOCR(file).then(function(resultado) {
          ...
          state.boleta.items = resultado.items.map(function(it) {
            return { id: nextReceiptItemId(), nombre: it.nombre || "", monto: Math.round(it.monto) || 0 };
          });
  ...
  phone.addEventListener("input", function(e) {
    ...
    if (txFieldFecha && txFieldFecha.value) {
      ...
      if (tx) {
        ...
        if (metaEl) metaEl.textContent = dayLabel(tx.fecha) + " \xB7 " + tx.hora + " \xB7 " + paymentMethodInfo(tx.medio).nombre;
      }
      return;
    }
    ...
    if (txFieldNota) {
      ...
      if (tx) {
        ...
        const notaEl = document.querySelector(".sheet-top [data-note-echo]");
        if (notaEl) {
    ...
    if (newPaymentMethodField) {
      ...
      const saveNewBtn = document.querySelector("[data-save-new-payment-method]");
    ...
    if (editParticipantName) {
      ...
      const saveBtn = document.querySelector("[data-save-edit-participant]");
    ...
    if (manualTransferMonto && state.manualTransferDraft) {
      ...
      const confirmBtn = document.querySelector("[data-manual-transfer-confirm]");
      if (confirmBtn) {
        const d = state.manualTransferDraft;
  ...
  phone.addEventListener("focusout", function(e) {
    ...
    const planBaseInput = e.target.closest("[data-plan-base-input]");
    if (planBaseInput) {
      planBaseInput.value = moneyPlain(PLANNER.base);
      return;
    }
  ...
  phone.addEventListener("pointermove", function(e) {
    ...
    if (!edgeSwipe.dragging) {
      ...
      const rootEl2 = document.getElementById("view-root");
      if (rootEl2) rootEl2.style.transition = "none";
    }
    e.preventDefault();
  ...
  __name(endEdgeSwipe, "endEdgeSwipe");
  phone.addEventListener("pointerup", endEdgeSwipe);
  phone.addEventListener("pointercancel", endEdgeSwipe);
  var SHEET_DISMISS_THRESHOLD_PX = 120;
  var SHEET_DISMISS_VELOCITY_PXMS = 0.6;
  var sheetDrag = null;
  phone.addEventListener("pointerdown", function(e) {
    if (e.button != null && e.button !== 0) return;
  ...
  phone.addEventListener("pointermove", function(e) {
    ...
    if (!sheetDrag.dragging) {
      ...
      const sheetEl2 = document.getElementById("sheet");
      if (sheetEl2) sheetEl2.style.transition = "none";
    }
    e.preventDefault();
  ...
  __name(renderIfListVisible, "renderIfListVisible");
  ...
  __name(boletaWorkerConfigured, "boletaWorkerConfigured");
  var sb = null;
  function setSb(v) {
    sb = v;
  }
  ...
  var saveTimer = null;
  var lastSavedBlobJSON = null;
  function emptyAppStateBlob() {
    const ym = todayISO().slice(0, 7);
    const catsBase = {};
    Object.keys(CATEGORY_SEED_DEFAULTS).forEach(function(k) {
      catsBase[k] = Object.assign({}, CATEGORY_SEED_DEFAULTS[k]);
    });
    const monthLabelObj = {};
  ...
  function buildFullStateBlob() {
    return deepClone({
      ...
      transacciones: TRANSACTIONS.filter((t) => !t.sharedByOthers),
      categorias: CATEGORIES,
      mediosPago: PAYMENT_METHODS,
      presupuestos: BUDGETS,
      monthlyBudgetTotal,
      metasGastoPct: SPENDING_GOAL_PCT,
      datosTransferencia: TRANSFER_INFO,
      metasInversion: INVESTMENT_GOALS,
      plataformas: PLATFORM_DATA,
  ...
  function applyStateBlob(blob) {
    ...
    const NOTA_IMPORT_LEGADA = "Importado autom\xE1ticamente desde tu correo";
    TRANSACTIONS.forEach(function(t) {
    ...
    Object.keys(MONTH_LABEL).forEach(function(k) {
      delete MONTH_LABEL[k];
    });
    const ml = blob.monthLabel || {};
    MONTHS.forEach(function(m) {
      MONTH_LABEL[m] = ml[m] || monthLabelFor(m);
    });
    ...
    state.categoryFilter = null;
    state.categoryFilterMonth = null;
    state.evolutionSelectedMonth = null;
    state.editingPlatformId = null;
    state.creatingPlatform = false;
    state.confirmDeletePlatformId = null;
    state.confirmArchivePlatformId = null;
  ...
  __name(applyStateBlob, "applyStateBlob");
  ...
  __name(showAuthError, "showAuthError");
  function clearAuthError() {
    const el = document.getElementById("auth-error");
    el.hidden = true;
    el.textContent = "";
  }
  __name(clearAuthError, "clearAuthError");
  ...
  __name(showAuthHint, "showAuthHint");
  ...
  __name(clearAuthHint, "clearAuthHint");
  function setAuthLoading(isLoading) {
    const btn = document.getElementById("auth-submit-btn");
    btn.disabled = isLoading;
    btn.textContent = isLoading ? "Un momento\u2026" : authMode === "login" ? "Iniciar sesi\xF3n" : "Crear cuenta";
  }
  __name(setAuthLoading, "setAuthLoading");
  ...
  __name(showAuthChecking, "showAuthChecking");
  ...
  function translateAuthError(err) {
    ...
    if (/Invalid login credentials/i.test(msg)) return "Correo o contrase\xF1a incorrectos.";
    if (/User already registered/i.test(msg)) return "Ya existe una cuenta con ese correo \u2014 intenta iniciar sesi\xF3n.";
  ...
  async function handleAuthSubmit() {
    ...
    clearAuthHint();
    if (!email || !password) {
      showAuthError("Completa tu correo y tu contrase\xF1a.");
      return;
    }
    if (password.length < 6) {
      showAuthError("La contrase\xF1a debe tener al menos 6 caracteres.");
      return;
    }
    setAuthLoading(true);
    try {
      if (authMode === "login") {
        ...
        if (!data.session) {
          ...
          showAuthHint("Cuenta creada \u2014 revisa tu correo (" + email + ") para confirmarla, y despu\xE9s inicia sesi\xF3n aqu\xED.", true);
          return;
        }
      }
    } catch (err) {
  ...
  async function onAuthenticated(user) {
    ...
    try {
      ...
      if (!memberRows || !memberRows.length) {
        throw new Error("Tu cuenta todav\xEDa no tiene un hogar asociado. Cierra sesi\xF3n, espera unos segundos y vuelve a entrar \u2014 se crea autom\xE1ticamente al registrarte.");
      }
      ...
      try {
        ...
        console.error("Pitucas sin lucas \u2014 error precargando el c\xF3digo de importaci\xF3n:", e);
      }
  ...
  __name(onAuthenticated, "onAuthenticated");
  ...
  __name(resetToLoggedOutState, "resetToLoggedOutState");
  ...
  function initSupabaseAuth() {
    ...
    if (sb) {
      ...
      sb.auth.getSession().then(function(res) {
        ...
        console.error("Pitucas sin lucas \u2014 error obteniendo la sesi\xF3n:", err);
        setAuthLoading(false);
        showAuthForm();
        showAuthError("No se pudo conectar con el servidor. Revisa tu internet y recarga la p\xE1gina.");
      });
    } else {
      setAuthLoading(false);
      showAuthForm();
      showAuthError("No se pudo cargar la librer\xEDa de conexi\xF3n (revisa tu internet y recarga la p\xE1gina).");
    }
  }
  __name(initSupabaseAuth, "initSupabaseAuth");
  ...
  function avatarHtml(nombre, color, size) {
    ...
    return '<span class="avatar-circle" style="width:' + s + "px;height:" + s + "px;font-size:" + Math.round(s * 0.42) + "px;--fill:var(--cat-" + c + "-fill);--ink:var(--cat-" + c + '-ink);">' + inicial + "</span>";
  }
  __name(avatarHtml, "avatarHtml");
  function groupScreenHead(title) {
  ...
  __name(groupScreenHead, "groupScreenHead");
  function myParticipantInGroup(groupId) {
    if (!currentUser) return null;
    return GROUP_PARTICIPANTS.find((p) => p.grupo_id === groupId && p.user_id === currentUser.id) || null;
  }
  __name(myParticipantInGroup, "myParticipantInGroup");
  function defaultShareDraft(txId, groupId) {
  ...
  function renderSplitDraftForm(tx, d) {
    ...
    const modalidadSeg = segmentedHtml("division-tipo", [
      { id: "iguales", label: "Por partes" },
      { id: "pct", label: "Por %" },
      { id: "montos", label: "Monto fijo" }
    ], d.divisionTipo);
    const groupSelectHtml = !d.groupId ? "" : '<label class="draft-label">Grupo</label><select data-share-group>' + GROUPS.map((g) => '<option value="' + g.id + '" ' + (g.id === d.groupId ? "selected" : "") + ">" + esc(g.icono) + " " + esc(g.nombre) + "</option>").join("") + "</select>";
    const rows = participantes.map((p) => {
      ...
      return '<div class="split-row" style="align-items:center;"><input type="checkbox" data-share-include="' + p.id + '" ' + (incluido ? "checked" : "") + ' style="width:18px;height:18px;flex-shrink:0;margin-right:8px;">' + avatarHtml(p.nombre, p.color, 24) + '<span style="flex:1;margin-left:8px;">' + esc(p.nombre) + "</span>" + valueField + (isContact ? '<button type="button" class="rm-btn" data-open-edit-contact="' + p.id + '" aria-label="Editar a ' + esc(p.nombre) + '">' + ICONS.edit + '</button><button type="button" class="rm-btn" data-ask-delete-contact="' + p.id + '" aria-label="Quitar a ' + esc(p.nombre) + '">' + ICONS.trash + "</button>" : "") + "</div>";
    }).join("");
  ...
  __name(renderGroupsView, "renderGroupsView");
  function renderGroupsList() {
    const cont = document.getElementById("view-root");
    if (state.joiningGroup) {
      cont.innerHTML = renderJoinGroupForm();
      return;
    }
    if (state.creatingGroup) {
      cont.innerHTML = renderCreateGroupForm();
      return;
    }
    if (!GROUPS.length) {
    ...
    const cards = GROUPS.map((g) => {
      ...
      return '<li><button class="menu-list-item" data-group-open="' + g.id + '"><span class="menu-item-icon" style="font-size:20px;">' + esc(g.icono || "\u{1F465}") + '</span><span class="menu-item-label">' + esc(g.nombre) + '<span class="menu-item-sub">' + n + " participante" + (n === 1 ? "" : "s") + " \xB7 " + balanceTxt + '</span></span><span class="menu-item-chev">' + ICONS.chevL + "</span></button></li>";
    }).join("");
    cont.innerHTML = '<ul class="menu-list">' + cards + '</ul><div style="display:flex;gap:10px;margin-top:16px;"><button class="save-tx-btn" style="flex:1;" data-group-create-open>' + ICONS.plus + ' Crear grupo</button><button class="save-tx-btn" style="flex:1;background:var(--surface-sunken);color:var(--text);" data-group-join-open>Unirme con un c\xF3digo</button></div>';
  }
  __name(renderGroupsList, "renderGroupsList");
  function renderCreateGroupForm() {
    const d = state.groupDraft;
    const iconos = ["\u{1F465}", "\u{1F3E0}", "\u2764\uFE0F", "\u2708\uFE0F", "\u{1F389}", "\u{1F393}"];
    return groupScreenHead("Crear grupo") + '<div class="sheet-block card" style="padding:16px;"><label class="draft-label">Nombre</label><input type="text" class="draft-input" data-group-draft-field="nombre" value="' + esc(d.nombre) + '" placeholder="Ej: Depto, Familia, Viaje a Chilo\xE9"><label class="draft-label" style="margin-top:12px;">\xCDcono</label><div class="icon-picker emoji-icon-picker">' + iconos.map((em) => '<button type="button" data-group-draft-icon="' + em + '" class="' + (d.icono === em ? "active" : "") + '">' + em + "</button>").join("") + '</div><div class="platform-hint muted" style="margin-top:10px;">Quedas t\xFA como primer participante -- despu\xE9s puedes invitar a alguien m\xE1s con cuenta, o agregar a alguien sin cuenta que t\xFA administres.</div><div style="display:flex;gap:10px;margin-top:16px;"><button class="save-tx-btn" style="background:var(--surface-sunken);color:var(--text);flex:1;" data-group-create-cancel>Cancelar</button><button class="save-tx-btn" style="flex:1;" data-group-create-confirm>Crear</button></div></div>';
  }
  ...
  function renderGroupSubtabsInner() {
    return Object.keys(GROUP_TABS_META).map(
      (id) => '<button class="subtab ' + (state.groupDetailTab === id ? "active" : "") + '" data-group-tab="' + id + '">' + GROUP_TABS_META[id].label + "</button>"
    ).join("");
  }
  __name(renderGroupSubtabsInner, "renderGroupSubtabsInner");
  function categoryForSharedExpense(g, groupId) {
  ...
  function renderGroupExpenseDetailCard(gasto, groupId) {
    ...
    return '<div class="sheet-block card" style="padding:16px;margin-top:10px;"><div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">' + catAvatarHtml(ci) + '<span style="flex:1;"><span class="sheet-block-title" style="margin:0;display:block;">' + esc(gasto.descripcion) + '</span><span class="muted" style="font-size:12px;">' + dayLabel(gasto.fecha) + '</span></span><span class="tabular" style="font-weight:700;">' + money(gasto.monto) + '</span></div><div class="muted" style="font-size:12.5px;margin-bottom:8px;">Pag\xF3 ' + esc(pagador ? pagador.nombre : "?") + "</div>" + (gasto.reparto || []).map((r) => {
      const p = participantes.find((pp) => pp.id === r.participante_id);
      return '<div class="split-row" style="align-items:center;">' + avatarHtml(p ? p.nombre : "?", p ? p.color : "neutral", 24) + '<span style="flex:1;margin-left:8px;">' + esc(p ? p.nombre : "?") + '</span><span class="tabular muted">' + money(r.monto) + "</span></div>";
    }).join("") + '<button class="save-tx-btn" style="background:var(--surface-sunken);color:var(--text);width:100%;margin-top:12px;" data-group-expense-close>Cerrar</button></div>';
  }
  __name(renderGroupExpenseDetailCard, "renderGroupExpenseDetailCard");
  function renderGroupGastosTab(groupId) {
    const participantes = participantsOfGroup(groupId);
    ...
    return '<div class="card stat-tile" style="padding:16px;text-align:center;margin-bottom:14px;"><div class="stat-label">Total gastado por el grupo</div><div class="stat-value tabular" style="font-size:24px;">' + money(total) + '</div></div><div class="sheet-block card" style="padding:16px;">' + (gastos.length ? '<div class="tx-list" style="box-shadow:none;border:none;">' + gastos.map((gc) => {
      ...
      return '<div class="tx-item" data-group-expense-open="' + gc.id + '">' + catAvatarHtml(ci) + '<span class="tx-info"><span class="tx-name">' + esc(gc.descripcion) + '</span><span class="tx-sub">' + dayLabel(gc.fecha) + " \xB7 pag\xF3 " + esc(pagador ? pagador.nombre : "?") + " \xB7 entre " + esc(entre) + '</span></span><span class="tx-right"><span class="tx-amount tabular">' + money(gc.monto) + "</span></span></div>";
    }).join("") + "</div>" : '<p class="muted" style="padding:8px 0;">Todav\xEDa no hay gastos en este grupo.</p>') + '<button class="split-add" data-group-create-expense-open="' + groupId + '">' + ICONS.plus + " Agregar un gasto</button></div>" + (abierto ? renderGroupExpenseDetailCard(abierto, groupId) : "");
  }
  __name(renderGroupGastosTab, "renderGroupGastosTab");
  function renderGroupBalancesTab(groupId) {
    ...
    const personSection = '<div class="sheet-block card" style="padding:16px;margin-bottom:14px;"><div class="sheet-block-title">Saldo por persona</div>' + balances.map((s) => {
      ...
      if (state.editingParticipantId === s.participantId) {
        return '<div class="split-row" style="align-items:center;gap:6px;">' + avatarHtml(s.nombre, s.color) + '<input type="text" class="draft-input" style="flex:1;margin-left:10px;" data-edit-participant-name value="' + state.editParticipantDraft.replace(/"/g, "&quot;") + '" autofocus><button class="chip" data-cancel-edit-participant>Cancelar</button><button class="chip" style="background:var(--accent-soft);color:var(--accent-ink);" data-save-edit-participant="' + s.participantId + '" ' + (state.editParticipantDraft.trim() ? "" : "disabled") + ">Guardar</button></div>";
      }
      if (state.confirmDeleteParticipantId === s.participantId) {
        return '<div class="split-row" style="align-items:center;flex-wrap:wrap;gap:6px;"><span style="flex:1 1 100%;font-size:12.5px;" class="muted">\xBFEliminar a <b>' + esc(s.nombre) + '</b> de este grupo?</span><button class="chip" data-cancel-delete-participant>Cancelar</button><button class="chip" style="background:var(--cat-pink-fill);color:var(--expense-ink);" data-confirm-delete-participant="' + s.participantId + '">S\xED, eliminar</button></div>';
      }
      return '<div class="split-row" style="align-items:center;">' + avatarHtml(s.nombre, s.color) + '<span style="flex:1;margin-left:10px;">' + esc(s.nombre) + (isMe ? " (t\xFA)" : "") + '</span><span class="tabular" style="color:' + (s.balance > 0 ? "var(--income-ink)" : s.balance < 0 ? "var(--expense-ink)" : "var(--text-secondary)") + ';font-weight:600;">' + (s.balance === 0 ? "Al d\xEDa" : (s.balance > 0 ? isMe ? "Te deben " : "Le deben " : isMe ? "Debes " : "Debe ") + money(Math.abs(s.balance))) + "</span>" + (sinCuenta ? '<button class="rm-btn" data-open-edit-participant="' + s.participantId + '" aria-label="Editar a ' + esc(s.nombre) + '">' + ICONS.edit + '</button><button class="rm-btn" data-ask-delete-participant="' + s.participantId + '" aria-label="Eliminar a ' + esc(s.nombre) + '">' + ICONS.trash + "</button>" : "") + "</div>";
    ...
    const transferSection = '<div class="sheet-block card" style="padding:16px;"><div class="sheet-block-title">Reembolsos sugeridos</div>' + (transfers.length ? transfers.map((t) => {
      ...
      return '<div class="split-row" style="align-items:center;' + (involvesMe ? "background:var(--accent-soft);border-radius:10px;padding:6px 8px;margin:2px -8px;" : "") + '">' + avatarHtml(from ? from.nombre : "?", from ? from.color : "neutral", 26) + '<span style="flex:1;margin-left:8px;">' + esc(from ? from.nombre : "?") + " \u2192 " + esc(to ? to.nombre : "?") + '</span><span class="tabular" style="font-weight:600;margin-right:8px;">' + money(t.monto) + '</span><button class="chip" data-mark-transfer-paid="' + groupId + "|" + t.from + "|" + t.to + "|" + Math.round(t.monto) + '">Marcar como pagado</button></div>';
    }).join("") : '<p class="muted" style="padding:8px 0;">Ya est\xE1 todo al d\xEDa -- no hay transferencias pendientes.</p>') + "</div>";
    return personSection + transferSection;
  }
  __name(renderGroupBalancesTab, "renderGroupBalancesTab");
  function renderManualTransferForm(groupId) {
    const d = state.manualTransferDraft;
    const participantes = participantsOfGroup(groupId);
    const ok = d.deId && d.aId && d.deId !== d.aId && d.monto > 0;
    return '<div class="sheet-block card" style="padding:12px;background:var(--surface-sunken);"><label class="draft-label">De</label><select data-manual-transfer-field="deId">' + participantes.map((p) => '<option value="' + p.id + '" ' + (p.id === d.deId ? "selected" : "") + ">" + esc(p.nombre) + "</option>").join("") + '</select><label class="draft-label" style="margin-top:10px;">A</label><select data-manual-transfer-field="aId">' + participantes.map((p) => '<option value="' + p.id + '" ' + (p.id === d.aId ? "selected" : "") + ">" + esc(p.nombre) + "</option>").join("") + '</select><label class="draft-label" style="margin-top:10px;">Monto</label><input type="text" inputmode="decimal" class="draft-input amount tabular" data-manual-transfer-field="monto" value="' + (d.monto || "") + '" placeholder="0"><label class="draft-label" style="margin-top:10px;">Fecha</label><input type="date" class="draft-input" data-manual-transfer-field="fecha" value="' + d.fecha + '">' + (d.deId === d.aId ? '<p class="muted" style="font-size:12px;margin:8px 0 0;">Elige dos personas distintas.</p>' : "") + '<div style="display:flex;gap:10px;margin-top:12px;"><button class="save-tx-btn" style="background:var(--surface);color:var(--text);flex:1;" data-manual-transfer-cancel>Cancelar</button><button class="save-tx-btn" style="flex:1;" data-manual-transfer-confirm="' + groupId + '" ' + (ok ? "" : "disabled") + ">Registrar</button></div></div>";
  }
  __name(renderManualTransferForm, "renderManualTransferForm");
  function renderGroupTransferenciasTab(groupId) {
    const participantes = participantsOfGroup(groupId);
    const nombreDe = /* @__PURE__ */ __name((id) => {
      const p = participantes.find((x) => x.id === id);
      return p ? p.nombre : "?";
    }, "nombreDe");
    const colorDe = /* @__PURE__ */ __name((id) => {
      const p = participantes.find((x) => x.id === id);
      return p ? p.color : "neutral";
    }, "colorDe");
    const historial = PAID_BALANCES.filter((s) => s.grupo_id === groupId).slice().sort((a, b) => b.fecha.localeCompare(a.fecha));
    const historyCard = '<div class="sheet-block card" style="padding:16px;margin-bottom:14px;"><div class="sheet-block-title">Historial de transferencias</div>' + (historial.length ? historial.map((s) => {
      return '<div class="split-row" style="align-items:center;">' + avatarHtml(nombreDe(s.de_participante), colorDe(s.de_participante), 26) + '<span style="flex:1;margin-left:8px;">' + nombreDe(s.de_participante) + " \u2192 " + nombreDe(s.a_participante) + '</span><span class="tabular" style="font-weight:600;margin-right:8px;">' + money(s.monto) + '</span><span class="muted" style="font-size:11.5px;">' + dayLabel(s.fecha) + "</span></div>";
    }).join("") : '<p class="muted" style="padding:8px 0;">Todav\xEDa no hay transferencias registradas.</p>') + "</div>";
    const manualCard = state.showManualTransferForm ? renderManualTransferForm(groupId) : '<button class="split-add" data-manual-transfer-open="' + groupId + '">' + ICONS.plus + " Registrar una transferencia</button>";
    return historyCard + manualCard;
  }
  __name(renderGroupTransferenciasTab, "renderGroupTransferenciasTab");
  function renderGroupDetail(groupId) {
    ...
    const contentHtml = tab === "balances" ? renderGroupBalancesTab(groupId) : tab === "transferencias" ? renderGroupTransferenciasTab(groupId) : renderGroupGastosTab(groupId);
    const inviteBlock = '<div class="card" style="padding:14px 16px;margin-top:12px;"><div class="sheet-block-title" style="margin-bottom:8px;">C\xF3digo de invitaci\xF3n</div><p class="muted" style="margin-bottom:10px;font-size:12px;">Comp\xE1rtelo con quien quieras que se una a "' + esc(g.nombre) + '".</p><div style="display:flex;gap:8px;"><input class="draft-input" readonly value="' + g.invite_code + '" style="font-size:11.5px;"><button class="budget-edit-btn" data-copy-text="' + g.invite_code + '" aria-label="Copiar c\xF3digo de invitaci\xF3n">' + ICONS.copy + "</button></div></div>";
    const canDelete = !!(currentUser && g.creado_por === currentUser.id);
    const deleteBlock = !canDelete ? "" : state.confirmDeleteGroupId === groupId ? '<div class="sheet-block card" style="padding:16px;margin-top:14px;"><p class="muted" style="font-size:12.5px;margin:0 0 10px;">\xBFSeguro que quieres eliminar "' + esc(g.nombre) + '"? Se borran todos sus gastos y saldos, para todos los participantes. No se puede deshacer.</p><div style="display:flex;gap:8px;"><button class="save-tx-btn" style="flex:1;background:var(--surface-sunken);color:var(--text);" data-cancel-delete-group>Cancelar</button><button class="save-tx-btn" style="flex:1;background:var(--cat-pink-fill);color:var(--expense-ink);" data-confirm-delete-group="' + groupId + '">S\xED, eliminar</button></div></div>' : '<button class="split-add" style="color:var(--expense-ink);margin-top:14px;" data-ask-delete-group="' + groupId + '">' + ICONS.trash + " Eliminar grupo</button>";
  ...
  __name(renderGroupDetail, "renderGroupDetail");
  function renderAddParticipantForm(groupId) {
    const d = state.participantDraft;
    return '<div class="sheet-block card" style="padding:12px;margin-top:10px;background:var(--surface-sunken);"><label class="draft-label">Nombre</label><input type="text" class="draft-input" data-participant-draft-field="nombre" value="' + esc(d.nombre) + '" placeholder="Sin cuenta -- la administras t\xFA"><div style="display:flex;gap:10px;margin-top:10px;"><button class="save-tx-btn" style="background:var(--surface);color:var(--text);flex:1;" data-group-add-participant-cancel>Cancelar</button><button class="save-tx-btn" style="flex:1;" data-group-add-participant-confirm="' + groupId + '">Agregar</button></div></div>';
  }
  __name(renderAddParticipantForm, "renderAddParticipantForm");
  ...
  __name(getTx, "getTx");
  var DRAFT_TX_ID = "__draft__";
  function currentEditableTx() {
    return state.creatingNew ? state.draftTx : state.openTxId ? getTx(state.openTxId) : null;
  }
  __name(currentEditableTx, "currentEditableTx");
  function segmentedHtml(name, options, value, disabled) {
    return '<div class="segmented" data-seg="' + name + '">' + options.map(
      (o) => '<button data-seg-val="' + o.id + '" class="' + (value === o.id ? "active" : "") + '" ' + (disabled ? "disabled" : "") + ">" + esc(o.label) + "</button>"
    ).join("") + "</div>";
  }
  __name(segmentedHtml, "segmentedHtml");
  function investOrPlainOptions(tipo, selectedCat) {
    ...
    return Object.keys(CATEGORIES).filter((k) => CATEGORIES[k].tipo === tipo).map((k) => {
      const icon2 = CATEGORIES[k].icon;
      const label = (ICONS[icon2] === void 0 ? icon2 + " " : "") + CATEGORIES[k].nombre;
      return { value: k, label };
    });
  }
  __name(investOrPlainOptions, "investOrPlainOptions");
  function renderCategoryRows(t, allowSplit) {
    ...
    const rows = list.map((c, idx) => {
      ...
      const opts = '<option value="">Sin categor\xEDa</option>' + investOrPlainOptions(t.tipo, c.cat).map(
        (o) => '<option value="' + o.value + '" ' + (c.cat === o.value ? "selected" : "") + ">" + esc(o.label) + "</option>"
      ).join("");
  ...
  function renderChargeSplitBlock(t) {
    ...
    const reembolsoRows = t.porCobrar.map((p, idx) => ({ p, idx })).filter((x) => x.p.tipo === "reembolso").map(({ p, idx }) => {
      ...
      const nameField = p.pagado ? '<span style="flex:1;min-width:0;display:flex;flex-direction:column;gap:2px;"><span class="pend-tipo-tag">Reembolso</span><span class="persona-label" style="font-size:13px;font-weight:600;">' + esc(p.persona || "Sin nombre") + "</span></span>" : '<span style="flex:1;min-width:0;display:flex;flex-direction:column;gap:3px;"><span class="pend-tipo-tag">Reembolso</span><input type="text" class="persona-label" style="width:100%;" data-charge-name="' + idx + '" value="' + esc(p.persona) + '" placeholder="Isapre, seguro\u2026"></span>';
  ...
  function renderDraftCategoryRow(d) {
    ...
    const opts = '<option value="">Sin categor\xEDa</option>' + investOrPlainOptions(d.tipo, chosen).map(
      (o) => '<option value="' + o.value + '" ' + (chosen === o.value ? "selected" : "") + ">" + esc(o.label) + "</option>"
    ).join("");
  ...
  __name(renderInvestGoalEmptyState, "renderInvestGoalEmptyState");
  ...
  __name(pendingCardTitle, "pendingCardTitle");
  function renderQuickActionsBlock(t) {
    ...
    return '<div class="sheet-block card" style="padding:16px;"><div class="sheet-block-title">Acciones r\xE1pidas</div><div class="quick-actions"><button class="action-btn ' + (t.estado === "confirmado" ? "selected" : "") + '" data-action="confirmar" data-tx="' + t.id + '">' + ICONS.checkCircle + ' Confirmar gasto</button><button class="action-btn ' + (tieneCobro ? "selected" : "") + '" data-action="porcobrar_persona" data-tx="' + t.id + '">' + ICONS.users + ' Por cobrar a alguien</button><button class="action-btn ' + (hasReceivableType(t, "reembolso") ? "selected" : "") + '" data-action="porcobrar_reembolso" data-tx="' + t.id + '">' + ICONS.inbox + ' Reembolso pendiente</button><button class="action-btn ' + (t.estado === "no_es_gasto" ? "selected" : "") + '" data-action="noesgasto" data-tx="' + t.id + '">' + ICONS.ban + " No es gasto</button></div></div>" + (t.estado === "por_cobrar" && cardTitle ? '<div class="sheet-block card" style="padding:16px;"><div class="sheet-block-title">' + cardTitle + "</div>" + renderChargeSplitBlock(t) + "</div>" : "");
  }
  __name(renderQuickActionsBlock, "renderQuickActionsBlock");
  function renderIncomeQuickActionsBlock(t) {
    return '<div class="sheet-block card" style="padding:16px;"><div class="sheet-block-title">Acciones r\xE1pidas</div><div class="quick-actions"><button class="action-btn ' + (t.estado === "no_es_gasto" ? "selected" : "") + '" data-action="noesgasto" data-tx="' + t.id + '">' + ICONS.ban + " No es ingreso</button></div></div>";
  }
  __name(renderIncomeQuickActionsBlock, "renderIncomeQuickActionsBlock");
  function renderIncomeNatureBlock(t) {
    ...
    return '<div class="sheet-block card" style="padding:16px;"><div class="sheet-block-title">' + (porClasificar ? "\xBFQu\xE9 tipo de entrada es esta?" : "Tipo de entrada") + "</div>" + (porClasificar ? '<p class="muted" style="font-size:12.5px;margin:0 0 10px;">Esta categor\xEDa puede ser ingreso real o solo un movimiento de tu propia plata (ej. rescatar una inversi\xF3n, vender un activo). Elige para que no infle tu tasa de ahorro por error.</p>' : "") + '<div class="quick-actions"><button class="action-btn ' + (nature === "ingreso" ? "selected" : "") + '" data-action="naturaleza-ingreso" data-tx="' + t.id + '">' + ICONS.trending + ' Es ingreso real</button><button class="action-btn ' + (nature === "movimiento_capital" ? "selected" : "") + '" data-action="naturaleza-capital" data-tx="' + t.id + '">' + ICONS.repeat + " Es movimiento de capital</button></div></div>";
  }
  __name(renderIncomeNatureBlock, "renderIncomeNatureBlock");
  function renderSheetContent(t) {
    const isIncome = t.tipo === "ingreso";
    const isInvest = t.tipo === "inversion";
    const cats = t.categorias;
    const needsClassifying = cats.length === 0 && t.estado === "pendiente";
    const categoriaSection = isInvest && INVESTMENT_GOALS.length === 0 ? renderInvestGoalEmptyState() : needsClassifying ? (t.sharedByOthers ? '<p class="cat-picker-hint">Este es tu parte de un gasto de grupo' + (t.suggestedOriginCategory ? ' que la otra persona anot\xF3 como "' + t.suggestedOriginCategory + '"' : "") + ". Elige tu categor\xEDa y la pr\xF3xima vez que registre algo as\xED se va a clasificar sola.</p>" : '<p class="cat-picker-hint">Todav\xEDa no le has puesto categor\xEDa. Elige una para clasificarla (y luego puedes activar el candado para que se repita sola).</p>') + // Antes acá se mostraba una grilla de chips SIEMPRE ABIERTA (catPickerGrid /
    ...
    renderCategoryRows(t, false) : renderCategoryRows(t, !isInvest);
    ...
    const cuotaBlock = t.tipo !== "gasto" || isInvest ? "" : t.cuotaProyectada ? '<div class="sheet-block card" style="padding:16px;"><div class="cuota-note">' + ICONS.layers + "<span>Esta es la cuota " + t.cuotaNumero + " de " + t.cuotaTotal + " de la compra en <b>" + esc(t.comercio) + "</b>. Se gener\xF3 sola a partir de la cuota 1 y va a dejar de aparecer despu\xE9s de la cuota " + t.cuotaTotal + ".</span></div></div>" : '<div class="sheet-block card" style="padding:16px;"><div class="sheet-block-title">Pago en cuotas</div><div class="cuota-row"><span class="cuota-icon">' + ICONS.layers + '</span><span class="cuota-text">La pagaste en cuotas y quieres verla los pr\xF3ximos meses</span><button class="switch ' + (t.cuotas ? "on" : "") + '" data-toggle-installments="' + t.id + '" aria-label="Pago en cuotas" aria-pressed="' + (t.cuotas ? "true" : "false") + '"></button></div>' + (t.cuotas ? '<div class="cuota-stepper-wrap"><span class="cs-label">N\xFAmero de cuotas</span><div class="stepper"><button data-installments-step="-1" data-tx="' + t.id + '" aria-label="Menos cuotas">' + ICONS.minus + '</button><span class="count tabular">' + t.cuotas.total + '</span><button data-installments-step="1" data-tx="' + t.id + '" aria-label="M\xE1s cuotas">' + ICONS.plus + '</button></div></div><div class="muted" style="font-size:12px;margin-top:6px;">' + money(t.monto) + " al mes \xB7 " + money(t.cuotas.montoTotal) + " en total</div>" : "") + "</div>";
    ...
    const tipoRecurrenciaCard = t.estado === "no_es_gasto" ? "" : isInvest ? '<div class="sheet-block card" style="padding:16px;"><div class="draft-label" style="margin-bottom:7px;">Tipo</div>' + tipoSelector + "</div>" : '<div class="sheet-block card" style="padding:16px;"><div class="draft-label" style="margin-bottom:7px;">Tipo</div>' + tipoSelector + '<div class="draft-label" style="margin:16px 0 7px;">Recurrencia</div>' + recurrenciaSelector + '<p class="cat-picker-hint" style="margin-top:8px;">"Mensual" y "Anual" cuentan como <b>gasto fijo</b> en tus metas de Resumen \xB7 Balance \u2014 "Variable" es todo lo dem\xE1s. No existe una opci\xF3n separada llamada "Fijo".</p></div>';
    return '<div class="sheet-top"><div class="merchant" id="sheet-title-el">' + esc(t.comercio) + '</div><div class="meta">' + dayLabel(t.fecha) + " \xB7 " + t.hora + " \xB7 " + esc(paymentMethodInfo(t.medio).nombre) + '</div><div class="sheet-amount ' + (isIncome ? "pos" : "") + ' tabular">' + (isIncome ? "+" : "") + money(t.monto) + '</div><div class="meta" data-note-echo style="margin-top:6px;' + (t.nota ? "" : "display:none;") + '">' + esc(t.nota || "") + '</div></div><div class="sheet-block card" style="padding:16px;"><div class="sheet-block-title">Nombre, monto y fecha</div><div class="draft-field"><label class="draft-label">Nombre</label><input type="text" class="draft-input" data-tx-field="comercio" data-tx="' + t.id + '" value="' + esc(t.comercio) + '"></div><div class="draft-field"><label class="draft-label">Monto</label><div class="edit-amount-row"><input type="text" inputmode="decimal" class="draft-input tabular" data-tx-field="monto" data-tx="' + t.id + '" value="' + t.monto + '"><span class="edit-amount-echo tabular">' + (isIncome ? "+" : "") + money(t.monto) + '</span></div></div><div class="edit-field-pair"><div class="edit-field-col draft-field"><label class="draft-label">Fecha</label><input type="date" class="draft-input" data-tx-field="fecha" data-tx="' + t.id + '" value="' + t.fecha + '"></div><div class="edit-field-col draft-field"><label class="draft-label">Hora</label><input type="time" class="draft-input" data-tx-field="hora" data-tx="' + t.id + '" value="' + t.hora + '"></div></div><div class="muted edit-day-hint">' + dayLabel(t.fecha) + '</div><div class="draft-field" style="margin:14px 0 0;"><label class="draft-label">Con qu\xE9 pagaste</label><select class="draft-select" data-tx-payment-method-select="' + t.id + '">' + medioOptsExisting + "</select></div></div>" + tipoRecurrenciaCard + cuotaBlock + '<div class="sheet-block card" style="padding:16px;"><div class="sheet-block-title">Categor\xEDa' + (cats.length > 1 ? "s" : "") + "</div>" + categoriaSection + "</div>" + (isInvest ? "" : '<div class="sheet-block card lock-card' + (t.reglaAuto ? " active-rule" : "") + '" style="padding:14px 16px;"><div class="lock-row"><span class="lock-icon">' + ICONS.lock + '</span><span class="lock-text">' + (t.reglaAuto ? "Ya clasificamos siempre as\xED lo de <b>" + esc(t.comercio) + "</b>" : "Clasificar siempre as\xED los gastos de <b>" + esc(t.comercio) + "</b>") + '</span><button class="switch ' + (t.reglaAuto ? "on" : "") + '" data-toggle-lock="' + t.id + '" aria-label="Activar regla autom\xE1tica" aria-pressed="' + (t.reglaAuto ? "true" : "false") + '"></button></div></div>') + (isInvest ? "" : isIncome ? renderIncomeQuickActionsBlock(t) + renderIncomeNatureBlock(t) + (function() {
      ...
      return '<div class="sheet-block card" style="padding:16px;"><div class="sheet-block-title">Cobros y reembolsos</div>' + (vinculo ? '<div class="cobro-banner-done">' + ICONS.checkCircle + "<span>Vinculado a " + (vinculo.persona || "un pendiente") + " \xB7 " + esc(vinculo.comercio) + '</span></div><button class="split-toggle-link" data-unlink-income="' + t.id + '">Quitar v\xEDnculo</button>' : '<p class="muted" style="font-size:12.5px;margin:0 0 10px;">Si este dep\xF3sito corresponde a un cobro o reembolso pendiente, vinc\xFAlalo para tacharlo de la lista.</p><button class="action-btn" data-open-link-income="' + t.id + '">' + ICONS.inbox + " Vincular a un pendiente</button>") + "</div>";
    })() : renderQuickActionsBlock(t)) + renderShareGroupSection(t) + '<div class="sheet-block card" style="padding:16px;"><div class="sheet-block-title">Nota</div><input type="text" class="draft-input nota-input" data-tx-field="nota" data-tx="' + t.id + '" value="' + esc(t.nota || "") + '" placeholder="' + esc(t.importadoEmail ? "Importado autom\xE1ticamente desde tu correo" : "Agregar notas personales") + '"></div>' + importedBlock + (state.confirmDeleteTxId === t.id ? '<div class="sheet-delete-confirm"><p class="muted" style="font-size:12.5px;margin:0 0 10px;">\xBFSeguro que quieres eliminar esta transacci\xF3n? No se puede deshacer.</p><div style="display:flex;gap:8px;"><button class="save-tx-btn" style="flex:1;background:var(--surface-sunken);color:var(--text);" data-cancel-delete-tx="' + t.id + '">Cancelar</button><button class="save-tx-btn" style="flex:1;background:var(--cat-pink-fill);color:var(--expense-ink);" data-confirm-delete-tx="' + t.id + '">S\xED, eliminar</button></div></div>' : '<div class="sheet-bottom-actions"><button class="sheet-delete-btn" data-ask-delete-tx="' + t.id + '" aria-label="Eliminar transacci\xF3n">' + ICONS.trash + '</button><button class="sheet-done-btn" data-close-sheet-done>' + ICONS.check + " Listo</button></div>");
  }
  ...
  __name(openSheetOverlay, "openSheetOverlay");
  function openSheet(txId) {
    ...
    setTimeout(() => {
      const b = document.getElementById("sheet-close-btn");
      if (b) b.focus();
    }, 260);
  }
  __name(openSheet, "openSheet");
  var draftIdCounter = 1;
  var paymentMethodIdCounter = 0;
  function setPaymentMethodIdCounter(v) {
  ...
  function openNewTxSheet(tipoInicial) {
    ...
    document.getElementById("sheet-content").scrollTop = 0;
    setTimeout(() => {
      const el = document.querySelector('[data-draft-field="comercio"]');
      if (el) el.focus();
    }, 260);
  }
  ...
  __name(openFilterSheet, "openFilterSheet");
  function closeSheet() {
    state.openTxId = null;
    state.creatingNew = false;
    state.draftTx = null;
  ...
  function renderLinkFlowContent() {
    ...
    if (lf.mode === "fromPendiente") {
      ...
      const asignadosHtml = linksDeEsteDeposito.length ? '<div class="card" style="padding:12px 16px;margin-bottom:12px;"><div class="sheet-block-title" style="margin-bottom:6px;">Ya asignado a</div>' + linksDeEsteDeposito.map((l) => {
        return '<div class="split-row" style="align-items:center;"><span style="flex:1;">' + (l.persona || "Sin nombre") + " \u2014 " + esc(l.comercio) + '</span><span class="tabular muted" style="margin-right:8px;">' + money(l.montoAsignado) + '</span><button class="rm-btn" data-unassign-income="' + l.expenseTxId + "|" + l.idx + "|" + ingresoTx.id + '" aria-label="Quitar esta asignaci\xF3n">' + ICONS.trash + "</button></div>";
      }).join("") + "</div>" : "";
      const rows = pendientes.map((p) => {
        const montoTxt = p.monto != null ? money(p.restante) + " restante" : "monto por confirmar";
        return '<button class="link-pick-row" data-select-pending="' + p.expenseTxId + "|" + p.idx + '"><span class="link-pick-body"><span class="link-pick-name">' + esc(p.persona || "Sin nombre") + (p.tipo === "reembolso" ? ' <span class="pend-tipo-tag" style="margin-left:4px;">Reembolso</span>' : "") + (p.estado === "parcial" ? ' <span class="pend-tipo-tag" style="margin-left:4px;">Parcial</span>' : "") + '</span><span class="link-pick-sub">' + esc(p.comercio) + " \xB7 " + dayLabel(p.fecha) + '</span></span><span class="link-pick-amt tabular muted">' + montoTxt + "</span></button>";
      }).join("");
      ...
      const gastoRows = gastos.map(
        (t) => '<button class="link-pick-row" data-pick-gasto-reembolso="' + t.id + '"><span class="link-pick-body"><span class="link-pick-name">' + esc(t.comercio) + '</span><span class="link-pick-sub">' + dayLabel(t.fecha) + '</span></span><span class="link-pick-amt tabular muted">' + money(t.monto) + "</span></button>"
      ).join("");
  ...
  __name(openReceiptFlow, "openReceiptFlow");
  function receiptPersonTotals() {
  ...
  __name(receiptPersonTotals, "receiptPersonTotals");
  function receiptTotal() {
    return state.boleta.items.reduce((s, i) => s + i.monto, 0);
  }
  __name(receiptTotal, "receiptTotal");
  function receiptTipAmount() {
    const b = state.boleta;
  ...
  __name(receiptTipAmount, "receiptTipAmount");
  function receiptTotalWithTip() {
    return receiptTotal() + receiptTipAmount();
  }
  __name(receiptTotalWithTip, "receiptTotalWithTip");
  function receiptPersonTotalsWithTip() {
  ...
  __name(receiptPersonTotalsWithTip, "receiptPersonTotalsWithTip");
  function renderReceiptCapture() {
    const b = state.boleta;
    const configurado = boletaWorkerConfigured();
    return '<div class="sheet-top" style="text-align:left;padding:8px 2px 4px;"><div class="merchant" style="font-size:17px;">Boleta de ' + esc(b.comercio || "esta transacci\xF3n") + '</div><div class="meta">S\xE1cale una foto o s\xFAbela desde tu galer\xEDa y la convertimos en una lista de items para repartir.</div></div><div class="boleta-capture-row"><label class="boleta-capture-btn" style="cursor:pointer;">' + ICONS.camera + '<span>Tomar foto</span><input type="file" accept="image/*" capture="environment" data-receipt-file-input style="display:none;"></label><label class="boleta-capture-btn" style="cursor:pointer;">' + ICONS.image + '<span>Elegir de galer\xEDa</span><input type="file" accept="image/*" data-receipt-file-input style="display:none;"></label></div>' + (configurado ? "" : '<div class="file-format-hint">Todav\xEDa falta terminar de configurar el lector de boletas -- mientras tanto puedes agregar los items a mano en el siguiente paso.</div>') + '<button class="split-add" data-receipt-goto="items">' + ICONS.plus + " Prefiero agregar los items a mano</button>";
  }
  __name(renderReceiptCapture, "renderReceiptCapture");
  function renderReceiptProcessing() {
    return '<div class="boleta-processing"><div class="boleta-spinner"></div><span>Leyendo tu boleta\u2026</span></div>';
  }
  __name(renderReceiptProcessing, "renderReceiptProcessing");
  function renderReceiptItems() {
    const b = state.boleta;
    const rows = b.items.map(
      (item, idx) => '<div class="split-row" data-receipt-item-row="' + idx + '"><input type="text" data-receipt-item-name="' + idx + '" value="' + esc(item.nombre) + '" placeholder="Nombre del item"><span class="num-wrap"><input type="text" inputmode="decimal" data-receipt-item-amount="' + idx + '" value="' + item.monto + '"><span>$</span></span><button class="rm-btn" data-receipt-item-remove="' + idx + '">' + ICONS.trash + "</button></div>"
    ).join("");
    const total = receiptTotal();
    const canContinue = b.items.length > 0 && total > 0;
    const quickChips = [10, 15, 20].map(
      (pct) => '<button class="boleta-tip-chip' + (b.propinaUnit === "%" && Number(b.propinaValor) === pct ? " active" : "") + '" data-receipt-tip-quick="' + pct + '">' + pct + "%</button>"
    ).join("");
    const propinaCard = '<div class="card boleta-propina-card"><div class="split-mode-row"><span class="muted" style="font-size:12.5px;">\xBFAgregaste propina?</span><div class="mini-toggle"><button data-receipt-tip-unit="%" class="' + (b.propinaUnit === "%" ? "active" : "") + '">%</button><button data-receipt-tip-unit="$" class="' + (b.propinaUnit === "$" ? "active" : "") + '">$</button></div></div>' + (b.propinaUnit === "%" ? '<div class="boleta-tip-chips">' + quickChips + "</div>" : "") + '<span class="num-wrap"><input type="text" inputmode="decimal" data-receipt-tip-input value="' + b.propinaValor + '" placeholder="' + (b.propinaUnit === "%" ? "Otro %" : "Monto $") + '"><span>' + b.propinaUnit + "</span></span></div>";
    return '<div class="sheet-top" style="text-align:left;padding:8px 2px 4px;"><div class="merchant" style="font-size:17px;">' + esc(b.comercio || "Tu boleta") + '</div><div class="meta">Revisa los items \u2014 puedes editarlos, agregar o borrar antes de repartir.</div></div>' + rows + '<button class="split-add" data-receipt-add-item>' + ICONS.plus + " Agregar item</button>" + propinaCard + '<div id="boleta-totals-summary">' + renderReceiptItemsTotalsSummary() + '</div><button class="save-tx-btn" style="width:100%;margin-top:14px;" data-receipt-goto="asignar" ' + (canContinue ? "" : "disabled") + ">Continuar</button>";
  }
  __name(renderReceiptItems, "renderReceiptItems");
  function renderReceiptItemsTotalsSummary() {
    const total = receiptTotal();
    const propina = receiptTipAmount();
    const totalConPropina = receiptTotalWithTip();
    return propina > 0 ? '<div class="split-remaining"><span>Subtotal (sin propina)</span><span class="tabular muted">' + money(total) + '</span></div><div class="split-remaining"><span>Propina</span><span class="tabular">' + money(propina) + '</span></div><div class="split-remaining"><span>Total con propina</span><span class="tabular" style="font-weight:800;">' + money(totalConPropina) + "</span></div>" : '<div class="split-remaining"><span>Total de la boleta</span><span class="tabular">' + money(total) + "</span></div>";
  }
  __name(renderReceiptItemsTotalsSummary, "renderReceiptItemsTotalsSummary");
  function renderReceiptAssign() {
    const b = state.boleta;
    const personas = receiptPeople();
    const itemBlocks = b.items.map((item) => {
      const asignados = b.asign[item.id] || [];
      const chips = personas.map(
        (p) => '<button class="boleta-person-chip' + (asignados.includes(p) ? " active" : "") + '" data-receipt-toggle-person="' + item.id + "|" + p + '">' + p + "</button>"
      ).join("");
      return '<div class="card boleta-item-block"><div class="boleta-item-head"><span class="boleta-item-name">' + esc(item.nombre) + '</span><span class="boleta-item-amt tabular">' + money(item.monto) + '</span></div><div class="boleta-person-chips">' + chips + "</div>" + (asignados.length === 0 ? '<div class="file-format-hint" style="margin-top:8px;">Sin asignar todav\xEDa.</div>' : "") + "</div>";
    }).join("");
    ...
    return '<div class="sheet-top" style="text-align:left;padding:8px 2px 4px;"><div class="merchant" style="font-size:17px;">\xBFQui\xE9n se comi\xF3 qu\xE9?</div><div class="meta">Toca los nombres de cada item \u2014 si lo comieron entre varios, se divide en partes iguales.</div></div>' + itemBlocks + '<div class="card boleta-totals-card">' + personas.map((p) => receiptTotalRowHtml(p, totalsSin[p] || 0, totals[p] || 0, conPropina, false)).join("") + '</div><button class="save-tx-btn" style="width:100%;margin-top:14px;" data-receipt-goto="resumen" ' + (sinAsignar ? "disabled" : "") + ">Continuar</button>" + (sinAsignar ? '<div class="field-error">Asigna cada item a al menos una persona para continuar.</div>' : "");
  }
  __name(renderReceiptAssign, "renderReceiptAssign");
  function receiptTotalRowHtml(nombre, sinPropina, conPropinaMonto, showBoth, esYo) {
    const amt = showBoth ? '<span class="amt-group"><span class="muted tabular boleta-amt-sin">' + money(sinPropina) + ' sin propina</span><span class="amt tabular">' + money(conPropinaMonto) + "</span></span>" : '<span class="amt tabular">' + money(conPropinaMonto) + "</span>";
    return '<div class="boleta-total-row"><span class="name">' + nombre + (esYo ? " (t\xFA)" : "") + "</span>" + amt + "</div>";
  }
  __name(receiptTotalRowHtml, "receiptTotalRowHtml");
  function renderReceiptSummary() {
    const b = state.boleta;
    const totals = receiptPersonTotalsWithTip();
    const totalsSin = receiptPersonTotals();
    const conPropina = receiptTipAmount() > 0;
    return '<div class="sheet-top" style="text-align:left;padding:8px 2px 4px;"><div class="merchant" style="font-size:17px;">As\xED queda repartido</div><div class="meta">Total de la boleta: ' + money(receiptTotalWithTip()) + (conPropina ? " (incluye propina)" : "") + '</div></div><div class="card boleta-totals-card">' + receiptPeople().map((p) => receiptTotalRowHtml(p, totalsSin[p] || 0, totals[p] || 0, conPropina, p === "Yo")).join("") + '</div><div class="boleta-preview-banner">' + ICONS.sparkle + '<span>Pr\xF3ximamente: vas a poder mandarles un link para que cada uno marque lo que consumi\xF3, sin que tengas que calcular t\xFA.</span></div><button class="save-tx-btn" style="width:100%;margin-top:14px;" data-receipt-save>Guardar reparto en la transacci\xF3n</button>';
  }
  __name(renderReceiptSummary, "renderReceiptSummary");
  function renderReceiptSheetContent() {
  ...
  __name(renderReceiptSheetContent, "renderReceiptSheetContent");
  function saveReceipt() {
    const b = state.boleta;
    const gastoTx = getTx(b.expenseTxId);
    if (!gastoTx) return;
    const totals = receiptPersonTotalsWithTip();
    const nuevoPorCobrar = receiptPeople().filter((p) => p !== "Yo" && totals[p] > 0).map((p) => ({ persona: p, monto: Math.round(totals[p]), pagado: false, tipo: "persona", montoRecibido: null, linkedTxId: null }));
    ...
    toast("Reparto guardado en la transacci\xF3n");
  }
  __name(saveReceipt, "saveReceipt");
  function chipToggle(attrName, id, label, icon2, active) {
    return '<button class="cat-picker-chip" data-' + attrName + '="' + id + '" ' + (active ? 'style="background:var(--accent-soft);border-color:var(--accent);color:var(--accent-ink);"' : "") + ">" + (icon2 ? catIconMarkup(icon2) + " " : "") + esc(label) + "</button>";
  }
  __name(chipToggle, "chipToggle");
  function renderFilterSheetContent() {
    ...
    const medioChips = '<div class="cat-picker-grid">' + Object.keys(PAYMENT_METHODS).filter((k) => k !== SHARED_EXPENSE_PAYMENT_METHOD_ID).map((k) => chipToggle("toggle-filter-medio", k, PAYMENT_METHODS[k].nombre, PAYMENT_METHODS[k].icon, af.medios.includes(k))).join("") + "</div>";
  ...
  function renderNewTxSheetContent(d) {
    const tipoOpts = [{ id: "gasto", label: "Gasto" }, { id: "ingreso", label: "Ingreso" }, { id: "inversion", label: "Inversi\xF3n" }];
    const medioOpts = Object.keys(PAYMENT_METHODS).map((k) => '<option value="' + k + '" ' + (d.medio === k ? "selected" : "") + ">" + esc(PAYMENT_METHODS[k].nombre) + "</option>").join("") + '<option value="__nuevo_medio__">+ Agregar tarjeta o medio nuevo\u2026</option>';
    ...
    (d.tipo === "gasto" ? renderQuickActionsBlock(d) : d.tipo === "ingreso" ? renderIncomeQuickActionsBlock(d) : "") + '<div class="sheet-block card" style="padding:16px;"><div class="sheet-block-title">Medio de pago</div>' + (state.addingPaymentMethod ? "" : '<select class="draft-select" data-draft-field="medio">' + medioOpts + "</select>") + newPaymentMethodForm + '</div><button class="save-tx-btn" data-save-draft="1" ' + (canSave ? "" : "disabled") + ">Guardar transacci\xF3n</button>" + (canSave ? "" : '<div class="field-error">Ponle un nombre de comercio y un monto para poder guardar.</div>');
  }
  __name(renderNewTxSheetContent, "renderNewTxSheetContent");
  ...
  function renderSheet() {
    ...
    if (state.creatingNew) {
      const contentEl2 = document.getElementById("sheet-content");
      contentEl2.innerHTML = renderNewTxSheetContent(state.draftTx);
      document.getElementById("sheet-close-btn").innerHTML = ICONS.close;
      return;
    }
    if (!state.openTxId) {
  ...
  __name(renderSheet, "renderSheet");

  // src/shared-expenses.ts
  function participantsOfGroup(groupId) {
    return GROUP_PARTICIPANTS.filter((p) => p.grupo_id === groupId);
  }
  __name(participantsOfGroup, "participantsOfGroup");
  function expensesOfGroup(groupId) {
    return SHARED_EXPENSES.filter((g) => g.grupo_id === groupId);
  }
  __name(expensesOfGroup, "expensesOfGroup");
  function participantHasHistory(participantId) {
  ...
  __name(participantHasHistory, "participantHasHistory");
  function splitsOfExpense(g) {
    return g.reparto || [];
  }
  __name(splitsOfExpense, "splitsOfExpense");
  function participantIdForUser(groupId, userId) {
    const p = GROUP_PARTICIPANTS.find((x) => x.grupo_id === groupId && x.user_id === userId);
    return p ? p.id : null;
  }
  __name(participantIdForUser, "participantIdForUser");
  ...
  __name(groupBalances, "groupBalances");
  function suggestedTransfers(groupId) {
  ...
  __name(suggestedTransfers, "suggestedTransfers");
  function splitEqually(monto, participantIds) {
  ...
  function splitByShares(total, participantIds, partes) {
    const n = participantIds.length;
    const out = {};
    if (n === 0) return out;
  ...
  __name(splitByShares, "splitByShares");
  ...
  function draftFromExistingSplit(t) {
    ...
    const seed = /* @__PURE__ */ __name((monto, divisionValor) => {
      if (divisionValor != null) return String(divisionValor);
      if (divisionTipo === "iguales") return "";
      if (divisionTipo === "pct") return String(t.monto ? Math.round(monto / t.monto * 1e3) / 10 : 0);
      return String(monto);
    }, "seed");
    if (t.pagador || deboRow) {
      ...
      const customValues2 = {};
      customValues2["tu"] = seed(monto, deboRow ? deboRow.divisionValor : void 0);
      customValues2[pagadoPorId] = seed(t.monto - monto, t.pagadorDivisionValor);
      return {
        txId: t.id,
        groupId: null,
        divisionTipo,
    ...
    personaRows.forEach((p) => {
      customValues[p.persona] = seed(p.monto || 0, p.divisionValor);
    });
    return {
      txId: t.id,
      groupId: null,
  ...
  __name(rawDivisionValor, "rawDivisionValor");
  ...
  function safeEvalExpr(raw) {
    ...
    __name(parseFactor, "parseFactor");
    try {
  ...
  __name(safeEvalExpr, "safeEvalExpr");
  ...
  __name(monthAddStr, "monthAddStr");
  function monthLabelFor(ym) {
    const [y, m] = ym.split("-").map(Number);
    const name = MONTHS_LONG[m - 1];
    return name.charAt(0).toUpperCase() + name.slice(1) + " " + y;
  }
  __name(monthLabelFor, "monthLabelFor");
  ...
  __name(ensureMonthExists, "ensureMonthExists");
  function dateForInstallment(rootFecha, monthsAhead) {
  ...
  __name(dateForInstallment, "dateForInstallment");
  function regenerateInstallmentsFor(rootId) {
    ...
    if (root && root.cuotas && root.cuotas.total > 1) {
      for (let k = 2; k <= root.cuotas.total; k++) {
        ...
        TRANSACTIONS.push({
          ...
          fecha,
          hora: root.hora,
          comercio: root.comercio,
          monto: root.monto,
          medio: root.medio,
          tipo: root.tipo,
          recurrencia: root.recurrencia,
          estado: "confirmado",
  ...
  __name(regenerateInstallmentsFor, "regenerateInstallmentsFor");
  ...
  var CATEGORY_SEED_DEFAULTS = (function() {
    const out = {};
    Object.keys(CATEGORIES).forEach(function(k) {
      if (CATEGORIES[k].tipo !== "inversion") out[k] = Object.assign({}, CATEGORIES[k]);
    });
    return out;
  })();
  var PAYMENT_METHODS = {
  ...
  var CONTACTS = ["Cata", "Fran", "Pancho", "Mam\xE1"];
  var BUDGETS = {
    supermercado: { meta: 18e4, alertas: { 80: true, 90: true, 100: true } },
    transporte: { meta: 9e4, alertas: { 80: true, 90: false, 100: true } },
    restoranes: { meta: 6e4, alertas: { 80: true, 90: true, 100: true } },
  ...
  var monthlyBudgetTotal = 9e5;
  var SPENDING_GOAL_PCT = { fijo: 45, variable: 17 };
  var TRANSFER_INFO = { nombre: "", rut: "", banco: "", tipoCuenta: "", numeroCuenta: "", email: "" };
  var goalIdCounter = 5;
  var importIdCounter3 = 0;
  var INVESTMENT_GOALS = [
    {
      id: "m1",
      nombre: "Fondo de emergencia",
      montoObjetivo: 3e6,
      aporteMensualMeta: 15e4,
      plataformaId: "banco_chile",
      plazo: "corto",
      comision: null,
    ...
    // scratch (startingAmount:0) since there's no pre-app history to seed for these three.
    {
      ...
      nombre: "APV Fintual",
      montoObjetivo: 3e6,
      aporteMensualMeta: 1e5,
      plataformaId: "fintual",
      plazo: "largo",
      comision: null,
      startMonth: "2026-04",
      startingAmount: 0,
    ...
    {
      id: "m4",
      nombre: "Portafolio Racional",
      montoObjetivo: 15e5,
      aporteMensualMeta: 5e4,
      plataformaId: "racional",
      plazo: "largo",
      comision: null,
      startMonth: "2026-04",
      startingAmount: 0,
    ...
    {
      id: "m5",
      nombre: "Cripto especulativo",
      montoObjetivo: 3e5,
      aporteMensualMeta: 2e4,
      plataformaId: "buda",
      plazo: "corto",
      comision: null,
      startMonth: "2026-04",
  ...
  var PLATFORM_DATA = {
    fintual: {
      valorHistorial: { "2026-04": 81600, "2026-05": 185400, "2026-06": 291200, "2026-07": 395600, "2026-08": 504e3 },
    ...
    racional: {
      valorHistorial: { "2026-04": 40800, "2026-05": 91800, "2026-06": 142800, "2026-07": 143500, "2026-08": 206e3 },
    ...
    banco_chile: {
      ...
      valorHistorial: { "2026-04": 37e5, "2026-05": 415e4, "2026-06": 45e5, "2026-07": 5e6, "2026-08": 55e5 },
    ...
    buda: {
      valorHistorial: { "2026-04": 0, "2026-05": 0, "2026-06": 0, "2026-07": 46e3, "2026-08": 17500 },
  ...
  __name(computeDefaultPlanBase, "computeDefaultPlanBase");
  function computeDefaultGoalPcts(base) {
    const out = {};
  ...
  __name(computeDefaultGoalPcts, "computeDefaultGoalPcts");
  ...
  __name(getPlannerDefaults, "getPlannerDefaults");
  var TRANSACTIONS = [
    { id: "t1", fecha: "2026-08-28", hora: "09:12", comercio: "Jumbo \xD1u\xF1oa", monto: 45e3, medio: "debito_bci", tipo: "gasto", recurrencia: "variable", estado: "confirmado", categorias: [{ cat: "supermercado", monto: 31500 }, { cat: "hogar", monto: 13500 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t2", fecha: "2026-08-28", hora: "08:05", comercio: "Copec Providencia", monto: 18e3, medio: "visa_bch", tipo: "gasto", recurrencia: "variable", estado: "confirmado", categorias: [{ cat: "transporte", monto: 18e3 }], porCobrar: [], reglaAuto: true, nota: "" },
    ...
    { id: "t4", fecha: "2026-08-27", hora: "13:15", comercio: "Copec Las Condes", monto: 22e3, medio: "visa_bch", tipo: "gasto", recurrencia: "variable", estado: "confirmado", categorias: [{ cat: "transporte", monto: 22e3 }], porCobrar: [], reglaAuto: true, nota: "" },
    { id: "t5", fecha: "2026-08-26", hora: "21:03", comercio: "Restobar Lastarria", monto: 64e3, medio: "visa_bch", tipo: "gasto", recurrencia: "variable", estado: "por_cobrar", categorias: [{ cat: "restoranes", monto: 64e3 }], porCobrar: [{ persona: "Cata", monto: 21333, pagado: true, tipo: "persona", montoRecibido: 21333, linkedTxId: null }, { persona: "Fran", monto: 21333, pagado: true, tipo: "persona", montoRecibido: 21333, linkedTxId: "t72" }], reglaAuto: false, nota: "Cumplea\xF1os Cata" },
    { id: "t6", fecha: "2026-08-25", hora: "07:50", comercio: "Sueldo Agosto", monto: 125e4, medio: "cuenta_vista", tipo: "ingreso", recurrencia: "mensual", estado: "confirmado", categorias: [{ cat: "sueldo", monto: 125e4 }], porCobrar: [], reglaAuto: false, nota: "" },
    ...
    { id: "t9", fecha: "2026-08-22", hora: "10:00", comercio: "Aporte Fintual", monto: 1e5, medio: "cuenta_vista", tipo: "inversion", recurrencia: "mensual", estado: "confirmado", categorias: [{ cat: "m3", monto: 1e5 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t70", fecha: "2026-08-16", hora: "11:40", comercio: "Retiro parcial Buda", monto: -2e4, medio: "cuenta_vista", tipo: "inversion", recurrencia: "variable", estado: "confirmado", categorias: [{ cat: "m5", monto: -2e4 }], porCobrar: [], reglaAuto: false, nota: "Retiro" },
    ...
    { id: "t10", fecha: "2026-08-21", hora: "18:22", comercio: "Cine Hoyts Costanera", monto: 12e3, medio: "visa_bch", tipo: "gasto", recurrencia: "variable", estado: "confirmado", categorias: [{ cat: "entretenimiento", monto: 12e3 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t11", fecha: "2026-08-20", hora: "09:00", comercio: "Freelance dise\xF1o web", monto: 18e4, medio: "cuenta_vista", tipo: "ingreso", recurrencia: "variable", estado: "confirmado", categorias: [{ cat: "pololos_extra", monto: 18e4 }], porCobrar: [], reglaAuto: false, nota: "" },
    ...
    { id: "t31", fecha: "2026-08-10", hora: "16:20", comercio: "Falabella \xB7 Notebook", monto: 3e4, medio: "visa_bch", tipo: "gasto", recurrencia: "variable", estado: "confirmado", categorias: [{ cat: "hogar", monto: 3e4 }], porCobrar: [], reglaAuto: false, nota: "", cuotas: { total: 3, montoTotal: 9e4 } },
    { id: "t32", fecha: "2026-08-07", hora: "13:30", comercio: "Sushi Bar Vitacura", monto: 36e3, medio: "debito_bci", tipo: "gasto", recurrencia: "variable", estado: "por_cobrar", categorias: [{ cat: "restoranes", monto: 36e3 }], porCobrar: [{ persona: "Pancho", monto: 18e3, pagado: true, tipo: "persona", montoRecibido: 18e3, linkedTxId: null }], reglaAuto: false, nota: "" },
    { id: "t12", fecha: "2026-08-18", hora: "11:00", comercio: "L\xEDder Express", monto: 23800, medio: "debito_bci", tipo: "gasto", recurrencia: "variable", estado: "confirmado", categorias: [{ cat: "supermercado", monto: 23800 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t13", fecha: "2026-08-15", hora: "14:10", comercio: "Transferencia entre mis cuentas", monto: 2e5, medio: "cuenta_vista", tipo: "gasto", recurrencia: "variable", estado: "no_es_gasto", categorias: [], porCobrar: [], reglaAuto: false, nota: "Traspaso, no es un gasto real" },
    ...
    { id: "t15", fecha: "2026-08-05", hora: "10:00", comercio: "Arriendo Depto", monto: 38e4, medio: "cuenta_vista", tipo: "gasto", recurrencia: "mensual", estado: "confirmado", categorias: [{ cat: "hogar", monto: 38e4 }], porCobrar: [], reglaAuto: true, nota: "" },
    { id: "t16", fecha: "2026-08-01", hora: "09:00", comercio: "Aporte Racional", monto: 6e4, medio: "cuenta_vista", tipo: "inversion", recurrencia: "mensual", estado: "confirmado", categorias: [{ cat: "m4", monto: 6e4 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t17", fecha: "2026-07-31", hora: "10:00", comercio: "Aporte Banco de Chile", monto: 5e4, medio: "cuenta_vista", tipo: "inversion", recurrencia: "mensual", estado: "confirmado", categorias: [{ cat: "m2", monto: 5e4 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t18", fecha: "2026-07-28", hora: "09:00", comercio: "Sueldo Julio", monto: 125e4, medio: "cuenta_vista", tipo: "ingreso", recurrencia: "mensual", estado: "confirmado", categorias: [{ cat: "sueldo", monto: 125e4 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t19", fecha: "2026-07-26", hora: "20:00", comercio: "Jumbo \xD1u\xF1oa", monto: 52e3, medio: "debito_bci", tipo: "gasto", recurrencia: "variable", estado: "confirmado", categorias: [{ cat: "supermercado", monto: 52e3 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t20", fecha: "2026-07-24", hora: "13:00", comercio: "Copec Las Condes", monto: 21e3, medio: "visa_bch", tipo: "gasto", recurrencia: "variable", estado: "confirmado", categorias: [{ cat: "transporte", monto: 21e3 }], porCobrar: [], reglaAuto: true, nota: "" },
    ...
    { id: "t22", fecha: "2026-07-18", hora: "09:00", comercio: "Freelance dise\xF1o web", monto: 15e4, medio: "cuenta_vista", tipo: "ingreso", recurrencia: "variable", estado: "confirmado", categorias: [{ cat: "pololos_extra", monto: 15e4 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t23", fecha: "2026-07-15", hora: "11:00", comercio: "Farmacias Cruz Verde", monto: 9800, medio: "debito_bci", tipo: "gasto", recurrencia: "variable", estado: "por_cobrar", categorias: [{ cat: "salud", monto: 9800 }], porCobrar: [{ persona: "Seguro complementario", monto: 8e3, pagado: true, tipo: "reembolso", montoRecibido: 7500, linkedTxId: "t71" }], reglaAuto: false, nota: "" },
    ...
    { id: "t25", fecha: "2026-07-10", hora: "10:00", comercio: "Arriendo Depto", monto: 38e4, medio: "cuenta_vista", tipo: "gasto", recurrencia: "mensual", estado: "confirmado", categorias: [{ cat: "hogar", monto: 38e4 }], porCobrar: [], reglaAuto: true, nota: "" },
    { id: "t26", fecha: "2026-07-08", hora: "12:00", comercio: "Aporte Fintual", monto: 1e5, medio: "cuenta_vista", tipo: "inversion", recurrencia: "mensual", estado: "confirmado", categorias: [{ cat: "m3", monto: 1e5 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t27", fecha: "2026-07-05", hora: "21:00", comercio: "Cine Hoyts Costanera", monto: 12e3, medio: "visa_bch", tipo: "gasto", recurrencia: "variable", estado: "confirmado", categorias: [{ cat: "entretenimiento", monto: 12e3 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t28", fecha: "2026-07-03", hora: "10:00", comercio: "Aporte Buda", monto: 4e4, medio: "cuenta_vista", tipo: "inversion", recurrencia: "variable", estado: "confirmado", categorias: [{ cat: "m5", monto: 4e4 }], porCobrar: [], reglaAuto: false, nota: "" },
    ...
    { id: "t33", fecha: "2026-06-25", hora: "07:50", comercio: "Sueldo Junio", monto: 122e4, medio: "cuenta_vista", tipo: "ingreso", recurrencia: "mensual", estado: "confirmado", categorias: [{ cat: "sueldo", monto: 122e4 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t34", fecha: "2026-06-18", hora: "09:00", comercio: "Freelance dise\xF1o web", monto: 9e4, medio: "cuenta_vista", tipo: "ingreso", recurrencia: "variable", estado: "confirmado", categorias: [{ cat: "pololos_extra", monto: 9e4 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t35", fecha: "2026-06-20", hora: "19:10", comercio: "Jumbo \xD1u\xF1oa", monto: 48e3, medio: "debito_bci", tipo: "gasto", recurrencia: "variable", estado: "confirmado", categorias: [{ cat: "supermercado", monto: 48e3 }], porCobrar: [], reglaAuto: false, nota: "" },
    ...
    { id: "t37", fecha: "2026-06-14", hora: "19:40", comercio: "Rappi Delivery", monto: 16e3, medio: "visa_bch", tipo: "gasto", recurrencia: "variable", estado: "confirmado", categorias: [{ cat: "restoranes", monto: 16e3 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t38", fecha: "2026-06-10", hora: "10:00", comercio: "Arriendo Depto", monto: 38e4, medio: "cuenta_vista", tipo: "gasto", recurrencia: "mensual", estado: "confirmado", categorias: [{ cat: "hogar", monto: 38e4 }], porCobrar: [], reglaAuto: false, nota: "" },
    ...
    { id: "t41", fecha: "2026-06-06", hora: "21:00", comercio: "Cine Hoyts Costanera", monto: 12e3, medio: "visa_bch", tipo: "gasto", recurrencia: "variable", estado: "confirmado", categorias: [{ cat: "entretenimiento", monto: 12e3 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t42", fecha: "2026-06-05", hora: "10:00", comercio: "Aporte Fintual", monto: 1e5, medio: "cuenta_vista", tipo: "inversion", recurrencia: "mensual", estado: "confirmado", categorias: [{ cat: "m3", monto: 1e5 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t43", fecha: "2026-06-03", hora: "10:00", comercio: "Aporte Racional", monto: 5e4, medio: "cuenta_vista", tipo: "inversion", recurrencia: "mensual", estado: "confirmado", categorias: [{ cat: "m4", monto: 5e4 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t44", fecha: "2026-06-02", hora: "10:00", comercio: "Aporte Banco de Chile", monto: 4e4, medio: "cuenta_vista", tipo: "inversion", recurrencia: "mensual", estado: "confirmado", categorias: [{ cat: "m1", monto: 4e4 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t45", fecha: "2026-05-25", hora: "07:50", comercio: "Sueldo Mayo", monto: 12e5, medio: "cuenta_vista", tipo: "ingreso", recurrencia: "mensual", estado: "confirmado", categorias: [{ cat: "sueldo", monto: 12e5 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t46", fecha: "2026-05-16", hora: "09:00", comercio: "Freelance dise\xF1o web", monto: 6e4, medio: "cuenta_vista", tipo: "ingreso", recurrencia: "variable", estado: "confirmado", categorias: [{ cat: "pololos_extra", monto: 6e4 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t47", fecha: "2026-05-19", hora: "19:30", comercio: "Jumbo \xD1u\xF1oa", monto: 50500, medio: "debito_bci", tipo: "gasto", recurrencia: "variable", estado: "confirmado", categorias: [{ cat: "supermercado", monto: 50500 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t48", fecha: "2026-05-21", hora: "08:10", comercio: "Copec Providencia", monto: 19e3, medio: "visa_bch", tipo: "gasto", recurrencia: "variable", estado: "confirmado", categorias: [{ cat: "transporte", monto: 19e3 }], porCobrar: [], reglaAuto: false, nota: "" },
    ...
    { id: "t51", fecha: "2026-05-10", hora: "10:00", comercio: "Arriendo Depto", monto: 38e4, medio: "cuenta_vista", tipo: "gasto", recurrencia: "mensual", estado: "confirmado", categorias: [{ cat: "hogar", monto: 38e4 }], porCobrar: [], reglaAuto: false, nota: "" },
    ...
    { id: "t54", fecha: "2026-05-04", hora: "21:00", comercio: "Cine Hoyts Costanera", monto: 12e3, medio: "visa_bch", tipo: "gasto", recurrencia: "variable", estado: "confirmado", categorias: [{ cat: "entretenimiento", monto: 12e3 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t55", fecha: "2026-05-05", hora: "10:00", comercio: "Aporte Fintual", monto: 1e5, medio: "cuenta_vista", tipo: "inversion", recurrencia: "mensual", estado: "confirmado", categorias: [{ cat: "m3", monto: 1e5 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t56", fecha: "2026-05-03", hora: "10:00", comercio: "Aporte Racional", monto: 5e4, medio: "cuenta_vista", tipo: "inversion", recurrencia: "mensual", estado: "confirmado", categorias: [{ cat: "m4", monto: 5e4 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t57", fecha: "2026-05-02", hora: "10:00", comercio: "Aporte Banco de Chile", monto: 3e4, medio: "cuenta_vista", tipo: "inversion", recurrencia: "mensual", estado: "confirmado", categorias: [{ cat: "m2", monto: 3e4 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t58", fecha: "2026-04-25", hora: "07:50", comercio: "Sueldo Abril", monto: 12e5, medio: "cuenta_vista", tipo: "ingreso", recurrencia: "mensual", estado: "confirmado", categorias: [{ cat: "sueldo", monto: 12e5 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t59", fecha: "2026-04-14", hora: "12:00", comercio: "Venta bicicleta", monto: 4e4, medio: "efectivo", tipo: "ingreso", recurrencia: "variable", estado: "confirmado", categorias: [], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t60", fecha: "2026-04-20", hora: "19:20", comercio: "Jumbo \xD1u\xF1oa", monto: 46e3, medio: "debito_bci", tipo: "gasto", recurrencia: "variable", estado: "confirmado", categorias: [{ cat: "supermercado", monto: 46e3 }], porCobrar: [], reglaAuto: false, nota: "" },
    ...
    { id: "t62", fecha: "2026-04-08", hora: "19:00", comercio: "Rappi Delivery", monto: 11e3, medio: "visa_bch", tipo: "gasto", recurrencia: "variable", estado: "confirmado", categorias: [{ cat: "restoranes", monto: 11e3 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t63", fecha: "2026-04-10", hora: "10:00", comercio: "Arriendo Depto", monto: 38e4, medio: "cuenta_vista", tipo: "gasto", recurrencia: "mensual", estado: "confirmado", categorias: [{ cat: "hogar", monto: 38e4 }], porCobrar: [], reglaAuto: false, nota: "" },
    ...
    { id: "t66", fecha: "2026-04-03", hora: "21:00", comercio: "Cine Hoyts Costanera", monto: 12e3, medio: "visa_bch", tipo: "gasto", recurrencia: "variable", estado: "confirmado", categorias: [{ cat: "entretenimiento", monto: 12e3 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t67", fecha: "2026-04-05", hora: "10:00", comercio: "Aporte Fintual", monto: 8e4, medio: "cuenta_vista", tipo: "inversion", recurrencia: "mensual", estado: "confirmado", categorias: [{ cat: "m3", monto: 8e4 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t68", fecha: "2026-04-03", hora: "10:00", comercio: "Aporte Racional", monto: 4e4, medio: "cuenta_vista", tipo: "inversion", recurrencia: "mensual", estado: "confirmado", categorias: [{ cat: "m4", monto: 4e4 }], porCobrar: [], reglaAuto: false, nota: "" },
    { id: "t69", fecha: "2026-04-02", hora: "10:00", comercio: "Aporte Banco de Chile", monto: 3e4, medio: "cuenta_vista", tipo: "inversion", recurrencia: "mensual", estado: "confirmado", categorias: [{ cat: "m1", monto: 3e4 }], porCobrar: [], reglaAuto: false, nota: "" }
  ];
  var PLANNER = getPlannerDefaults();
  var GROUPS = [];
  ...
  function currentMonthIndex() {
    ...
    if (!MONTHS.includes(ym)) {
      MONTHS.push(ym);
      MONTHS.sort();
      if (!MONTH_LABEL[ym]) MONTH_LABEL[ym] = monthLabelFor(ym);
    }
    return MONTHS.indexOf(ym);
  }
  __name(currentMonthIndex, "currentMonthIndex");
  ...
  __name(moneyPlain, "moneyPlain");
  function moneyPlainMasked(n) {
    return moneyPlain(n);
  }
  __name(moneyPlainMasked, "moneyPlainMasked");
  ...
  __name(moneyShort, "moneyShort");
  function monthAbbr(monthNum1based) {
    const s = MONTHS_LONG[monthNum1based - 1];
    return s.charAt(0).toUpperCase() + s.slice(1, 3);
  }
  __name(monthAbbr, "monthAbbr");
  var state = {
    tab: "transacciones",
    // transacciones | resumen | menu
    summarySub: "balance",
    // balance | presupuesto | evolucion | inversiones
    filter: "todas",
    // todas | entradas | porcobrar | pendientes
    categoryFilter: null,
    // cat id or null
    categoryFilterMonth: null,
    // 'YYYY-MM', a bare 'YYYY' (Balance año mode -- the whole year, not just one month), or null, set together with categoryFilter from a Balance drill-down
    monthIndex: currentMonthIndex(),
    balancePeriodo: "mes",
    // mes | año — Resumen > Balance shows the current month or the full current year (never a past/future year)
    openTxId: null,
    creatingNew: false,
    draftTx: null,
    splitCategoryMode: {},
    ...
    editingGoalId: null,
    // id of the goal being edited, or 'nueva', or null
    ...
    openPlatformId: null,
    // id of the platform with its accordion expanded in Investments, or null (all closed)
    editingPlatformId: null,
    // id of the platform being edited ("update value"), or null
    platformDraft: { valor: "", tasaAnual: "", comision: "", plazo: "" },
    creatingPlatform: false,
    // true while the "new platform" form is shown
    confirmDeletePlatformId: null,
    ...
    summarySubOrder: ["balance", "presupuesto", "evolucion", "inversiones"],
    // order of Summary sub-tabs, reorderable via drag and drop
    subtabDragId: null,
    // id of the sub-tab currently being dragged, or null
    // ---- Menu ----
    menuSection: null,
    // null | 'categorias' | 'medios' | 'reglas' | 'exportar' | 'respaldo' | 'importar' | 'demo' | 'asesoria' | 'cuenta' | 'importarcorreo' | 'notificaciones'
    emailImportLoaded: false,
    // whether the import code has already been loaded at least once
    emailImportLoading: false,
    emailImportError: null,
    importToken: null,
    // household's import code (households.import_token), for Apps Script and the push Worker
    // ---- Push notifications (new imported transaction, budget alert) ----
    notifLoaded: false,
    // whether this browser's subscription status has already been checked at least once
    notifLoading: false,
    notifError: null,
    notifSubscribed: false,
    // whether THIS browser already has a push subscription saved
    notifBusy: false,
    // subscribing/unsubscribing right now (disables the button)
    notifTestBusy: false,
    ...
    // id of the transaction showing "are you sure you want to delete it?"
    salaryBannerDismissedMonth: null,
    ...
    reconciliar: {
      archivo: null,
      // name of the PDF that was read
      cargando: false,
      error: null,
      tipo: null,
      // 'cuenta_corriente' | 'tarjeta_nacional'
      movimientos: [],
      // [{fecha,detalle,monto,tipoMov,esEspecial,yaRegistrada,idSugerido}]
      pagosTarjeta: null,
      // separate summary for the "CARGO POR PAGO TC" rows
      disponibles: [],
      // statements that arrived by themselves via email, not used yet
      usandoId: null,
      // id of the one about to be opened (asking for the password), or null
      passwordDraft: "",
      errorPassword: null,
      archivoBuffer: null,
    ...
    linkFlow: null,
    // null | {mode:'fromPendiente', expenseTxId, idx} | {mode:'fromIngreso', incomeTxId}
    // ---- Split receipt (real OCR via cloudflare-worker-ocr/, "share by link" step still simulated) ----
    boleta: null,
    // null when the assistant is closed, or {step, expenseTxId, comercio, items, asign} —
    // always tied to an already-existing transaction marked "por cobrar"
    // ---- Shared expenses ----
    workspace: "personal",
    // 'personal' | groupId — which workspace is being viewed right now
    openGroupId: null,
    // groupId whose detail view is open, or null (group list)
    creatingGroup: false,
    // true while the "Create group" form is shown
    ...
    // participantId being renamed right now (no-account only), or null
    editParticipantDraft: "",
    // the name being typed while editingParticipantId is set
    confirmDeleteParticipantId: null,
    // participantId with a pending "sure you want to delete?" ask, or null
    settleWithId: null,
    // participantId being "Settled up" right now, or null
    ...
    // 'gastos' | 'balances' | 'transferencias' -- which sub-tab is open
    openGroupExpenseId: null,
    // id of the SharedExpense whose inline detail card is expanded in "Gastos", or null
    showManualTransferForm: false,
    // true while "Registrar una transferencia" (manual, Tab "Transferencias") is open
    manualTransferDraft: null,
    // null, or {deId, aId, monto, fecha}
  ...
  var subtabDrag = null;
  var suppressNextSubtabClick = false;
  function todayISO() {
    const d = /* @__PURE__ */ new Date();
  ...
  var ultimoRespaldo = null;
  function setUltimoRespaldo2(v) {
    ultimoRespaldo = v;
  }
  __name(setUltimoRespaldo2, "setUltimoRespaldo");
  function setBudgets(v) {
    BUDGETS = v;
  }
  __name(setBudgets, "setBudgets");
  function setMonthlyBudgetTotal(v) {
    monthlyBudgetTotal = v;
  }
  __name(setMonthlyBudgetTotal, "setMonthlyBudgetTotal");
  function setSpendingGoalPct(v) {
    SPENDING_GOAL_PCT = v;
  }
  __name(setSpendingGoalPct, "setSpendingGoalPct");
  function setTransferInfo(v) {
    TRANSFER_INFO = v;
  }
  __name(setTransferInfo, "setTransferInfo");
  function setInvestmentGoals(v) {
    INVESTMENT_GOALS = v;
  }
  __name(setInvestmentGoals, "setInvestmentGoals");
  function setPlatformData(v) {
    PLATFORM_DATA = v;
  }
  __name(setPlatformData, "setPlatformData");
  function setTotalGoalChecks(v) {
    TOTAL_GOAL_CHECKS = v;
  }
  __name(setTotalGoalChecks, "setTotalGoalChecks");
  function setBudgetAlertsSent(v) {
    BUDGET_ALERTS_SENT = v;
  }
  __name(setBudgetAlertsSent, "setBudgetAlertsSent");
  function setPlanner(v) {
    PLANNER = v;
  }
  __name(setPlanner, "setPlanner");
  function setGroups(v) {
    GROUPS = v;
  }
  __name(setGroups, "setGroups");
  function setGroupParticipants(v) {
    GROUP_PARTICIPANTS = v;
  }
  __name(setGroupParticipants, "setGroupParticipants");
  function setSharedExpenses(v) {
    SHARED_EXPENSES = v;
  }
  __name(setSharedExpenses, "setSharedExpenses");
  function setPaidBalances(v) {
    PAID_BALANCES = v;
  }
  __name(setPaidBalances, "setPaidBalances");
  function setCategoryMappings(v) {
    CATEGORY_MAPPINGS = v;
  }
  __name(setCategoryMappings, "setCategoryMappings");
  function setGoalIdCounter(v) {
    goalIdCounter = v;
  }
  __name(setGoalIdCounter, "setGoalIdCounter");
  function setImportIdCounter(v) {
    importIdCounter3 = v;
  }
  __name(setImportIdCounter, "setImportIdCounter");
  function nextImportId() {
    importIdCounter3++;
    return importIdCounter3;
  }
  __name(nextImportId, "nextImportId");
  function setSubtabDrag(v) {
    subtabDrag = v;
  }
  __name(setSubtabDrag, "setSubtabDrag");
  function setSuppressNextSubtabClick(v) {
    suppressNextSubtabClick = v;
  }
  __name(setSuppressNextSubtabClick, "setSuppressNextSubtabClick");
  ...
  window.__debug = {
    TRANSACTIONS: TRANSACTIONS, CATEGORIES: CATEGORIES, PAYMENT_METHODS: PAYMENT_METHODS, BUDGETS: BUDGETS,
    INVESTMENT_GOALS: INVESTMENT_GOALS, PLATFORM_DATA: PLATFORM_DATA,
    PLANNER: PLANNER, MONTHS: MONTHS, MONTH_LABEL: MONTH_LABEL,
    state: state, render: render, monthTotals: monthTotals, yearTotals: yearTotals,
    metaAcumuladoActual: metaAcumuladoActual, annualInvestmentGoalProgress: annualInvestmentGoalProgress,
    metaAportadoNeto: metaAportadoNeto, metaMonths: metaMonths, metaHistorialAt: metaHistorialAt,
    metaContribTxs: metaContribTxs, metaContribAmount: metaContribAmount,
    investmentCatOptions: investmentCatOptions, generalCatIdFor: generalCatIdFor,
    goalsForPlatform: goalsForPlatform, goalCapablePlatformIds: goalCapablePlatformIds,
    platformIds: platformIds, platformAportadoNeto: platformAportadoNeto,
    platformAportadoNetoHastaMes: platformAportadoNetoHastaMes, valorTotalEnMesONull: valorTotalEnMesONull,
    aportadoAcumuladoHastaMesONull: aportadoAcumuladoHastaMesONull,
    platformCurrentValue: platformCurrentValue, computeDefaultPlanBase: computeDefaultPlanBase,
    ensurePaymentMethodForSuggestion: ensurePaymentMethodForSuggestion, guessPaymentMethodIdFromSuggestion: guessPaymentMethodIdFromSuggestion,
    lastSalaryTx: lastSalaryTx, currentMonthHasSalary: currentMonthHasSalary, todayISO: todayISO,
    parseStatementPDF: parseStatementPDF, findSimilarTx: findSimilarTx, createTxFromMovement: createTxFromMovement,
    SPENDING_GOAL_PCT: SPENDING_GOAL_PCT, investmentGoalPct: investmentGoalPct, monthlyInvestmentGoalCLP: monthlyInvestmentGoalCLP,
    ...
    reimbursementExcess: reimbursementExcess, applyUnexpectedReimbursement: applyUnexpectedReimbursement,
    ...
    importStatementRows: importStatementRows, catInfo: catInfo, writeOffReceivable: writeOffReceivable,
    catIconMarkup: catIconMarkup, updateSyncIndicator: updateSyncIndicator,
    ...
    checkBudgetPushAlerts: checkBudgetPushAlerts, enviarPushHogar: enviarPushHogar,
    ...
    useImportedStatement: useImportedStatement,
    enterDemoMode: enterDemoMode, exitDemoMode: exitDemoMode, isDemoActive: isDemoActive,
    preserveGroupTransactionsBeforeUnlink: preserveGroupTransactionsBeforeUnlink,
    navPush: navPush, navPop: navPop, navPeek: navPeek, navDepth: navDepth,
    navClear: navClear, navClearType: navClearType, navPopIfTop: navPopIfTop,
    navStackSnapshot: navStackSnapshot, navigateBack: navigateBack,
    ...
    get SHARED_EXPENSES(){ return SHARED_EXPENSES; }, set SHARED_EXPENSES(v){ SHARED_EXPENSES = v; },
    get PAID_BALANCES(){ return PAID_BALANCES; }, set PAID_BALANCES(v){ PAID_BALANCES = v; },
    get CATEGORY_MAPPINGS(){ return CATEGORY_MAPPINGS; }, set CATEGORY_MAPPINGS(v){ CATEGORY_MAPPINGS = v; },
    get GROUP_CATEGORY_RULES(){ return GROUP_CATEGORY_RULES; }, set GROUP_CATEGORY_RULES(v){ GROUP_CATEGORY_RULES = v; },
    shareDraftForTx: shareDraftForTx,
    groupBalances: groupBalances, suggestedTransfers: suggestedTransfers, splitEqually: splitEqually,
    participantsOfGroup: participantsOfGroup, expensesOfGroup: expensesOfGroup,
    syncSharedExpenses: syncSharedExpenses, participantIdForUser: participantIdForUser,
    classifySharedExpenseFromOthers: classifySharedExpenseFromOthers, ensureSharedExpensePaymentMethod: ensureSharedExpensePaymentMethod,
    registerPaidBalance: registerPaidBalance, categoryForSharedExpense: categoryForSharedExpense,
    ...
    get inPasswordRecovery(){ return inPasswordRecovery; }, set inPasswordRecovery(v){ inPasswordRecovery = v; },
    onAuthenticated: onAuthenticated, showAuthRecoveryForm: showAuthRecoveryForm,
    ...
    buildIntegrantesRows: buildIntegrantesRows, buildTransaccionesRows: buildTransaccionesRows,
    buildDetalleDivisionRows: buildDetalleDivisionRows, buildTransferenciasRows: buildTransferenciasRows,
    groupExportSheets: groupExportSheets, buildGroupExportWorkbookArrayBuffer: buildGroupExportWorkbookArrayBuffer,
