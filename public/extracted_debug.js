(() => {
  ...
  function renderGroupBalancesTab(groupId) {
    ...
    const personSection = '<div class="sheet-block card" style="padding:16px;margin-bottom:14px;"><div class="sheet-block-title">Saldo por persona</div>' + balances.map((s) => {
      ...
      return '<div class="split-row" style="align-items:center;">' + avatarHtml(s.nombre, s.color) + '<span style="flex:1;margin-left:10px;">' + s.nombre + (isMe ? " (t\xFA)" : "") + '</span><span class="tabular" style="color:' + (s.balance > 0 ? "var(--income-ink)" : s.balance < 0 ? "var(--expense-ink)" : "var(--text-secondary)") + ';font-weight:600;">' + (s.balance === 0 ? "Al d\xEDa" : (s.balance > 0 ? "Le deben " : "Debe ") + money(Math.abs(s.balance))) + "</span></div>";
