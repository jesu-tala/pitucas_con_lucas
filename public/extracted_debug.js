      ...
      if (group === "division-tipo" && state.shareDraft) {
        ...
        if (t2) {
          ...
          d.participantesIncluidos.forEach((id) => {
            if (val === "pct") d.customValues[id] = String(t2.monto ? Math.round((base[id] || 0) / t2.monto * 1e3) / 10 : 0);
            else if (val === "montos") d.customValues[id] = String(base[id] || 0);
            else d.customValues[id] = String(base[id] || 0);
          });
        }
        renderSheet();
        return;
