...
// the header echo (data-nota-echo) shows/hides correctly, and it survives a real close+reopen
...
(async () => {
  ...
  // t3 = Uber, sin nota por defecto en la maqueta.
  await page.click('[data-tx="t3"]');
  await page.waitForTimeout(200);

  const antes = await page.evaluate(() => {
    const echo = document.querySelector('.sheet-top [data-nota-echo]');
    return { notaTx: window.__debug.TX.find(t => t.id === 't3').nota, echoVisible: echo ? getComputedStyle(echo).display !== 'none' : null };
  });
  ...
  const tras = await page.evaluate(() => {
    const echo = document.querySelector('.sheet-top [data-nota-echo]');
    return {
      notaTx: window.__debug.TX.find(t => t.id === 't3').nota,
  ...
  // (c) cerrar y volver a abrir por el flujo real de UI, no tocando el estado a mano.
