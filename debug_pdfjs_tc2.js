...
(async () => {
  ...
  const uber = allWords.filter(w => /UBER/i.test(w.text));
  console.log('UBER matches:', JSON.stringify(uber, null, 1));
  // dump full rows near first uber match top
  if (uber.length) {
    const t = uber[0].top, pg = uber[0].page;
    const rowWords = allWords.filter(w => w.page===pg && Math.abs(w.top - t) <= 5).sort((a,b)=>a.x0-b.x0);
    console.log('Full row words:', JSON.stringify(rowWords, null, 1));
  }
  await browser.close();
})();
