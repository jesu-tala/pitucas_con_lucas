/**
 * Pitucas sin lucas — servicio de notificaciones push + lector de boletas (OCR)
 * ---------------------------------------------------------------------------
 * Este archivo va completo, tal cual, en un Cloudflare Worker (gratis, sin
 * necesidad de dominio propio ni instalar nada en tu computador). Un mismo
 * Worker sirve DOS funciones separadas de la app (dos endpoints distintos) --
 * no hace falta crear dos Workers.
 *
 ...
 *     → pestaña "Workers" → "Create Worker" (dale un nombre, ej. pitucas-worker).
 ...
 *     estas 6 (todas como "secret" salvo que digan lo contrario):
 ...
 *       GEMINI_API_KEY         = tu clave "AIza..." de https://aistudio.google.com/apikey
 *  4. Copia la URL del Worker (algo como https://pitucas-worker.tu-cuenta.workers.dev)
 *     — la vas a necesitar en TRES lugares: en tu Apps Script (variable
 *     PUSH_WORKER_URL), y en src/supabase.ts de la app (tanto PUSH_WORKER_URL
 *     como OCR_WORKER_URL -- son el mismo Worker, así que van con la misma URL).
 *
 * Qué expone: dos endpoints --
 *   POST /notify        lo llama tu Apps Script cada vez que importa transacciones
 *                        nuevas, y este Worker les manda la notificación a todos
 *                        los dispositivos suscritos de tu hogar.
 *   POST /leer-boleta    lo llama la app cada vez que escaneas una boleta -- le
 *                        manda la foto a Gemini y devuelve los items separados.
 * Ninguno de los dos expone nada útil para quien no tenga el código de tu hogar
 * (ambos validan contra Supabase en cada llamada).
 */
...
// ---------- Lector de boletas: mandar la foto a Gemini y pedirle un JSON estructurado ----------
// generationConfig.responseSchema fuerza a Gemini a devolver el JSON con esta forma exacta
// (sin que tenga que "adivinar" el formato por el prompt solo) --
...
export default {
  async fetch(request, env) {
    ...
    // salir del navegador.
    if (request.method === 'OPTIONS' && (url.pathname === '/notify' || url.pathname === '/leer-boleta')) {
    ...
    return new Response('Pitucas sin lucas — notificaciones push + lector de boletas. Nada que ver por acá directamente.', { status: 200, headers: CORS_HEADERS_ });
  }
};
