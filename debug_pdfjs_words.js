...
(async () => {
  ...
  const rows = await page.evaluate(async (b64) => {
    ...
    if (!window.pdfjsLib.GlobalWorkerOptions.workerSrc) window.pdfjsLib.GlobalWorkerOptions.workerSrc = 'pdf.worker.min.js';
    ...
    return { height: viewport.height, words: words.slice(20, 90) };
  }, b64);
  console.log('viewport height:', rows.height);
  rows.words.forEach(w => console.log(w.top.toFixed(1).padStart(7), w.x0.toFixed(1).padStart(7), JSON.stringify(w.text)));
  await browser.close();
})();
