...
(function(){
  ...
  type EstadoTx = 'confirmado' | 'pendiente' | 'por_cobrar' | 'no_es_gasto';
  type Recurrencia = 'variable' | 'mensual';

  interface CategoriaAsignada { cat: string; monto: number; }
  ...
  interface CuotasInfo { total: number; }
  ...
  type DivisionTipo = 'iguales' | 'montos' | 'pct';
  ...
  const MEDIOS: Record<string, Medio> = {
  ...
  let GRUPOS: Grupo[] = [];
  let GRUPO_PARTICIPANTES: GrupoParticipante[] = [];
  let GASTOS_COMPARTIDOS: GastoCompartido[] = [];
  let SALDOS_PAGADOS: SaldoPagado[] = [];
  let MAPEO_CATEGORIAS: MapeoCategoria[] = [];
  ...
  interface AdvFilters { cats: string[]; medios: string[]; dateFrom: string; dateTo: string; }
  ...
  function participantesDeGrupo(grupoId: string): GrupoParticipante[] {
    return GRUPO_PARTICIPANTES.filter(p=>p.grupo_id===grupoId);
  }
  function gastosDeGrupo(grupoId: string): GastoCompartido[] {
    return GASTOS_COMPARTIDOS.filter(g=>g.grupo_id===grupoId);
  }
  function repartoDeGasto(g: GastoCompartido): GastoReparto[] {
    return g.reparto || [];
  }
  ...
  function saldoGrupo(grupoId: string): SaldoParticipante[] {
  ...
  function transferenciasSugeridas(grupoId: string): TransferenciaSugerida[] {
  ...
  function repartirIguales(monto: number, participanteIds: string[]): Record<string, number> {
  ...
  function bucketizeRow(row, cols): Record<string, string> {
  ...
  function crearTxDesdeMovimiento(m, opts?: {noEsGasto?: boolean}){
    opts = opts || {};
    const reglaByComercio = {};
  ...
  function txDesdeImportEmail(row): Transaccion {
    const reglaByComercio = {};
    reglasAgrupadas().forEach(function(r){ reglaByComercio[r.comercio] = r; });
  ...
  async function cargarGastosCompartidos(){
    ...
    try{
      ...
      GRUPOS = grupos||[]; SALDOS_PAGADOS = saldos||[]; MAPEO_CATEGORIAS = mapeo||[];
      const grupoIds = GRUPOS.map(g=>g.id);
      if(!grupoIds.length){ GRUPO_PARTICIPANTES=[]; GASTOS_COMPARTIDOS=[]; sincronizarGastosCompartidos(); return; }
  ...
  phone.addEventListener('click', function(e: any){
    ...
    if(suppressNextSubtabClick){ suppressNextSubtabClick = false; return; }
  ...
  function resetToLoggedOutState(){
    suppressAutoSave = true;
    applyStateBlob(emptyAppStateBlob());
    GRUPOS = []; GRUPO_PARTICIPANTES = []; GASTOS_COMPARTIDOS = []; SALDOS_PAGADOS = []; MAPEO_CATEGORIAS = [];
    state.espacio = 'personal'; state.grupoAbiertoId = null;
    if(gruposRealtimeChannel && sb){ sb.removeChannel(gruposRealtimeChannel); gruposRealtimeChannel = null; }
